...
(defun ids-update-index (&optional (s t))
  ...
  (let (products ucs)
    ...
    (some-in-character-feature
	 ...
	 (dolist (comp (remove c (char-ucs-chars c)))
	   ...
	   (dolist (p_c (get-char-attribute comp 'ideographic-products))
	     ...
	     ;; (format s "[~a (~X) : ~a] ...~a (~X)~%"
	     ;; 	   c (char-id c) v p_c (char-id p_c))
	     ;; (setq products (adjoin p_c products))
	     (adjoin-char-attribute c "ideographic-products" p_c)
	     ))
	 ;; (format s "[~a (~X) : ~a] -> ~a~%" c (char-id c) v products)
    ...
    (some-in-character-feature
     (lambda (c v)
       (setq products (get-char-attribute c 'ideographic-products))
       (dolist (comp (delete c (char-ucs-chars c)))
	 (put-char-attribute
