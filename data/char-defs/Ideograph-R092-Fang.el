;; -*- coding: utf-8-mcs-er -*-
(define-char
  '((<-denotational@usage
     ((==>ucs@bucs	  . #x7259)	; &BUCS+7259;
      ))
    (ideographic-radical . 92)	; ⽛
    (ideographic-strokes . 0)
    (=ucs		. #x7259)	; 牙
    (=mj		. 016801)	; &I-MJ016801;
    (=adobe-japan1-0	. 01383)	; &I-AJ1-01383;
    (=jis-x0208		. #x3267)	; &I-J90-3267; [18-71]
    (=gb2312		. #x5140)	; &I-G0-5140; [49-32]
    (=ks-x1001		. #x6433)	; &I-K0-6433; [68-19]
    (=cns11643-1	. #x455B)	; &I-C1-455B; [37-59]
    (=jis-x0213-1	. #x3267)	; &I-JX1-3267; [18-71]
    (=big5		. #xA4FA)	; &I-B-A4FA;
    (=hanyo-denshi/ja	. #x3267)	; &I-HD-JA-3267; [18-71]
    (=hanyo-denshi/hg	. #x3026)	; &I-HD-HG-3026; [16-06]
    (=gt		. 25560)	; &I-GT-25560;
    (=gt-k		. 00900)	; &I-GT-K00900;
    (=gt-pj-1		. #x3267)	; &I-GT-25560; [18-71]
    (=daikanwa		. 19909)	; &I-M-19909;
    (=daijiten		. 07066)	; &I-DJT-07066;
    (->subsumptive
     ((total-strokes	   . 4)
      (=>>ucs@unicode	  . #x7259)	; &G-MJ016801;
      (=>>mj		  . 016801)	; &G-MJ016801;
      (=>>cns11643-1	  . #x455B)	; &G-MJ016801; [37-59]
      (=>>jis-x0213-1@2004 . #x3267)	; &G-MJ016801; [18-71]
      (=>>hanyo-denshi/hg . #x3026)	; &G-MJ016801; [16-06]
      (=>>daikanwa	  . 19909)	; &G-MJ016801;
      (->subsumptive
       ((==daikanwa	    . 19909)	; &g2-M-19909;
	)
       ((==ucs@cns	    . #x7259)	; &g2-CU+7259;
	(==cns11643-1	    . #x455B)	; &g2-CU+7259; [37-59]
	)
       ((=decomposition@hanyo-denshi
	 ((=ucs		      . #x7259)	; 牙
	  )
	 ((=ucs		      . #xE0103)	; 󠄃
	  ))
	(==ucs@unicode	    . #x7259)	; &g2-MJ016801;
	(==mj		    . 016801)	; &g2-MJ016801;
	(==gb2312	    . #x5140)	; &g2-MJ016801; [49-32]
	(==jis-x0213-1@2004 . #x3267)	; &g2-MJ016801; [18-71]
	(==hanyo-denshi/hg  . #x3026)	; &g2-MJ016801; [16-06]
	(->subsumptive
	 ((===jis-x0213-1@2004 . #x3267)	; &R-HD-HG-3026; [18-71]
	  (===hanyo-denshi/hg . #x3026)	; &R-HD-HG-3026; [16-06]
	  )
	 ((===mj	      . 016801)	; &R-MJ016801;
	  )
	 ((===ucs@unicode     . #x7259)	; &R-UU+7259;
	  ))
	))
      )
     ((total-strokes	   . 5)
      (=>>ucs@jis/1990	  . #x7259)	; &MJ016802;
      (=>>ucs@jis/2000	  . #x7259)	; &MJ016802;
      (=>>ucs@ks	  . #x7259)	; &MJ016802;
      (=mj		  . 016802)	; &MJ016802;
      (=>>adobe-japan1-0  . 01383)	; &MJ016802;
      (=>>jis-x0208	  . #x3267)	; &MJ016802; [18-71]
      (=>>jis-x0213-1@2000 . #x3267)	; &MJ016802; [18-71]
      (=>>hanyo-denshi/ja . #x3267)	; &MJ016802; [18-71]
      (=>>gt		  . 25560)	; &MJ016802;
      (=>>gt-k		  . 00900)	; &MJ016802;
      (=jef-china3	  . #x58C5)	; &I-JC3-58C5;
      (->subsumptive
       ((=decomposition@hanyo-denshi
	 ((=ucs		      . #x7259)	; 牙
	  )
	 ((=ucs		      . #xE0102)	; 󠄂
	  ))
	(==ucs@jis/1990	    . #x7259)	; &g2-MJ016802;
	(==ucs@jis/2000	    . #x7259)	; &g2-MJ016802;
	(==mj		    . 016802)	; &g2-MJ016802;
	(==adobe-japan1-0   . 01383)	; &g2-MJ016802;
	(==jis-x0208	    . #x3267)	; &g2-MJ016802; [18-71]
	(==jis-x0213-1@2000 . #x3267)	; &g2-MJ016802; [18-71]
	(==hanyo-denshi/ja  . #x3267)	; &g2-MJ016802; [18-71]
	(==gt		    . 25560)	; &g2-MJ016802;
	(==gt-k		    . 00900)	; &g2-MJ016802;
	(->subsumptive
	 ((===jis-x0208	      . #x3267)	; &R-J0-3267; [18-71]
	  (===jis-x0213-1@2000 . #x3267)	; &R-J0-3267; [18-71]
	  )
	 ((===adobe-japan1-0  . 01383)	; &R-AJ1-01383;
	  )
	 ((===mj	      . 016802)	; &R-MJ016802;
	  )
	 ((===gt	      . 25560)	; &R-GT-25560;
	  (===gt-k	      . 00900)	; &R-GT-25560;
	  ))
	)
       ((==ucs@ks	    . #x7259)	; &g2-DJT-07066;
	(==ks-x1001	    . #x6433)	; &g2-DJT-07066; [68-19]
	(==daijiten	    . 07066)	; &g2-DJT-07066;
	(==jef-china3	    . #x58C5)	; &g2-DJT-07066;
	(->subsumptive
	 ((===daijiten	      . 07066)	; &R-DJT-07066;
	  )
	 ((===ucs@ks	      . #x7259)	; &R-K0-6433;
	  (===ks-x1001	      . #x6433)	; &R-K0-6433; [68-19]
	  )
	 ((===jef-china3      . #x58C5)	; &R-JC3-58C5;
	  ))
	))
      ))
    ))
(define-char
  '((ideographic-radical . 92)	; ⽛
    (ideographic-strokes . 0)
    (total-strokes	 . 4)
    (hanyu-dazidian	1 13 1)
    (=ucs		. #x3E26)	; 㸦
    (=cns11643-3	. #x216A)	; &I-C3-216A; [01-74]
    (=big5-cdp		. #x88DC)	; &I-CDP-88DC;
    (=daikanwa		. 19910)	; &I-M-19910;
    (<-vulgar
     ((=ucs@unicode	  . #x4E92)	; &AJ1-01939;
      (=adobe-japan1-0	  . 01939)	; &AJ1-01939;
      (=daikanwa	  . 00255)	; &I-M-00255;
      ))
    (<-vulgar*sources
     guangyun daikanwa)
    (->mistakable
     ((=ucs		  . #x7259)	; 牙
      (=daikanwa	  . 19909)	; &I-M-19909;
      ))
    (->mistakable*sources
     kangxi-biansi-2char daikanwa)
    (->subsumptive
     ((==daikanwa	  . 19910)	; &g2-M-19910;
      )
     ((==ucs@cns	  . #x3E26)	; &g2-CU+3E26;
      (==cns11643-3	  . #x216A)	; &g2-CU+3E26; [01-74]
      )
     ((==ucs@unicode	  . #x3E26)	; &g2-CDP-88DC;
      (==big5-cdp	  . #x88DC)	; &g2-CDP-88DC;
      (->subsumptive
       ((===big5-cdp	    . #x88DC)	; &R-CDP-88DC;
	)
       ((===ucs@unicode	    . #x3E26)	; &R-UU+3E26;
	))
      )
     ((===hng-kyu	  . 00020)	; &HNG047-00020;
      ))
    ))
(define-char
  '((ideographic-radical@ucs . 92)	; ⽛
    (ideographic-strokes@ucs . 3)
    (ideographic-radical@ucs*sources
     daikanwa ucs)
    (ideographic-radical@cns . 39)	; ⼦
    (ideographic-strokes@cns . 4)
    (total-strokes	 . 7)
    (=ucs		. #x24605)	; 𤘅
    (=cns11643-5	. #x2368)	; &I-C5-2368; [03-72]
    (=daikanwa		. 19911)	; &I-M-19911;
    ))
(define-char
  '((ideographic-radical . 92)	; ⽛
    (ideographic-strokes . 4)
    (total-strokes	 . 8)
    (=ucs		. #x24606)	; 𤘆
    (=daikanwa		. 19912)	; &I-M-19912;
    ))
(define-char
  '((ideographic-radical . 92)	; ⽛
    (ideographic-strokes . 5)
    (total-strokes	 . 9)
    (=ucs		. #x24607)	; 𤘇
    (->subsumptive
     ((=daikanwa	  . 19913)	; &M-19913;
      )
     ((=ucs@iso		  . #x24607)	; &HZK04-CEC3;
      (=hanziku-4	  . #xCEC3)	; &HZK04-CEC3;
      ))
    ))
(define-char
  '((ideographic-radical . 92)	; ⽛
    (ideographic-strokes . 6)
    (total-strokes	 . 10)
    (=ucs		. #x3E27)	; 㸧
    (->subsumptive
     ((=daikanwa	  . 19914)	; &M-19914;
      )
     ((hanyu-dazidian	  2 1420 7)
      (=ucs@unicode	  . #x3E27)	; &HZK04-CEC7;
      (=hanziku-4	  . #xCEC7)	; &HZK04-CEC7;
      )
     ((=cns11643-3	  . #x3233)	; &C3-3233; [18-19]
      ))
    ))
(define-char
  '((<-denotational@usage
     ((=ucs		  . #x24608)	; 𤘈
      ))
    (ideographic-radical . 92)	; ⽛
    (ideographic-strokes . 6)
    (total-strokes	 . 10)
    (ideographic-structure
     ((name . "IDEOGRAPHIC DESCRIPTION CHARACTER ABOVE TO BELOW")
      (=ucs		  . #x2FF1)	; ⿱
      )
     ((=ucs		  . #x7259)	; 牙
      )
     ((=ucs@unicode	  . #x81FC)	; &MJ021316;
      (=mj		  . 021316)	; &MJ021316;
      ))
    (=ucs@iso		. #x24608)	; &C6-3943;
    (=cns11643-6	. #x3943)	; &C6-3943; [25-35]
    (=daikanwa 		. 19915)	; &I-M-19915;
    (=daijiten		. 07068)	; &I-DJT-07068;
    (->subsumptive
     ((==ucs@iso	  . #x24608)	; &g2-M-19915;
      (==daikanwa	  . 19915)	; &g2-M-19915;
      (->subsumptive
       ((===daikanwa	    . 19915)	; &R-M-19915;
	)
       ((===ucs@iso	    . #x24608)	; &R-U-00024608;
	))
      )
     ((==daijiten	  . 07068)	; &g2-DJT-07068;
      )
     ((==ucs@cns	  . #x24608)	; &g2-CU+24608;
      (==cns11643-6	  . #x3943)	; &g2-CU+24608; [25-35]
      ))
    ))
(define-char
  '((ideographic-radical . 92)	; ⽛
    (ideographic-strokes . 6)
    (total-strokes	 . 10)
    (=ucs		. #x24609)	; 𤘉
    (->subsumptive
     ((=daikanwa	  . 19916)	; &M-19916;
      )
     ((=ucs@iso		  . #x24609)	; &U-00024609;
      )
     ((=cns11643-6	  . #x3942)	; &C6-3942; [25-34]
      ))
    ))
(define-char
  '((ideographic-radical . 92)	; ⽛
    (ideographic-strokes . 7)
    (total-strokes	 . 11)
    (=ucs		. #x2460A)	; 𤘊
    (->subsumptive
     ((=daikanwa	  . 19917)	; &M-19917;
      )
     ((=ucs@iso		  . #x2460A)	; &HZK04-CEC6;
      (=hanziku-4	  . #xCEC6)	; &HZK04-CEC6;
      ))
    ))
(define-char
  '((<-denotational@usage
     ((==>ucs@bucs	  . #x7259)	; &BUCS+7259;
      ))
    (ideographic-radical . 92)	; ⽛
    (ideographic-structure
     ((name . "IDEOGRAPHIC DESCRIPTION CHARACTER ABOVE TO BELOW")
      (=ucs		  . #x2FF1)	; ⿱
      )
     ((=ucs		  . #x7259)	; 牙
      )
     ((=>ucs@component	  . #x81FC)	; &A-compU+81FC;
      (=>iwds-1		  .  0168)	; &A-compU+81FC;
      ))
    (=ucs		. #x24608)	; 𤘈
    ))
(define-char
  '((<-denotational
     ((=ucs		  . #x24608)	; 𤘈
      ))
    (=>ucs*		. #x24608)	; 𤘈
    (ideographic-radical . 92)	; ⽛
    (ideographic-strokes . 7)
    (total-strokes	 . 11)
    (ideographic-structure
     ((name . "IDEOGRAPHIC DESCRIPTION CHARACTER ABOVE TO BELOW")
      (=ucs		  . #x2FF1)	; ⿱
      )
     ((=ucs		  . #x7259)	; 牙
      )
     ((=ucs		  . #x26951)	; 𦥑
      ))
    (=ucs		. #x2F923)	; 𤘈
    (=cns11643-6	. #x4163)	; &I-C6-4163; [33-67]
    (->subsumptive
     ((==ucs@cns	  . #x2F923)	; &g2-CU+2F923;
      (==cns11643-6	  . #x4163)	; &g2-CU+2F923; [33-67]
      ))
    ))
(define-char
  '((ideographic-radical . 92)	; ⽛
    (ideographic-strokes . 8)
    (total-strokes	 . 12)
    (=ucs		. #x2460C)	; 𤘌
    (->subsumptive
     ((=daikanwa	  . 19918)	; &M-19918;
      )
     ((=cns11643-4	  . #x386F)	; &C4-386F; [24-79]
      )
     ((=ucs@iso		  . #x2460C)	; &U-0002460C;
      ))
    ))
(define-char
  '((ideographic-radical . 92)	; ⽛
    (ideographic-strokes . 8)
    (ideographic-structure
     ((name . "IDEOGRAPHIC DESCRIPTION CHARACTER ABOVE TO BELOW")
      (=ucs		  . #x2FF1)	; ⿱
      )
     ((=>ucs@iwds-1	  . #x5C19)	; &J97-3E30;
      (=>jis-x0208	  . #x3E30)	; &J97-3E30; [30-16]
      )
     ((=ucs		  . #x7259)	; 牙
      ))
    (=ucs		. #x725A)	; 牚
    ))
(define-char
  '((<-denotational
     ((=ucs		  . #x725A)	; 牚
      ))
    (ideographic-radical . 92)	; ⽛
    (ideographic-strokes . 8)
    (ideographic-structure
     ((name . "IDEOGRAPHIC DESCRIPTION CHARACTER ABOVE TO BELOW")
      (=ucs		  . #x2FF1)	; ⿱
      )
     ((=gt-k		  . 05014)	; &GT-K05014;
      )
     ((=ucs		  . #x7259)	; 牙
      ))
    (=ucs@unicode	. #x725A)	; &AJ1-19526;
    (=adobe-japan1-5	. 19526)	; &AJ1-19526;
    (=jis-x0212		. #x4A57)	; &I-JSP-4A57; [42-55]
    (=iso-ir165		. #x2C37)	; &I-EGB-2C37; [12-23]
    (=cns11643-2	. #x3D6B)	; &I-C2-3D6B; [29-75]
    (=big5		. #xDA67)	; &I-B-DA67;
    (=daikanwa		. 19919)	; &I-M-19919;
    (->subsumptive
     ((total-strokes	   . 12)
      (==ucs@cns	  . #x725A)	; &g2-M-19919;
      (==cns11643-2	  . #x3D6B)	; &g2-M-19919; [29-75]
      (==daikanwa	  . 19919)	; &g2-M-19919;
      (->subsumptive
       ((===daikanwa	    . 19919)	; &R-M-19919;
	)
       ((===cns11643-2	    . #x3D6B)	; &R-C2-3D6B; [29-75]
	))
      )
     ((total-strokes	   . 12)
      (==ucs@unicode	  . #x725A)	; &g2-UU+725A;
      (==ucs@JP/hanazono  . nil)
      (==iso-ir165	  . #x2C37)	; &g2-UU+725A; [12-23]
      )
     ((total-strokes	   . 13)
      (==ucs@jis	  . #x725A)	; &g2-AJ1-19526;
      (==adobe-japan1-5	  . 19526)	; &g2-AJ1-19526;
      (==jis-x0212	  . #x4A57)	; &g2-AJ1-19526; [42-55]
      (->subsumptive
       ((===jis-x0212	    . #x4A57)	; &R-JSP-4A57; [42-55]
	)
       ((===adobe-japan1-5  . 19526)	; &R-AJ1-19526;
	))
      ))
    ))
(define-char
  '((ideographic-radical . 92)	; ⽛
    (ideographic-strokes . 8)
    (total-strokes	 . 12)
    (=ucs		. #x2460D)	; 𤘍
    (->subsumptive
     ((=ucs@iso		  . #x2460D)	; &U-0002460D;
      )
     ((=cns11643-6	  . #x4B2F)	; &C6-4B2F; [43-15]
      ))
    ))
(define-char
  '((ideographic-radical . 92)	; ⽛
    (ideographic-strokes . 9)
    (total-strokes	 . 13)
    (=ucs		. #x24610)	; 𤘐
    (->subsumptive
     ((=daikanwa	  . 19920)	; &M-19920;
      )
     ((=ucs@iso		  . #x24610)	; &U-00024610;
      )
     ((=cns11643-6	  . #x5471)	; &C6-5471; [52-81]
      ))
    ))
(define-char
  '((ideographic-radical . 92)	; ⽛
    (ideographic-strokes . 10)
    (total-strokes	 . 14)
    (=ucs		. #x24611)	; 𤘑
    (->subsumptive
     ((=daikanwa	  . 19921)	; &M-19921;
      )
     ((=ucs@iso		  . #x24611)	; &U-00024611;
      )
     ((=cns11643-6	  . #x5E5D)	; &C6-5E5D; [62-61]
      ))
    ))
(define-char
  '((ideographic-radical . 92)	; ⽛
    (ideographic-strokes . 14)
    (ideographic-structure
     ((name . "IDEOGRAPHIC DESCRIPTION CHARACTER LEFT TO RIGHT")
      (=ucs		  . #x2FF0)	; ⿰
      )
     ((=ucs		  . #x7259)	; 牙
      )
     ((=ucs		  . #x5BE7)	; 寧
      ))
    (=ucs		. #x24613)	; 𤘓
    ))
(define-char
  '((<-denotational
     ((=ucs		  . #x24613)	; 𤘓
      ))
    (ideographic-radical . 92)	; ⽛
    (ideographic-strokes . 14)
    (total-strokes	 . 18)
    (ideographic-structure
     ((name . "IDEOGRAPHIC DESCRIPTION CHARACTER LEFT TO RIGHT")
      (=ucs		  . #x2FF0)	; ⿰
      )
     ((=ucs		  . #x7259)	; 牙
      )
     ((=ucs@unicode	  . #x5BE7)	; &MJ010230;
      (=mj		  . 010230)	; &MJ010230;
      ))
    (=ucs@iso		. #x24613)	; &U-00024613;
    (->subsumptive
     ((==ucs@iso	  . #x24613)	; &g2-IU-00024613;
      ))
    ))
(define-char
  '((ideographic-radical . 92)	; ⽛
    (==>ucs@bucs	. #x7259)	; &BUCS+7259;
    (==>daijiten	. 07065)	; &BUCS+7259;
    ))
