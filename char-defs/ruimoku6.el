;; -*- coding: utf-8-mcs-er -*-
(define-char
  '((script		Oracle-Bones Ideographs)
    (sources		ruimoku=A016070-00002)
    (=ruimoku-v6	. #xE000)	; &RUI6-E000;
    (<-unknown
     ((=zinbun-oracle	  . 02523)	; &ZOB-2523;
      )
     ((=zinbun-oracle	  . 00589)	; &ZOB-0589;
      ))
    ))
(define-char
  '((script		Oracle-Bones Ideographs)
    (sources		ruimoku=A016070-00002)
    (=ruimoku-v6	. #xE001)	; &RUI6-E001;
    ))
(define-char
  '((sources		ruimoku=A015208-00001)
    (=ruimoku-v6	. #xE002)	; &RUI6-E002;
    ))
(define-char
  '((sources		ruimoku=A015208-00001)
    (=ruimoku-v6	. #xE003)	; &RUI6-E003;
    ))
(define-char
  '((sources		ruimoku=A015208-00001)
    (=ruimoku-v6	. #xE004)	; &RUI6-E004;
    ))
(define-char
  '((<-denotational
     ((=ucs		  . #x304A6)	; 𰒦
      ))
    (ideographic-radical . 62)	; ⼽
    (ideographic-strokes . 5)
    (total-strokes	 . 9)
    (ideographic-structure
     ((name . "IDEOGRAPHIC DESCRIPTION CHARACTER SURROUND FROM UPPER RIGHT")
      (=ucs		  . #x2FF9)	; ⿹
      )
     ((=ucs		  . #x6208)	; 戈
      )
     ((=ucs@unicode	  . #x51AC)	; &MJ007359;
      (=mj		  . 007359)	; &MJ007359;
      ))
    (sources		ruimoku=A016481-00008)
    (=ruimoku-v6	. #xE005)	; &RUI6-E005;
    (->subsumptive
     ((==ruimoku-v6	  . #xE005)	; &g2-RUI6-E005;
      ))
    ))
(define-char
  '((ideographic-radical . 37)	; ⼤
    (sources		ruimoku=A016462-00003 ruimoku=A016430-00004
			ruimoku=A016594-00007)
    (=ruimoku-v6	. #xE006)	; &RUI6-E006;
    (<-unknown
     ((=ucs		  . #x215DB)	; 𡗛
      ))
    ))
(define-char
  '((ideographic-radical . 26)	; ⼙
    (ideographic-strokes . 4)
    (total-strokes	 . 6)
    (ideographic-structure
     ((name . "IDEOGRAPHIC DESCRIPTION CHARACTER LEFT TO RIGHT")
      (=ucs		  . #x2FF0)	; ⿰
      )
     ((=ucs		  . #x6208)	; 戈
      )
     ((=ucs		  . #x5369)	; 卩
      ))
    (=ruimoku-v6	. #xE007)	; &RUI6-E007;
    (<-error@ruimoku/2001
     ((=ucs		  . #x31458)	; 𱑘
      (=ruimoku-v6	  . #xE00F)	; &I-RUI6-E00F;
      ))
    (<-error@ruimoku/2001*sources
     ruimoku-v6)
    ))
(define-char
  '((<-denotational
     ((=>ruimoku-v6	  . #xE008)	; &A-RUI6-E008;
      ))
    (ideographic-radical . 46)	; ⼭
    (ideographic-strokes . 14)
    (total-strokes	 . 17)
    (ideographic-structure
     ((name . "IDEOGRAPHIC DESCRIPTION CHARACTER LEFT TO RIGHT")
      (=ucs		  . #x2FF0)	; ⿰
      )
     ((=ucs		  . #x5C71)	; 山
      )
     ((=ucs@unicode	  . #x96A0)	; &MJ027675;
      (=mj		  . 027675)	; &MJ027675;
      ))
    (sources		ruimoku=A015330-00006)
    (=ruimoku-v6	. #xE008)	; &RUI6-E008;
    (->subsumptive
     ((==ruimoku-v6	  . #xE008)	; &g2-RUI6-E008;
      ))
    ))
(define-char
  '((<-denotational
     ((=ucs		  . #x21A5C)	; 𡩜
      ))
    (ideographic-radical . 40)	; ⼧
    (ideographic-strokes . 9)
    (total-strokes	 . 12)
    (ideographic-structure
     ((name . "IDEOGRAPHIC DESCRIPTION CHARACTER ABOVE TO MIDDLE AND BELOW")
      (=ucs		  . #x2FF3)	; ⿳
      )
     ((=ucs		  . #x5B80)	; 宀
      )
     ((=ucs		  . #x9FB6)	; 龶
      )
     ((name		  . "CJK RADICAL EYE")
      (=ucs		  . #x2EAB)	; ⺫
      ))
    (sources		ruimoku=A015412-00003)
    (=ucs@iso		. #x21A5C)	; &RUI6-E009;
    (=ruimoku-v6	. #xE009)	; &RUI6-E009;
    (->subsumptive
     ((==ucs@iso	  . #x21A5C)	; &g2-IU-00021A5C;
      (==ruimoku-v6	  . #xE009)	; &g2-IU-00021A5C;
      (->subsumptive
       ((===ruimoku-v6	    . #xE009)	; &R-RUI6-E009;
	)
       ((===ucs@iso	    . #x21A5C)	; &R-U-00021A5C;
	)
       ((=hanziku-12	    . #xECD1)	; &HZK12-ECD1;
	))
      ))
    ))
(define-char
  '((<-denotational
     ((=ucs		  . #x29C2C)	; 𩰬
      ))
    (ideographic-radical . 193)	; ⿀
    (ideographic-strokes . 3)
    (total-strokes	 . 13)
    (ideographic-structure
     ((name . "IDEOGRAPHIC DESCRIPTION CHARACTER SURROUND FROM LEFT")
      (=ucs		  . #x2FF7)	; ⿷
      )
     ((=ucs@unicode	  . #x9B32)	; &C1-586F;
      (=gb2312		  . #x582A)	; &C1-586F; [56-10]
      (=cns11643-1	  . #x586F)	; &I-C1-586F; [56-79]
      )
     ((=ucs		  . #x3404)	; 㐄
      ))
    (sources		ruimoku=A016374-00004)
    (=ucs@iso		. #x29C2C)	; &RUI6-E00A;
    (=cns11643-6	. #x5A4E)	; &RUI6-E00A; [58-46]
    (=ruimoku-v6	. #xE00A)	; &I-RUI6-E00A;
    (->subsumptive
     ((==ucs@iso	  . #x29C2C)	; &g2-IU-00029C2C;
      (==ruimoku-v6	  . #xE00A)	; &g2-IU-00029C2C;
      (->subsumptive
       ((===ucs@iso	    . #x29C2C)	; &R-U-00029C2C;
	)
       ((===ruimoku-v6	    . #xE00A)	; &R-RUI6-E00A;
	))
      )
     ((==ucs@cns	  . #x29C2C)	; &g2-CU+29C2C;
      (==cns11643-6	  . #x5A4E)	; &g2-CU+29C2C; [58-46]
      ))
    ))
(define-char
  '((ideographic-radical . 69)	; ⽄
    (ideographic-strokes . 5)
    (total-strokes	 . 9)
    (ideographic-structure
     ((name . "IDEOGRAPHIC DESCRIPTION CHARACTER LEFT TO RIGHT")
      (=ucs		  . #x2FF0)	; ⿰
      )
     ((=ucs		  . #x211A5)	; 𡆥
      )
     ((=ucs		  . #x65A4)	; 斤
      ))
    (sources		ruimoku=A2001-16138-00013)
    (=ruimoku-v6	. #xE00B)	; &RUI6-E00B;
    ))
(define-char
  '((<-denotational
     ((=ucs		  . #x2DAA4)	; 𭪤
      ))
    (ideographic-radical . 75)	; ⽊
    (ideographic-strokes . 8)
    (total-strokes	 . 12)
    (ideographic-structure
     ((name . "IDEOGRAPHIC DESCRIPTION CHARACTER LEFT TO RIGHT")
      (=ucs		  . #x2FF0)	; ⿰
      )
     ((=ucs		  . #x6728)	; 木
      )
     ((=ucs@unicode	  . #x5CB8)	; &AJ1-01563;
      (=adobe-japan1-0	  . 01563)	; &AJ1-01563;
      ))
    (sources		ruimoku=A016758-00009)
    (=ucs@JP/hanazono	. #x2DAA4)	; &RUI6-E00C;
    (=ruimoku-v6	. #xE00C)	; &RUI6-E00C;
    (->subsumptive
     ((==ucs@JP/hanazono  . #x2DAA4)	; &g2-hanaJU+2DAA4;
      (==ruimoku-v6	  . #xE00C)	; &g2-hanaJU+2DAA4;
      (->subsumptive
       ((===ruimoku-v6	    . #xE00C)	; &R-RUI6-E00C;
	))
      ))
    ))
(define-char
  '((name*		. "latin small letter a with macron and acute")
    (general-category	letter lowercase) ; Normative Category
    (=decomposition
     ((name		  . "LATIN SMALL LETTER A WITH MACRON")
      (=ucs		  . #x0101)	; ā
      )
     ((name		  . "COMBINING ACUTE ACCENT")
      (=ucs		  . #x0301)	; ́
      ))
    (sources		ruimoku=C011091-00007)
    (=ruimoku-v6	. #xE00D)	; &RUI6-E00D;
    ))
(define-char
  '((<-denotational
     ((=ucs		  . #x22189)	; 𢆉
      ))
    (ideographic-radical . 51)	; ⼲
    (ideographic-strokes . 2)
    (total-strokes	 . 5)
    (ideographic-structure
     ((name . "IDEOGRAPHIC DESCRIPTION CHARACTER ABOVE TO BELOW")
      (=ucs		  . #x2FF1)	; ⿱
      )
     ((=ucs		  . #x4E37)	; 丷
      )
     ((=ucs		  . #x5E72)	; 干
      ))
    (sources		ruimoku=A016219-00020)
    (=ucs@iso		. #x22189)	; &MJ035213;
    (=mj		. 035213)	; &MJ035213;
    (=cns11643-4	. #x2178)	; &I-C4-2178; [01-88]
    (=big5-cdp		. #x8B75)	; &I-CDP-8B75;
    (=koseki		. 103680)	; &MJ035213;
    (=daikanwa		. 09166)	; &I-M-09166;
    (=daijiten		. 02898)	; &I-DJT-02898;
    (=cbeta		. 08862)	; &I-CB08862;
    (=ruimoku-v6	. #xE00E)	; &I-RUI6-E00E;
    (<-simplified@component
     ((=ucs		  . #x5C70)	; 屰
      ))
    (<-simplified@component*note "different from M-09166 (C4-2178, U+22189).")
    (->subsumptive
     ((==ucs@iso	  . #x22189)	; &g2-MJ035213;
      (==mj		  . 035213)	; &g2-MJ035213;
      (==cns11643-4	  . #x2178)	; &g2-MJ035213; [01-88]
      (==big5-cdp	  . #x8B75)	; &g2-MJ035213;
      (==koseki		  . 103680)	; &g2-MJ035213;
      (==daikanwa	  . 09166)	; &g2-MJ035213;
      (==daijiten	  . 02898)	; &g2-MJ035213;
      (==cbeta		  . 08862)	; &g2-MJ035213;
      (==ruimoku-v6	  . #xE00E)	; &g2-MJ035213;
      (->subsumptive
       ((===daikanwa	    . 09166)	; &R-M-09166;
	)
       ((===daijiten	    . 02898)	; &R-DJT-02898;
	)
       ((===mj		    . 035213)	; &R-MJ035213;
	)
       ((===big5-cdp	    . #x8B75)	; &R-CDP-8B75;
	)
       ((===cns11643-4	    . #x2178)	; &R-C4-2178; [01-88]
	)
       ((===ucs@iso	    . #x22189)	; &R-U-00022189;
	)
       ((===ruimoku-v6	    . #xE00E)	; &R-RUI6-E00E;
	)
       ((===cbeta	    . 08862)	; &R-CB08862;
	))
      ))
    ))
(define-char
  '((ideographic-radical . 26)	; ⼙
    (ideographic-strokes . 3)
    (total-strokes	 . 5)
    (ideographic-structure
     ((name . "IDEOGRAPHIC DESCRIPTION CHARACTER LEFT TO RIGHT")
      (=ucs		  . #x2FF0)	; ⿰
      )
     ((=ucs		  . #x5F0B)	; 弋
      )
     ((=ucs		  . #x5369)	; 卩
      ))
    (sources		ruimoku=A016728-00002)
    (=ucs		. #x31458)	; 𱑘
    (=ruimoku-v6	. #xE00F)	; &I-RUI6-E00F;
    (->subsumptive
     ((==ucs@JP/hanazono  . #x31458)	; &g2-hanaJU+31458;
      (==ruimoku-v6	  . #xE00F)	; &g2-hanaJU+31458;
      (->subsumptive
       ((===ruimoku-v6	    . #xE00F)	; &R-RUI6-E00F;
	))
      ))
    ))
(define-char
  '((ideographic-structure
     ((name . "IDEOGRAPHIC DESCRIPTION CHARACTER SURROUND FROM LOWER LEFT")
      (=ucs		  . #x2FFA)	; ⿺
      )
     ((=ucs		  . #x8FB6)	; 辶
      )
     ((=ruimoku-v6	  . #xE011)	; &RUI6-E011;
      ))
    (sources		ruimoku=A021239-00010)
    (=ruimoku-v6	. #xE010)	; &RUI6-E010;
    ))
(define-char
  '((sources		ruimoku=A022115-00007)
    (=ruimoku-v6	. #xE011)	; &RUI6-E011;
    ))
(define-char
  '((sources		ruimoku=A2003-10051-00010)
    (=ruimoku-v6	. #xE012)	; &RUI6-E012;
    ))
(define-char
  '((<-denotational
     ((=ucs		  . #x30CAD)	; 𰲭
      ))
    (ideographic-radical . 142)	; ⾍
    (ideographic-strokes . 4)
    (total-strokes	 . 10)
    (ideographic-structure
     ((name . "IDEOGRAPHIC DESCRIPTION CHARACTER ABOVE TO BELOW")
      (=ucs		  . #x2FF1)	; ⿱
      )
     ((=ucs		  . #x65E5)	; 日
      )
     ((=ucs		  . #x866B)	; 虫
      ))
    (sources		ruimoku=A021610-00006)
    (=ucs@JP/hanazono	. #x30CAD)	; &RUI6-E013;
    (=ruimoku-v6	. #xE013)	; &RUI6-E013;
    (->subsumptive
     ((==ruimoku-v6	  . #xE013)	; &g2-RUI6-E013;
      ))
    ))
(define-char
  '((<-denotational
     ((=>ruimoku-v6	  . #xE014)	; &A-RUI6-E014;
      ))
    (ideographic-radical . 181)	; ⾴
    (ideographic-strokes . 10)
    (total-strokes	 . 19)
    (ideographic-structure
     ((name . "IDEOGRAPHIC DESCRIPTION CHARACTER LEFT TO RIGHT")
      (=ucs		  . #x2FF0)	; ⿰
      )
     ((=ucs@JP/hanazono	  . #x30CAD)	; &RUI6-E013;
      (=ruimoku-v6	  . #xE013)	; &RUI6-E013;
      )
     ((=ucs		  . #x9801)	; 頁
      ))
    (sources		ruimoku=A021610-00006)
    (=ruimoku-v6	. #xE014)	; &RUI6-E014;
    (->subsumptive
     ((==ruimoku-v6	  . #xE014)	; &g2-RUI6-E014;
      ))
    ))
(define-char
  '((<-denotational
     ((=>ruimoku-v6	  . #xE015)	; &A-RUI6-E015;
      ))
    (ideographic-radical . 61)	; ⼼
    (ideographic-strokes . 5)
    (total-strokes	 . 9)
    (ideographic-structure
     ((name . "IDEOGRAPHIC DESCRIPTION CHARACTER ABOVE TO BELOW")
      (=ucs		  . #x2FF1)	; ⿱
      )
     ((=ucs@JP/hanazono	  . #x2B889)	; &CDP-8D4A;
      (=big5-cdp	  . #x8D4A)	; &CDP-8D4A;
      )
     ((=ucs		  . #x5FC3)	; 心
      ))
    (sources		ruimoku=A2002-21879-00012)
    (=ruimoku-v6	. #xE015)	; &RUI6-E015;
    (->subsumptive
     ((==ruimoku-v6	  . #xE015)	; &g2-RUI6-E015;
      ))
    ))
(define-char
  '((<-denotational
     ((=ucs		  . #x24F08)	; 𤼈
      ))
    (ideographic-radical . 104)	; ⽧
    (ideographic-strokes . 16)
    (total-strokes	 . 21)
    (ideographic-structure
     ((name . "IDEOGRAPHIC DESCRIPTION CHARACTER SURROUND FROM UPPER LEFT")
      (=ucs		  . #x2FF8)	; ⿸
      )
     ((=ucs		  . #x7592)	; 疒
      )
     ((=ucs@unicode	  . #x8208)	; &AJ1-01717;
      (=adobe-japan1-0	  . 01717)	; &AJ1-01717;
      ))
    (sources		ruimoku=A2003-10051-00005)
    (=ucs@iso		. #x24F08)	; &RUI6-E016;
    (=ruimoku-v6	. #xE016)	; &RUI6-E016;
    (->subsumptive
     ((==ucs@iso	  . #x24F08)	; &g2-IU-00024F08;
      (==ruimoku-v6	  . #xE016)	; &g2-IU-00024F08;
      (->subsumptive
       ((===ruimoku-v6	    . #xE016)	; &R-RUI6-E016;
	)
       ((===ucs@iso	    . #x24F08)	; &R-U-00024F08;
	))
      ))
    ))
(define-char
  '((<-denotational
     ((=ucs		  . #x30BFC)	; 𰯼
      ))
    (ideographic-radical . 134)	; ⾅
    (ideographic-strokes . 16)
    (total-strokes	 . 23)
    (ideographic-structure
     ((name . "IDEOGRAPHIC DESCRIPTION CHARACTER ABOVE TO BELOW")
      (=ucs		  . #x2FF1)	; ⿱
      )
     ((=ucs-itaiji-009	  . #x8208)	; &U-i009+8208;
      )
     ((=ucs		  . #x6614)	; 昔
      ))
    (sources		ruimoku=A021003-00010 ruimoku=A2003-10164-00008)
    (=ucs@JP/hanazono	. #x30BFC)	; &RUI6-E017;
    (=ruimoku-v6	. #xE017)	; &RUI6-E017;
    (->subsumptive
     ((==ucs@JP/hanazono  . #x30BFC)	; &g2-hanaJU+30BFC;
      )
     ((==ruimoku-v6	  . #xE017)	; &g2-RUI6-E017;
      ))
    ))
(define-char
  '((<-denotational
     ((=ucs		  . #x2C2DD)	; 𬋝
      ))
    (ideographic-radical . 86)	; ⽕
    (ideographic-strokes . 16)
    (total-strokes	 . 20)
    (ideographic-structure
     ((name . "IDEOGRAPHIC DESCRIPTION CHARACTER ABOVE TO BELOW")
      (=ucs		  . #x2FF1)	; ⿱
      )
     ((=ucs-itaiji-001	  . #x38C8)	; &U-i001+38C8;
      )
     ((=ucs		  . #x706B)	; 火
      ))
    (sources		ruimoku=A2003-10320-00001 ruimoku=A2003-10320-00002
			ruimoku=A2003-10320-00004 ruimoku=A2003-10620-00005
			ruimoku=A2003-10693-00007 ruimoku=A2003-11142-00007)
    (=ucs@JP/hanazono	. #x2C2DD)	; &RUI6-E018;
    (=ruimoku-v6	. #xE018)	; &RUI6-E018;
    (<-unknown
     ((=ucs		  . #x8C73)	; 豳
      )
     ((=ucs		  . #x71F9)	; 燹
      ))
    (->subsumptive
     ((==ucs@JP/hanazono  . #x2C2DD)	; &g2-hanaJU+2C2DD;
      )
     ((==ruimoku-v6	  . #xE018)	; &g2-RUI6-E018;
      ))
    ))
(define-char
  '((<-denotational
     ((=>ruimoku-v6	  . #xE019)	; &A-RUI6-E019;
      ))
    (ideographic-radical . 129)	; ⾀
    (ideographic-strokes . 8)
    (total-strokes	 . 14)
    (ideographic-structure
     ((name . "IDEOGRAPHIC DESCRIPTION CHARACTER LEFT TO RIGHT")
      (=ucs		  . #x2FF0)	; ⿰
      )
     ((=ucs-itaiji-005	  . #x38C7)	; &U-i005+38C7;
      )
     ((=ucs		  . #x807F)	; 聿
      ))
    (sources		ruimoku=A021708-00010)
    (=ruimoku-v6	. #xE019)	; &RUI6-E019;
    (->subsumptive
     ((==ruimoku-v6	  . #xE019)	; &g2-RUI6-E019;
      ))
    ))
(define-char
  '((<-denotational
     ((=ucs		  . #x20E04)	; 𠸄
      ))
    (ideographic-radical . 30)	; ⼝
    (ideographic-strokes . 9)
    (total-strokes	 . 12)
    (ideographic-structure
     ((name . "IDEOGRAPHIC DESCRIPTION CHARACTER LEFT TO RIGHT")
      (=ucs		  . #x2FF0)	; ⿰
      )
     ((=ucs		  . #x53E3)	; 口
      )
     ((=ucs@cns		  . #x82F1)	; &MJ021701;
      (=mj		  . 021701)	; &MJ021701;
      ))
    (sources		ruimoku=A021761-00005)
    (=ucs@iso		. #x20E04)	; &RUI6-E01A;
    (=ruimoku-v6	. #xE01A)	; &RUI6-E01A;
    (->subsumptive
     ((==ruimoku-v6	  . #xE01A)	; &g2-RUI6-E01A;
      )
     ((==ucs@iso	  . #x20E04)	; &g2-IU-00020E04;
      ))
    ))
(define-char
  '((ideographic-radical . 9)	; ⼈
    (ideographic-strokes . 12)
    (total-strokes	 . 14)
    (ideographic-structure
     ((name . "IDEOGRAPHIC DESCRIPTION CHARACTER LEFT TO RIGHT")
      (=ucs		  . #x2FF0)	; ⿰
      )
     ((=ucs		  . #x4EBB)	; 亻
      )
     ((=ucs		  . #x26A36)	; 𦨶
      ))
    (sources		ruimoku=A021894-00009)
    (=ruimoku-v6	. #xE01B)	; &RUI6-E01B;
    (->subsumptive
     ((==ruimoku-v6	  . #xE01B)	; &g2-RUI6-E01B;
      ))
    ))
(define-char
  '((<-denotational
     ((=ucs		  . #x3694)	; 㚔
      ))
    (ideographic-radical . 37)	; ⼤
    (ideographic-strokes . 5)
    (total-strokes	 . 8)
    (ideographic-structure
     ((name . "IDEOGRAPHIC DESCRIPTION CHARACTER ABOVE TO BELOW")
      (=ucs		  . #x2FF1)	; ⿱
      )
     ((=ucs		  . #x5927)	; 大
      )
     ((=ucs@iso		  . #x22189)	; &MJ035213;
      (=mj		  . 035213)	; &MJ035213;
      ))
    (sources		ruimoku=A2003-10837-00013)
    (=ucs@unicode	. #x3694)	; &MJ000571;
    (=mj		. 000571)	; &MJ000571;
    (=cns11643-4	. #x2571)	; &I-C4-2571; [05-81]
    (=big5-cdp		. #x8ABA)	; &I-CDP-8ABA;
    (=koseki		. 067420)	; &MJ000571;
    (=daikanwa		. 05884)	; &I-M-05884;
    (=ruimoku-v6	. #xE01C)	; &I-RUI6-E01C;
    (->subsumptive
     ((==ucs@unicode	  . #x3694)	; &g2-MJ000571;
      (==mj		  . 000571)	; &g2-MJ000571;
      (==cns11643-4	  . #x2571)	; &g2-MJ000571; [05-81]
      (==big5-cdp	  . #x8ABA)	; &g2-MJ000571;
      (==koseki		  . 067420)	; &g2-MJ000571;
      (==daikanwa	  . 05884)	; &g2-MJ000571;
      (==ruimoku-v6	  . #xE01C)	; &g2-MJ000571;
      (->subsumptive
       ((===daikanwa	    . 05884)	; &R-M-05884;
	)
       ((===mj		    . 000571)	; &R-MJ000571;
	)
       ((===big5-cdp	    . #x8ABA)	; &R-CDP-8ABA;
	)
       ((===cns11643-4	    . #x2571)	; &R-C4-2571; [05-81]
	)
       ((===ucs@unicode	    . #x3694)	; &R-UU+3694;
	)
       ((===ruimoku-v6	    . #xE01C)	; &R-RUI6-E01C;
	))
      ))
    ))
(define-char
  '((ideographic-radical . 169)	; ⾨
    (ideographic-strokes . 7)
    (total-strokes	 . 15)
    (ideographic-structure
     ((name . "IDEOGRAPHIC DESCRIPTION CHARACTER SURROUND FROM ABOVE")
      (=ucs		  . #x2FF5)	; ⿵
      )
     ((=ucs		  . #x9580)	; 門
      )
     ((=ucs		  . #x4E32)	; 串
      ))
    (sources		ruimoku=A2003-10922-00012)
    (=ucs		. #x2B522)	; 𫔢
    (=ruimoku-v6	. #xE01D)	; &I-RUI6-E01D;
    (->subsumptive
     ((==ucs@iso	  . #x2B522)	; &g2-IU-0002B522;
      (==ruimoku-v6	  . #xE01D)	; &g2-IU-0002B522;
      (->subsumptive
       ((===ucs@iso	    . #x2B522)	; &R-U-0002B522;
	)
       ((===ruimoku-v6	    . #xE01D)	; &R-RUI6-E01D;
	))
      ))
    ))
(define-char
  '((ideographic-radical . 79)	; ⽎
    (ideographic-strokes . 8)
    (total-strokes	 . 12)
    (ideographic-structure
     ((name . "IDEOGRAPHIC DESCRIPTION CHARACTER SURROUND FROM UPPER RIGHT")
      (=ucs		  . #x2FF9)	; ⿹
      )
     ((ideographic-structure
       ((name . "IDEOGRAPHIC DESCRIPTION CHARACTER LEFT TO RIGHT")
	(=ucs		    . #x2FF0)	; ⿰
	)
       ((=ucs		    . #x81FC)	; 臼
	)
       ((=ucs		    . #x6BB3)	; 殳
	))
      )
     ((=ucs		  . #x53B6)	; 厶
      ))
    (sources		ruimoku=A2003-10440-00005)
    (=ruimoku-v6	. #xE01E)	; &RUI6-E01E;
    ))
(define-char
  '((ideographic-radical . 149)	; ⾔
    (ideographic-strokes . 12)
    (total-strokes	 . 19)
    (ideographic-structure
     ((name . "IDEOGRAPHIC DESCRIPTION CHARACTER LEFT TO RIGHT")
      (=ucs		  . #x2FF0)	; ⿰
      )
     ((=ucs		  . #x8A00)	; 言
      )
     ((=ucs		  . #x83D0)	; 菐
      ))
    (sources		ruimoku=A2003-10165-00011 ruimoku=A2003-00594-00003)
    (=ruimoku-v6	. #xE01F)	; &RUI6-E01F;
    (->subsumptive
     ((==ruimoku-v6	  . #xE01F)	; &g2-RUI6-E01F;
      ))
    ))
(define-char
  '((sources		ruimoku=A2003-00092-00005)
    (=ruimoku-v6	. #xE020)	; &RUI6-E020;
    ))
(define-char
  '((<-denotational
     ((=>ruimoku-v6	  . #xE021)	; &A-RUI6-E021;
      ))
    (ideographic-radical . 162)	; ⾡
    (ideographic-strokes . 10)
    (total-strokes	 . 13)
    (ideographic-structure
     ((name . "IDEOGRAPHIC DESCRIPTION CHARACTER SURROUND FROM LOWER LEFT")
      (=ucs		  . #x2FFA)	; ⿺
      )
     ((name		  . "CJK RADICAL SIMPLIFIED WALK")
      (=ucs		  . #x2ECC)	; ⻌
      )
     ((=ucs-itaiji-002	  . #x209AA)	; &U-i002+209AA;
      ))
    (sources		ruimoku=A2003-10439-00012 ruimoku=A2003-11145-00003)
    (=ruimoku-v6	. #xE021)	; &RUI6-E021;
    ))
(define-char
  '((ideographic-radical . 27)	; ⼚
    (ideographic-strokes . 7)
    (total-strokes	 . 9)
    (ideographic-structure
     ((name . "IDEOGRAPHIC DESCRIPTION CHARACTER SURROUND FROM UPPER LEFT")
      (=ucs		  . #x2FF8)	; ⿸
      )
     ((=ucs		  . #x5382)	; 厂
      )
     ((=ucs		  . #x653C)	; 攼
      ))
    (sources		ruimoku=A2003-10271-00006)
    (=ruimoku-v6	. #xE022)	; &RUI6-E022;
    ))
(define-char
  '((sources		ruimoku=A2003-10525-00013)
    (=ruimoku-v6	. #xE023)	; &RUI6-E023;
    ))
(define-char
  '((sources		ruimoku=A2004-30015-00022)
    (=ruimoku-v6	. #xE024)	; &RUI6-E024;
    ))
(define-char
  '((=ruimoku-v6	. #xE025)	; &RUI6-E025;
    ))
(define-char
  '((=ruimoku-v6	. #xE026)	; &RUI6-E026;
    ))
(define-char
  '((ideographic-radical . 170)	; ⾩
    (ideographic-strokes . 14)
    (total-strokes	 . 17)
    (ideographic-structure
     ((name . "IDEOGRAPHIC DESCRIPTION CHARACTER LEFT TO RIGHT")
      (=ucs		  . #x2FF0)	; ⿰
      )
     ((name		  . "CJK RADICAL MOUND TWO")
      (=ucs		  . #x2ED6)	; ⻖
      )
     ((=ruimoku-v6	  . #xE05F)	; &RUI6-E05F;
      ))
    (=ruimoku-v6	. #xE027)	; &RUI6-E027;
    (->subsumptive
     ((==ruimoku-v6	  . #xE027)	; &g2-RUI6-E027;
      ))
    ))
(define-char
  '((ideographic-radical . 61)	; ⼼
    (ideographic-strokes . 4)
    (total-strokes	 . 8)
    (ideographic-structure
     ((name . "IDEOGRAPHIC DESCRIPTION CHARACTER ABOVE TO BELOW")
      (=ucs		  . #x2FF1)	; ⿱
      )
     ((=ucs		  . #x5143)	; 元
      )
     ((=ucs		  . #x5FC3)	; 心
      ))
    (=ucs		. #x2BE75)	; 𫹵
    (=ruimoku-v6	. #xE028)	; &I-RUI6-E028;
    (->subsumptive
     ((==ucs@iso	  . #x2BE75)	; &g2-IU-0002BE75;
      )
     ((==ruimoku-v6	  . #xE028)	; &g2-RUI6-E028;
      ))
    ))
(define-char
  '((=ruimoku-v6	. #xE029)	; &RUI6-E029;
    ))
(define-char
  '((<-denotational
     ((=>ruimoku-v6	  . #xE02A)	; &A-RUI6-E02A;
      ))
    (ideographic-radical . 18)	; ⼑
    (ideographic-strokes . 9)
    (total-strokes	 . 11)
    (ideographic-structure
     ((name . "IDEOGRAPHIC DESCRIPTION CHARACTER LEFT TO RIGHT")
      (=ucs		  . #x2FF0)	; ⿰
      )
     ((=ucs@unicode	  . #x65E2)	; &AJ1-01591;
      (=adobe-japan1-0	  . 01591)	; &AJ1-01591;
      )
     ((=ucs		  . #x5202)	; 刂
      ))
    (=ruimoku-v6	. #xE02A)	; &RUI6-E02A;
    (->subsumptive
     ((==ruimoku-v6	  . #xE02A)	; &g2-RUI6-E02A;
      ))
    ))
(define-char
  '((=ruimoku-v6	. #xE02B)	; &RUI6-E02B;
    ))
(define-char
  '((=ruimoku-v6	. #xE02C)	; &RUI6-E02C;
    ))
(define-char
  '((<-denotational
     ((=ucs		  . #x2C433)	; 𬐳
      ))
    (ideographic-radical . 108)	; ⽫
    (ideographic-strokes . 12)
    (total-strokes	 . 17)
    (ideographic-structure
     ((name . "IDEOGRAPHIC DESCRIPTION CHARACTER ABOVE TO BELOW")
      (=ucs		  . #x2FF1)	; ⿱
      )
     ((=ucs@unicode	  . #x7121)	; &AJ1-03777;
      (=adobe-japan1-0	  . 03777)	; &AJ1-03777;
      )
     ((=ucs		  . #x76BF)	; 皿
      ))
    (=ucs@JP/hanazono	. #x2C433)	; &RUI6-E02D;
    (=ruimoku-v6	. #xE02D)	; &RUI6-E02D;
    (->subsumptive
     ((==ucs@JP/hanazono  . #x2C433)	; &g2-hanaJU+2C433;
      (==ruimoku-v6	  . #xE02D)	; &g2-hanaJU+2C433;
      ))
    ))
(define-char
  '((ideographic-radical . 18)	; ⼑
    (ideographic-strokes . 4)
    (total-strokes	 . 6)
    (ideographic-structure
     ((name . "IDEOGRAPHIC DESCRIPTION CHARACTER ABOVE TO BELOW")
      (=ucs		  . #x2FF1)	; ⿱
      )
     ((=ucs		  . #x5200)	; 刀
      )
     ((=big5-cdp	  . #x89AE)	; &CDP-89AE;
      ))
    (=ruimoku-v6	. #xE02E)	; &RUI6-E02E;
    (<-formed
     ((=ucs		  . #x5215)	; 刕
      ))
    (->subsumptive
     ((==ruimoku-v6	  . #xE02E)	; &g2-RUI6-E02E;
      ))
    ))
(define-char
  '((<-denotational
     ((=>ruimoku-v6	  . #xE02F)	; &A-RUI6-E02F;
      ))
    (ideographic-radical . 86)	; ⽕
    (ideographic-strokes . 6)
    (total-strokes	 . 10)
    (ideographic-structure
     ((name . "IDEOGRAPHIC DESCRIPTION CHARACTER LEFT TO RIGHT")
      (=ucs		  . #x2FF0)	; ⿰
      )
     ((=ucs		  . #x706B)	; 火
      )
     ((=ucs		  . #x826E)	; 艮
      ))
    (=ruimoku-v6	. #xE02F)	; &RUI6-E02F;
    (->subsumptive
     ((==ruimoku-v6	  . #xE02F)	; &g2-RUI6-E02F;
      ))
    ))
(define-char
  '((<-denotational
     ((=ucs		  . #x2C2E7)	; 𬋧
      ))
    (ideographic-radical . 87)	; ⽖
    (ideographic-strokes . 6)
    (total-strokes	 . 10)
    (ideographic-structure
     ((name . "IDEOGRAPHIC DESCRIPTION CHARACTER ABOVE TO BELOW")
      (=ucs		  . #x2FF1)	; ⿱
      )
     ((name		  . "CJK RADICAL PAW ONE")
      (=ucs		  . #x2EA4)	; ⺤
      )
     ((=ucs@unicode	  . #x8863)	; &AJ1-01189;
      (=adobe-japan1-0	  . 01189)	; &AJ1-01189;
      ))
    (=ruimoku-v6	. #xE030)	; &RUI6-E030;
    (->subsumptive
     ((==ruimoku-v6	  . #xE030)	; &g2-RUI6-E030;
      ))
    ))
(define-char
  '((=ruimoku-v6	. #xE031)	; &RUI6-E031;
    ))
(define-char
  '((=ruimoku-v6	. #xE032)	; &RUI6-E032;
    ))
(define-char
  '((<-denotational
     ((=>ruimoku-v6	  . #xE033)	; &A-RUI6-E033;
      ))
    (ideographic-radical . 189)	; ⾼
    (ideographic-strokes . 11)
    (total-strokes	 . 19)
    (ideographic-structure
     ((name . "IDEOGRAPHIC DESCRIPTION CHARACTER LEFT TO RIGHT")
      (=ucs		  . #x2FF0)	; ⿰
      )
     ((=ucs@iso		  . #x2CD18)	; &U-0002CD18;
      )
     ((=ucs@unicode	  . #x53CD)	; &AJ1-03411;
      (=adobe-japan1-0	  . 03411)	; &AJ1-03411;
      ))
    (sources		ruimoku=A2005-10807-00005 ruimoku=A2005-10807-00006)
    (=ruimoku-v6	. #xE033)	; &RUI6-E033;
    (->subsumptive
     ((==ruimoku-v6	  . #xE033)	; &g2-RUI6-E033;
      ))
    ))
(define-char
  '((script		Oracle-Bones Ideographs)
    (sources		ruimoku=A2005-10815-00001)
    (=ruimoku-v6	. #xE034)	; &RUI6-E034;
    ))
(define-char
  '((ideographic-radical . 30)	; ⼝
    (ideographic-strokes . 4)
    (total-strokes	 . 7)
    (ideographic-structure
     ((name . "IDEOGRAPHIC DESCRIPTION CHARACTER ABOVE TO BELOW")
      (=ucs		  . #x2FF1)	; ⿱
      )
     ((=ucs		  . #x53E3)	; 口
      )
     ((=ucs		  . #x6587)	; 文
      ))
    (=ruimoku-v6	. #xE035)	; &RUI6-E035;
    (->subsumptive
     ((==ruimoku-v6	  . #xE035)	; &g2-RUI6-E035;
      ))
    ))
(define-char
  '((ideographic-radical . 149)	; ⾔
    (ideographic-strokes . 6)
    (total-strokes	 . 13)
    (ideographic-structure
     ((name . "IDEOGRAPHIC DESCRIPTION CHARACTER LEFT TO RIGHT")
      (=ucs		  . #x2FF0)	; ⿰
      )
     ((=ucs		  . #x8A00)	; 言
      )
     ((ideographic-structure
       ((name . "IDEOGRAPHIC DESCRIPTION CHARACTER ABOVE TO BELOW")
	(=ucs		    . #x2FF1)	; ⿱
	)
       ((=ucs		    . #x5348)	; 午
	)
       ((=ucs		    . #x53C8)	; 又
	))
      ))
    (=ruimoku-v6	. #xE036)	; &RUI6-E036;
    (->subsumptive
     ((==ruimoku-v6	  . #xE036)	; &g2-RUI6-E036;
      ))
    ))
(define-char
  '((ideographic-radical . 172)	; ⾫
    (ideographic-strokes . 9)
    (total-strokes	 . 17)
    (ideographic-structure@apparent
     ((name . "IDEOGRAPHIC DESCRIPTION CHARACTER LEFT TO RIGHT")
      (=ucs		  . #x2FF0)	; ⿰
      )
     ((=ucs		  . #x4EB2)	; 亲
      )
     ((=ucs		  . #x96B9)	; 隹
      ))
    (sound@ja/on	"zatu")
    (=ucs		. #x2E980)	; 𮦀
    (=mj		. 060323)	; &I-MJ060323;
    (=ruimoku-v6	. #xE037)	; &I-RUI6-E037;
    (->subsumptive
     ((==ucs@JP/hanazono  . #x2E980)	; &g2-MJ060323;
      (==mj		  . 060323)	; &g2-MJ060323;
      (==ruimoku-v6	  . #xE037)	; &g2-MJ060323;
      (->subsumptive
       ((===mj		    . 060323)	; &R-MJ060323;
	)
       ((===ruimoku-v6	    . #xE037)	; &R-RUI6-E037;
	))
      ))
    ))
(define-char
  '((<-denotational
     ((=ucs		  . #x2A75D)	; 𪝝
      ))
    (ideographic-radical . 9)	; ⼈
    (ideographic-strokes . 10)
    (total-strokes	 . 12)
    (ideographic-structure
     ((name . "IDEOGRAPHIC DESCRIPTION CHARACTER LEFT TO RIGHT")
      (=ucs		  . #x2FF0)	; ⿰
      )
     ((=ucs		  . #x4EBB)	; 亻
      )
     ((=ucs@unicode	  . #x6715)	; &AJ1-03035;
      (=adobe-japan1-0	  . 03035)	; &AJ1-03035;
      ))
    (=ruimoku-v6	. #xE038)	; &RUI6-E038;
    (->subsumptive
     ((==ruimoku-v6	  . #xE038)	; &g2-RUI6-E038;
      ))
    ))
(define-char
  '((<-denotational
     ((=ucs		  . #x2B41F)	; 𫐟
      ))
    (ideographic-radical . 162)	; ⾡
    (ideographic-strokes . 3)
    (total-strokes	 . 6)
    (ideographic-structure
     ((name . "IDEOGRAPHIC DESCRIPTION CHARACTER SURROUND FROM LOWER LEFT")
      (=ucs		  . #x2FFA)	; ⿺
      )
     ((name		  . "CJK RADICAL SIMPLIFIED WALK")
      (=ucs		  . #x2ECC)	; ⻌
      )
     ((=gt-k		  . 00233)	; &GT-K00233;
      ))
    (sources		ruimoku=2006-10304-00006)
    (=ruimoku-v6	. #xE039)	; &RUI6-E039;
    (->subsumptive
     ((==ruimoku-v6	  . #xE039)	; &g2-RUI6-E039;
      ))
    ))
(define-char
  '((=ruimoku-v6	. #xE03A)	; &RUI6-E03A;
    ))
(define-char
  '((ideographic-radical . 40)	; ⼧
    (ideographic-strokes . 11)
    (total-strokes	 . 14)
    (ideographic-structure
     ((name . "IDEOGRAPHIC DESCRIPTION CHARACTER ABOVE TO BELOW")
      (=ucs		  . #x2FF1)	; ⿱
      )
     ((=ucs		  . #x5B80)	; 宀
      )
     ((=ucs		  . #x2A7CD)	; 𪟍
      ))
    (=ruimoku-v6	. #xE03B)	; &RUI6-E03B;
    (->subsumptive
     ((==ruimoku-v6	  . #xE03B)	; &g2-RUI6-E03B;
      ))
    ))
(define-char
  '((ideographic-radical . 18)	; ⼑
    (ideographic-strokes . 9)
    (total-strokes	 . 11)
    (ideographic-structure
     ((name . "IDEOGRAPHIC DESCRIPTION CHARACTER LEFT TO RIGHT")
      (=ucs		  . #x2FF0)	; ⿰
      )
     ((=ucs		  . #x2668F)	; 𦚏
      )
     ((=ucs		  . #x5202)	; 刂
      ))
    (=ucs		. #x2A7CD)	; 𪟍
    (=ruimoku-v6	. #xE03C)	; &I-RUI6-E03C;
    (<-construed@chu
     ((=ucs		  . #x534A)	; 半
      ))
    (->subsumptive
     ((==ucs@JP/hanazono  . #x2A7CD)	; &g2-hanaJU+2A7CD;
      (==ruimoku-v6	  . #xE03C)	; &g2-hanaJU+2A7CD;
      ))
    ))
(define-char
  '((<-denotational
     ((=ucs		  . #x26BD7)	; 𦯗
      ))
    (ideographic-radical . 140)	; ⾋
    (ideographic-strokes . 7)
    (total-strokes	 . 11)
    (ideographic-structure
     ((name . "IDEOGRAPHIC DESCRIPTION CHARACTER ABOVE TO BELOW")
      (=ucs		  . #x2FF1)	; ⿱
      )
     ((=ucs@cns		  . #x8289)	; &C4-2475;
      (=cns11643-4	  . #x2475)	; &C4-2475; [04-85]
      (=daikanwa	  . 30668)	; &I-M-30668;
      )
     ((ideographic-structure
       ((name . "IDEOGRAPHIC DESCRIPTION CHARACTER SURROUND FROM UPPER RIGHT")
	(=ucs		    . #x2FF9)	; ⿹
	)
       ((=ucs		    . #x200CC)	; 𠃌
	)
       ((=ucs		    . #x53E3)	; 口
	))
      ))
    (=ruimoku-v6	. #xE03D)	; &RUI6-E03D;
    (<-unknown
     ((=ucs		  . #x26C86)	; 𦲆
      ))
    ))
(define-char
  '((<-denotational
     ((=>ruimoku-v6	  . #xE03E)	; &A-RUI6-E03E;
      ))
    (ideographic-radical . 140)	; ⾋
    (ideographic-strokes . 9)
    (total-strokes	 . 12)
    (ideographic-structure
     ((name . "IDEOGRAPHIC DESCRIPTION CHARACTER FULL SURROUND")
      (=ucs		  . #x2FF4)	; ⿴
      )
     ((ideographic-structure
       ((name . "IDEOGRAPHIC DESCRIPTION CHARACTER ABOVE TO BELOW")
	(=ucs		    . #x2FF1)	; ⿱
	)
       ((name		    . "CJK RADICAL GRASS ONE")
	(=ucs		    . #x2EBE)	; ⺾
	)
       ((=ucs		    . #x5EFE)	; 廾
	))
      )
     ((ideographic-structure
       ((name . "IDEOGRAPHIC DESCRIPTION CHARACTER ABOVE TO BELOW")
	(=ucs		    . #x2FF1)	; ⿱
	)
       ((=ucs		    . #x201A2)	; 𠆢
	)
       ((=ucs		    . #x65B9)	; 方
	))
      ))
    (=ruimoku-v6	. #xE03E)	; &RUI6-E03E;
    (<-unknown
     ((=ucs		  . #x2C767)	; 𬝧
      ))
    (->subsumptive
     ((==ruimoku-v6	  . #xE03E)	; &g2-RUI6-E03E;
      ))
    ))
(define-char
  '((ideographic-radical . 163)	; ⾢
    (ideographic-strokes . 13)
    (total-strokes	 . 16)
    (ideographic-structure
     ((name . "IDEOGRAPHIC DESCRIPTION CHARACTER LEFT TO RIGHT")
      (=ucs		  . #x2FF0)	; ⿰
      )
     ((=ucs		  . #x5ECC)	; 廌
      )
     ((name		  . "CJK RADICAL CITY")
      (=ucs		  . #x2ECF)	; ⻏
      ))
    (=ruimoku-v6	. #xE03F)	; &RUI6-E03F;
    (->subsumptive
     ((==ruimoku-v6	  . #xE03F)	; &g2-RUI6-E03F;
      ))
    ))
(define-char
  '((=ruimoku-v6	. #xE040)	; &RUI6-E040;
    ))
(define-char
  '((<-denotational
     ((=ucs		  . #x2A894)	; 𪢔
      ))
    (ideographic-radical . 30)	; ⼝
    (ideographic-strokes . 13)
    (total-strokes	 . 16)
    (ideographic-structure
     ((name . "IDEOGRAPHIC DESCRIPTION CHARACTER LEFT TO RIGHT")
      (=ucs		  . #x2FF0)	; ⿰
      )
     ((=ucs		  . #x53E3)	; 口
      )
     ((=ucs		  . #x5ECC)	; 廌
      ))
    (=ucs@JP/hanazono	. #x2A894)	; &RUI6-E041;
    (=ruimoku-v6	. #xE041)	; &RUI6-E041;
    (->subsumptive
     ((==ucs@JP/hanazono  . #x2A894)	; &g2-hanaJU+2A894;
      (==ruimoku-v6	  . #xE041)	; &g2-hanaJU+2A894;
      (->subsumptive
       ((===ruimoku-v6	    . #xE041)	; &R-RUI6-E041;
	))
      ))
    ))
(define-char
  '((ideographic-radical . 20)	; ⼓
    (ideographic-strokes . 14)
    (total-strokes	 . 16)
    (ideographic-structure
     ((name . "IDEOGRAPHIC DESCRIPTION CHARACTER SURROUND FROM UPPER RIGHT")
      (=ucs		  . #x2FF9)	; ⿹
      )
     ((=ucs		  . #x52F9)	; 勹
      )
     ((=ucs		  . #x718F)	; 熏
      ))
    (=ucs		. #x31441)	; 𱑁
    (=ruimoku-v6	. #xE042)	; &I-RUI6-E042;
    (->subsumptive
     ((==ucs@JP/hanazono  . #x31441)	; &g2-hanaJU+31441;
      )
     ((==ruimoku-v6	  . #xE042)	; &g2-RUI6-E042;
      ))
    ))
(define-char
  '((=ruimoku-v6	. #xE043)	; &RUI6-E043;
    ))
(define-char
  '((=ruimoku-v6	. #xE044)	; &RUI6-E044;
    ))
(define-char
  '((=ruimoku-v6	. #xE045)	; &RUI6-E045;
    ))
(define-char
  '((ideographic-radical . 108)	; ⽫
    (ideographic-strokes . 16)
    (total-strokes	 . 21)
    (ideographic-structure
     ((name . "IDEOGRAPHIC DESCRIPTION CHARACTER ABOVE TO BELOW")
      (=ucs		  . #x2FF1)	; ⿱
      )
     ((ideographic-structure
       ((name . "IDEOGRAPHIC DESCRIPTION CHARACTER LEFT TO RIGHT")
	(=ucs		    . #x2FF0)	; ⿰
	)
       ((=ucs		    . #x4EBB)	; 亻
	)
       ((=gt-k		    . 11538)	; &GT-K11538;
	))
      )
     ((=ucs		  . #x76BF)	; 皿
      ))
    (=ruimoku-v6	. #xE046)	; &RUI6-E046;
    (->subsumptive
     ((==ruimoku-v6	  . #xE046)	; &g2-RUI6-E046;
      ))
    ))
(define-char
  '((<-denotational
     ((=>ruimoku-v6	  . #xE047)	; &A-RUI6-E047;
      ))
    (ideographic-radical . 29)	; ⼜
    (ideographic-strokes . 20)
    (total-strokes	 . 22)
    (ideographic-structure
     ((name . "IDEOGRAPHIC DESCRIPTION CHARACTER SURROUND FROM LOWER LEFT")
      (=ucs		  . #x2FFA)	; ⿺
      )
     ((=ucs-var-001	  . #x23FE1)	; &U-v001+23FE1;
      )
     ((=ucs		  . #x821F)	; 舟
      ))
    (ideographic-structure@apparent
     ((name . "IDEOGRAPHIC DESCRIPTION CHARACTER ABOVE TO BELOW")
      (=ucs		  . #x2FF1)	; ⿱
      )
     ((ideographic-structure
       ((name . "IDEOGRAPHIC DESCRIPTION CHARACTER LEFT TO RIGHT")
	(=ucs		    . #x2FF0)	; ⿰
	)
       ((=ucs@unicode	    . #x6F01)	; &MJ015800;
	(=mj		    . 015800)	; &MJ015800;
	)
       ((=ucs		    . #x821F)	; 舟
	))
      )
     ((=ucs		  . #x53C8)	; 又
      ))
    (=ruimoku-v6	. #xE047)	; &RUI6-E047;
    (->subsumptive
     ((==ruimoku-v6	  . #xE047)	; &g2-RUI6-E047;
      ))
    ))
(define-char
  '((ideographic-radical . 61)	; ⼼
    (ideographic-strokes . 7)
    (total-strokes	 . 11)
    (ideographic-structure
     ((name . "IDEOGRAPHIC DESCRIPTION CHARACTER ABOVE TO BELOW")
      (=ucs		  . #x2FF1)	; ⿱
      )
     ((=ucs		  . #x8EAB)	; 身
      )
     ((=ucs		  . #x5FC3)	; 心
      ))
    (=ucs		. #x30462)	; 𰑢
    (=ruimoku-v6	. #xE048)	; &I-RUI6-E048;
    (->subsumptive
     ((==ruimoku-v6	  . #xE048)	; &g2-RUI6-E048;
      )
     ((==ucs@JP/hanazono  . #x30462)	; &g2-hanaJU+30462;
      ))
    ))
(define-char
  '((=ruimoku-v6	. #xE049)	; &RUI6-E049;
    ))
(define-char
  '((=ruimoku-v6	. #xE04A)	; &RUI6-E04A;
    ))
(define-char
  '((=ruimoku-v6	. #xE04B)	; &RUI6-E04B;
    ))
(define-char
  '((ideographic-radical . 170)	; ⾩
    (ideographic-strokes . 10)
    (total-strokes	 . 13)
    (ideographic-structure
     ((name . "IDEOGRAPHIC DESCRIPTION CHARACTER LEFT TO RIGHT")
      (=ucs		  . #x2FF0)	; ⿰
      )
     ((name		  . "CJK RADICAL MOUND TWO")
      (=ucs		  . #x2ED6)	; ⻖
      )
     ((ideographic-structure
       ((name . "IDEOGRAPHIC DESCRIPTION CHARACTER ABOVE TO BELOW")
	(=ucs		    . #x2FF1)	; ⿱
	)
       ((=ucs		    . #x5723)	; 圣
	)
       ((=ucs		    . #x5723)	; 圣
	))
      ))
    (=ruimoku-v6	. #xE04C)	; &RUI6-E04C;
    ))
(define-char
  '((=ruimoku-v6	. #xE04D)	; &RUI6-E04D;
    ))
(define-char
  '((=ruimoku-v6	. #xE04E)	; &RUI6-E04E;
    ))
(define-char
  '((ideographic-radical . 108)	; ⽫
    (ideographic-strokes . 6)
    (total-strokes	 . 11)
    (ideographic-structure
     ((name . "IDEOGRAPHIC DESCRIPTION CHARACTER ABOVE TO BELOW")
      (=ucs		  . #x2FF1)	; ⿱
      )
     ((=ucs		  . #x5B87)	; 宇
      )
     ((=ucs		  . #x76BF)	; 皿
      ))
    (=ucs		. #x2C41E)	; 𬐞
    (=ruimoku-v6	. #xE04F)	; &I-RUI6-E04F;
    (->subsumptive
     ((==ucs@JP/hanazono  . #x2C41E)	; &g2-hanaJU+2C41E;
      (==ruimoku-v6	  . #xE04F)	; &g2-hanaJU+2C41E;
      (->subsumptive
       ((===ruimoku-v6	    . #xE04F)	; &R-RUI6-E04F;
	)
       ((=ruimoku-v6	    . #xE0EE)	; &RUI6-E0EE;
	(->subsumptive
	 ((==ruimoku-v6	      . #xE0EE)	; &g2-RUI6-E0EE;
	  ))
	))
      ))
    ))
(define-char
  '((ideographic-structure
     ((name . "IDEOGRAPHIC DESCRIPTION CHARACTER SURROUND FROM UPPER LEFT")
      (=ucs		  . #x2FF8)	; ⿸
      )
     ((=ucs		  . #x864D)	; 虍
      )
     ((=ucs		  . #x9C7C)	; 鱼
      ))
    (=ruimoku-v6	. #xE050)	; &RUI6-E050;
    (<-simplified@CN/misc
     ((=ucs		  . #x2B2A3)	; 𫊣
      ))
    (->subsumptive
     ((==ruimoku-v6	  . #xE050)	; &g2-RUI6-E050;
      ))
    ))
(define-char
  '((total-strokes	 . 6)
    (ideographic-structure
     ((name . "IDEOGRAPHIC DESCRIPTION CHARACTER ABOVE TO BELOW")
      (=ucs		  . #x2FF1)	; ⿱
      )
     ((=ucs		  . #x51E0)	; 几
      )
     ((=ucs		  . #x65E5)	; 日
      ))
    (=ruimoku-v6	. #xE051)	; &RUI6-E051;
    (->subsumptive
     ((==ruimoku-v6	  . #xE051)	; &g2-RUI6-E051;
      ))
    ))
(define-char
  '((=ruimoku-v6	. #xE052)	; &RUI6-E052;
    ))
(define-char
  '((=ruimoku-v6	. #xE053)	; &RUI6-E053;
    ))
(define-char
  '((ideographic-structure
     ((name . "IDEOGRAPHIC DESCRIPTION CHARACTER LEFT TO RIGHT")
      (=ucs		  . #x2FF0)	; ⿰
      )
     ((=gt-k		  . 05309)	; &GT-K05309;
      )
     ((=ucs		  . #x96B9)	; 隹
      ))
    (=ruimoku-v6	. #xE054)	; &RUI6-E054;
    ))
(define-char
  '((=ruimoku-v6	. #xE055)	; &RUI6-E055;
    ))
(define-char
  '((=ruimoku-v6	. #xE056)	; &RUI6-E056;
    ))
(define-char
  '((<-denotational
     ((=ucs		  . #x30C9B)	; 𰲛
      ))
    (ideographic-radical . 141)	; ⾌
    (ideographic-strokes . 5)
    (total-strokes	 . 11)
    (ideographic-structure
     ((name . "IDEOGRAPHIC DESCRIPTION CHARACTER SURROUND FROM UPPER LEFT")
      (=ucs		  . #x2FF8)	; ⿸
      )
     ((=ucs@unicode	  . #x864E)	; &G0-3B22;
      (=gb2312		  . #x3B22)	; &G0-3B22; [27-02]
      )
     ((=ucs		  . #x53E3)	; 口
      ))
    (sources		ruimoku=A2007-10507-00090)
    (=ucs@JP/hanazono	. #x30C9B)	; &RUI6-E057;
    (=ruimoku-v6	. #xE057)	; &RUI6-E057;
    (->subsumptive
     ((==ucs@JP/hanazono  . #x30C9B)	; &g2-hanaJU+30C9B;
      (==ruimoku-v6	  . #xE057)	; &g2-hanaJU+30C9B;
      (->subsumptive
       ((===ruimoku-v6	    . #xE057)	; &R-RUI6-E057;
	))
      ))
    ))
(define-char
  '((ideographic-structure
     ((name . "IDEOGRAPHIC DESCRIPTION CHARACTER LEFT TO RIGHT")
      (=ucs		  . #x2FF0)	; ⿰
      )
     ((=ucs		  . #x9F20)	; 鼠
      )
     ((=ucs@unicode	  . #x5BFD)	; &C3-255E;
      (=cns11643-3	  . #x255E)	; &C3-255E; [05-62]
      ))
    (=ruimoku-v6	. #xE058)	; &RUI6-E058;
    ))
(define-char
  '((<-denotational
     ((=>ruimoku-v6	  . #xE059)	; &A-RUI6-E059;
      ))
    (ideographic-radical . 12)	; ⼋
    (ideographic-strokes . 11)
    (total-strokes	 . 13)
    (ideographic-structure
     ((name . "IDEOGRAPHIC DESCRIPTION CHARACTER ABOVE TO BELOW")
      (=ucs		  . #x2FF1)	; ⿱
      )
     ((=ucs		  . #x516B)	; 八
      )
     ((=ucs@unicode	  . #x9B5A)	; &AJ1-01685;
      (=adobe-japan1-0	  . 01685)	; &AJ1-01685;
      ))
    (=ruimoku-v6	. #xE059)	; &RUI6-E059;
    ))
(define-char
  '((=ruimoku-v6	. #xE05A)	; &RUI6-E05A;
    ))
(define-char
  '((=ruimoku-v6	. #xE05B)	; &RUI6-E05B;
    ))
(define-char
  '((ideographic-structure
     ((name . "IDEOGRAPHIC DESCRIPTION CHARACTER LEFT TO RIGHT")
      (=ucs		  . #x2FF0)	; ⿰
      )
     ((name		  . "CJK RADICAL SILK")
      (=ucs		  . #x2EAF)	; ⺯
      )
     ((ideographic-structure
       ((name . "IDEOGRAPHIC DESCRIPTION CHARACTER ABOVE TO BELOW")
	(=ucs		    . #x2FF1)	; ⿱
	)
       ((=ucs		    . #x81FC)	; 臼
	)
       ((=ucs		    . #x5B50)	; 子
	))
      ))
    (=ruimoku-v6	. #xE05C)	; &RUI6-E05C;
    ))
(define-char
  '((ideographic-structure
     ((name . "IDEOGRAPHIC DESCRIPTION CHARACTER LEFT TO RIGHT")
      (=ucs		  . #x2FF0)	; ⿰
      )
     ((=gt-k		  . 00495)	; &GT-K00495;
      )
     ((ideographic-structure
       ((name . "IDEOGRAPHIC DESCRIPTION CHARACTER ABOVE TO BELOW")
	(=ucs		    . #x2FF1)	; ⿱
	)
       ((=ucs		    . #x81FC)	; 臼
	)
       ((=ucs		    . #x5B50)	; 子
	))
      ))
    (=ruimoku-v6	. #xE05D)	; &RUI6-E05D;
    ))
(define-char
  '((ideographic-radical . 30)	; ⼝
    (ideographic-strokes . 7)
    (total-strokes	 . 10)
    (hanyu-dazidian	1 630 7)
    (ideographic-structure
     ((name . "IDEOGRAPHIC DESCRIPTION CHARACTER ABOVE TO BELOW")
      (=ucs		  . #x2FF1)	; ⿱
      )
     ((=ucs		  . #x5405)	; 吅
      )
     ((=ucs		  . #x6587)	; 文
      ))
    (=ucs		. #x20CF5)	; 𠳵
    (=ruimoku-v6	. #xE05E)	; &I-RUI6-E05E;
    (->subsumptive
     ((==ruimoku-v6	  . #xE05E)	; &g2-RUI6-E05E;
      )
     ((==ucs@iso	  . #x20CF5)	; &g2-IU-00020CF5;
      (->subsumptive
       ((===ucs@iso	    . #x20CF5)	; &R-U-00020CF5;
	)
       ((=hanziku-2	    . #xD3F0)	; &HZK02-D3F0;
	))
      ))
    ))
(define-char
  '((ideographic-radical . 61)	; ⼼
    (ideographic-strokes . 10)
    (total-strokes	 . 14)
    (ideographic-structure
     ((name . "IDEOGRAPHIC DESCRIPTION CHARACTER ABOVE TO BELOW")
      (=ucs		  . #x2FF1)	; ⿱
      )
     ((=ucs		  . #x20CF5)	; 𠳵
      )
     ((=ucs		  . #x5FC3)	; 心
      ))
    (=ruimoku-v6	. #xE05F)	; &RUI6-E05F;
    (->subsumptive
     ((==ruimoku-v6	  . #xE05F)	; &g2-RUI6-E05F;
      ))
    ))
(define-char
  '((=ruimoku-v6	. #xE060)	; &RUI6-E060;
    ))
(define-char
  '((ideographic-radical . 38)	; ⼥
    (ideographic-strokes . 12)
    (total-strokes	 . 15)
    (ideographic-structure
     ((name . "IDEOGRAPHIC DESCRIPTION CHARACTER LEFT TO RIGHT")
      (=ucs		  . #x2FF0)	; ⿰
      )
     ((=ucs		  . #x5973)	; 女
      )
     ((=ucs-itaiji-002	  . #x45B5)	; &U-i002+45B5;
      ))
    (=ruimoku-v6	. #xE061)	; &RUI6-E061;
    (->subsumptive
     ((==ruimoku-v6	  . #xE061)	; &g2-RUI6-E061;
      ))
    ))
(define-char
  '((ideographic-structure
     ((name . "IDEOGRAPHIC DESCRIPTION CHARACTER LEFT TO RIGHT")
      (=ucs		  . #x2FF0)	; ⿰
      )
     ((=ucs		  . #x793B)	; 礻
      )
     ((ideographic-structure
       ((name . "IDEOGRAPHIC DESCRIPTION CHARACTER ABOVE TO MIDDLE AND BELOW")
	(=ucs		    . #x2FF3)	; ⿳
	)
       ((=ucs		    . #x866B)	; 虫
	)
       ((=ucs		    . #x53E3)	; 口
	)
       ((=ucs		    . #x866B)	; 虫
	))
      ))
    (=ruimoku-v6	. #xE062)	; &RUI6-E062;
    ))
(define-char
  '((<-denotational
     ((=ucs		  . #x24C9F)	; 𤲟
      ))
    (ideographic-radical . 102)	; ⽥
    (ideographic-strokes . 8)
    (total-strokes	 . 13)
    (ideographic-structure
     ((name . "IDEOGRAPHIC DESCRIPTION CHARACTER LEFT TO RIGHT")
      (=ucs		  . #x2FF0)	; ⿰
      )
     ((=ucs		  . #x7530)	; 田
      )
     ((=ucs		  . #x9752)	; 青
      ))
    (=ucs@iso		. #x24C9F)	; &RUI6-E063;
    (=ruimoku-v6	. #xE063)	; &RUI6-E063;
    (->subsumptive
     ((==ucs@iso	  . #x24C9F)	; &g2-IU-00024C9F;
      (==ruimoku-v6	  . #xE063)	; &g2-IU-00024C9F;
      (->subsumptive
       ((===ucs@iso	    . #x24C9F)	; &R-U-00024C9F;
	)
       ((===ruimoku-v6	    . #xE063)	; &R-RUI6-E063;
	))
      ))
    ))
(define-char
  '((<-denotational
     ((=ucs		  . #x22F38)	; 𢼸
      ))
    (ideographic-radical . 66)	; ⽁
    (ideographic-strokes . 6)
    (total-strokes	 . 10)
    (ideographic-structure
     ((name . "IDEOGRAPHIC DESCRIPTION CHARACTER LEFT TO RIGHT")
      (=ucs		  . #x2FF0)	; ⿰
      )
     ((=ucs@iso		  . #x21D42)	; &U-00021D42;
      )
     ((=ucs		  . #x6535)	; 攵
      ))
    (=ucs@iso		. #x22F38)	; &RUI6-E064;
    (=ruimoku-v6	. #xE064)	; &RUI6-E064;
    (->subsumptive
     ((==ucs@iso	  . #x22F38)	; &g2-IU-00022F38;
      (->subsumptive
       ((===ucs@iso	    . #x22F38)	; &R-U-00022F38;
	)
       ((=hanziku-4	    . #xD65A)	; &HZK04-D65A;
	))
      )
     ((==ruimoku-v6	  . #xE064)	; &g2-RUI6-E064;
      ))
    ))
(define-char
  '((ideographic-radical . 149)	; ⾔
    (ideographic-strokes . 14)
    (total-strokes	 . 21)
    (ideographic-structure
     ((name . "IDEOGRAPHIC DESCRIPTION CHARACTER ABOVE TO BELOW")
      (=ucs		  . #x2FF1)	; ⿱
      )
     ((=ucs		  . #x275F8)	; 𧗸
      )
     ((=ucs		  . #x8A00)	; 言
      ))
    (=ruimoku-v6	. #xE065)	; &RUI6-E065;
    (->subsumptive
     ((==ruimoku-v6	  . #xE065)	; &g2-RUI6-E065;
      ))
    ))
(define-char
  '((ideographic-radical . 147)	; ⾒
    (ideographic-strokes . 7)
    (total-strokes	 . 14)
    (ideographic-structure
     ((name . "IDEOGRAPHIC DESCRIPTION CHARACTER LEFT TO RIGHT")
      (=ucs		  . #x2FF0)	; ⿰
      )
     ((ideographic-structure
       ((name . "IDEOGRAPHIC DESCRIPTION CHARACTER ABOVE TO MIDDLE AND BELOW")
	(=ucs		    . #x2FF3)	; ⿳
	)
       ((=ucs		    . #x53E3)	; 口
	)
       ((=ucs		    . #x3405)	; 㐅
	)
       ((=ucs		    . #x3405)	; 㐅
	))
      )
     ((=ucs		  . #x898B)	; 見
      ))
    (=ruimoku-v6	. #xE066)	; &RUI6-E066;
    ))
(define-char
  '((ideographic-radical . 62)	; ⼽
    (ideographic-strokes . 12)
    (total-strokes	 . 16)
    (ideographic-structure
     ((name . "IDEOGRAPHIC DESCRIPTION CHARACTER LEFT TO RIGHT")
      (=ucs		  . #x2FF0)	; ⿰
      )
     ((ideographic-structure
       ((name . "IDEOGRAPHIC DESCRIPTION CHARACTER ABOVE TO MIDDLE AND BELOW")
	(=ucs		    . #x2FF3)	; ⿳
	)
       ((=ucs		    . #x268FB)	; 𦣻
	)
       ((=gt-k		    . 00058)	; &GT-K00058;
	)
       ((=gt-k		    . 00439)	; &GT-K00439;
	))
      )
     ((=ucs		  . #x6208)	; 戈
      ))
    (=ruimoku-v6	. #xE067)	; &RUI6-E067;
    ))
(define-char
  '((ideographic-radical . 111)	; ⽮
    (ideographic-strokes . 3)
    (total-strokes	 . 8)
    (ideographic-structure
     ((name . "IDEOGRAPHIC DESCRIPTION CHARACTER ABOVE TO BELOW")
      (=ucs		  . #x2FF1)	; ⿱
      )
     ((=ucs		  . #x77E2)	; 矢
      )
     ((=ucs		  . #x5EFE)	; 廾
      ))
    (=ruimoku-v6	. #xE068)	; &RUI6-E068;
    ))
(define-char
  '((=ruimoku-v6	. #xE069)	; &RUI6-E069;
    (<-unknown
     ((=>zinbun-oracle	  . 00180)	; &A-ZOB-0180;
      ))
    ))
(define-char
  '((<-denotational
     ((=ucs		  . #x2BFF7)	; 𫿷
      ))
    (ideographic-radical . 68)	; ⽃
    (ideographic-strokes . 12)
    (total-strokes	 . 16)
    (ideographic-structure
     ((name . "IDEOGRAPHIC DESCRIPTION CHARACTER SURROUND FROM UPPER RIGHT")
      (=ucs		  . #x2FF9)	; ⿹
      )
     ((=ucs@unicode	  . #x659E)	; &C2-3C40;
      (=cns11643-2	  . #x3C40)	; &C2-3C40; [28-32]
      )
     ((=>ucs@component	  . #x6708)	; &A-compU+6708;
      (=>iwds-1		  .  0053)	; &A-compU+6708;
      ))
    (=ucs@JP/hanazono	. #x2BFF7)	; &RUI6-E06A;
    (=ruimoku-v6	. #xE06A)	; &RUI6-E06A;
    (->subsumptive
     ((==ucs@JP/hanazono  . #x2BFF7)	; &g2-hanaJU+2BFF7;
      )
     ((==ruimoku-v6	  . #xE06A)	; &g2-RUI6-E06A;
      ))
    ))
(define-char
  '((ideographic-radical . 163)	; ⾢
    (ideographic-strokes . 0)
    (total-strokes	 . 7)
    (ideographic-structure
     ((name . "IDEOGRAPHIC DESCRIPTION CHARACTER LEFT TO RIGHT")
      (=ucs		  . #x2FF0)	; ⿰
      )
     ((=ucs		  . #x81EC)	; 臬
      )
     ((name		  . "CJK RADICAL CITY")
      (=ucs		  . #x2ECF)	; ⻏
      ))
    (=ruimoku-v6	. #xE06B)	; &RUI6-E06B;
    (<-unknown
     ((=ucs		  . #x2E803)	; 𮠃
      ))
    (->subsumptive
     ((==ruimoku-v6	  . #xE06B)	; &g2-RUI6-E06B;
      ))
    ))
(define-char
  '((=ruimoku-v6	. #xE06C)	; &RUI6-E06C;
    ))
(define-char
  '((ideographic-radical . 82)	; ⽑
    (ideographic-strokes . 0)
    (total-strokes	 . 4)
    (ideographic-structure
     ((name . "IDEOGRAPHIC DESCRIPTION CHARACTER SURROUND FROM LOWER LEFT")
      (=ucs		  . #x2FFA)	; ⿺
      )
     ((=ucs		  . #x6BDB)	; 毛
      )
     ((=ucs		  . #x67BC)	; 枼
      ))
    (=ruimoku-v6	. #xE06D)	; &RUI6-E06D;
    (->subsumptive
     ((==ruimoku-v6	  . #xE06D)	; &g2-RUI6-E06D;
      ))
    ))
(define-char
  '((<-denotational
     ((=>ruimoku-v6	  . #xE06E)	; &A-RUI6-E06E;
      ))
    (ideographic-radical . 196)	; ⿃
    (ideographic-strokes . 8)
    (total-strokes	 . 19)
    (ideographic-structure
     ((name . "IDEOGRAPHIC DESCRIPTION CHARACTER ABOVE TO BELOW")
      (=ucs		  . #x2FF1)	; ⿱
      )
     ((=ucs@unicode	  . #x65BC)	; &AJ1-01305;
      (=adobe-japan1-0	  . 01305)	; &AJ1-01305;
      )
     ((=ucs@unicode	  . #x9CE5)	; &AJ1-03031;
      (=adobe-japan1-0	  . 03031)	; &I-AJ1-03031;
      ))
    (=ruimoku-v6	. #xE06E)	; &RUI6-E06E;
    (->subsumptive
     ((==ruimoku-v6	  . #xE06E)	; &g2-RUI6-E06E;
      ))
    ))
(define-char
  '((=ruimoku-v6	. #xE06F)	; &RUI6-E06F;
    ))
(define-char
  '((=ruimoku-v6	. #xE070)	; &RUI6-E070;
    ))
(define-char
  '((=ruimoku-v6	. #xE071)	; &RUI6-E071;
    ))
(define-char
  '((<-denotational
     ((=ucs		  . #x2C686)	; 𬚆
      ))
    (ideographic-radical . 124)	; ⽻
    (ideographic-strokes . 10)
    (total-strokes	 . 16)
    (ideographic-structure
     ((name . "IDEOGRAPHIC DESCRIPTION CHARACTER ABOVE TO BELOW")
      (=ucs		  . #x2FF1)	; ⿱
      )
     ((=ucs		  . #xFA1E)	; 羽
      )
     ((=ucs		  . #x80FD)	; 能
      ))
    (=ruimoku-v6	. #xE072)	; &RUI6-E072;
    (->subsumptive
     ((ideographic-structure
       ((name . "IDEOGRAPHIC DESCRIPTION CHARACTER ABOVE TO BELOW")
	(=ucs		    . #x2FF1)	; ⿱
	)
       ((=gt-k		    . 01762)	; &GT-K01762;
	)
       ((==ucs@unicode	    . #x80FD)	; &g2-AJ1-03315;
	(==adobe-japan1-0   . 03315)	; &g2-AJ1-03315;
	))
      (==ruimoku-v6	  . #xE072)	; &g2-RUI6-E072;
      ))
    ))
(define-char
  '((<-denotational
     ((=>ruimoku-v6	  . #xE073)	; &A-RUI6-E073;
      ))
    (ideographic-radical . 154)	; ⾙
    (ideographic-strokes . 6)
    (total-strokes	 . 13)
    (ideographic-structure
     ((name . "IDEOGRAPHIC DESCRIPTION CHARACTER ABOVE TO BELOW")
      (=ucs		  . #x2FF1)	; ⿱
      )
     ((=ruimoku-v6	  . #xE15F)	; &RUI6-E15F;
      )
     ((=ucs		  . #x8C9D)	; 貝
      ))
    (=ruimoku-v6	. #xE073)	; &RUI6-E073;
    ))
(define-char
  '((<-denotational
     ((=>ruimoku-v6	  . #xE074)	; &A-RUI6-E074;
      ))
    (ideographic-radical . 163)	; ⾢
    (ideographic-strokes . 10)
    (total-strokes	 . 13)
    (ideographic-structure
     ((name . "IDEOGRAPHIC DESCRIPTION CHARACTER LEFT TO RIGHT")
      (=ucs		  . #x2FF0)	; ⿰
      )
     ((=ucs@unicode	  . #x7559)	; &MJ017681;
      (=mj		  . 017681)	; &MJ017681;
      )
     ((name		  . "CJK RADICAL CITY")
      (=ucs		  . #x2ECF)	; ⻏
      ))
    (=ruimoku-v6	. #xE074)	; &RUI6-E074;
    (->subsumptive
     ((==ruimoku-v6	  . #xE074)	; &g2-RUI6-E074;
      ))
    ))
(define-char
  '((=ruimoku-v6	. #xE075)	; &RUI6-E075;
    ))
(define-char
  '((<-denotational
     ((=>ruimoku-v6	  . #xE076)	; &A-RUI6-E076;
      ))
    (ideographic-radical . 187)	; ⾺
    (ideographic-strokes . 9)
    (total-strokes	 . 19)
    (ideographic-structure
     ((name . "IDEOGRAPHIC DESCRIPTION CHARACTER LEFT TO RIGHT")
      (=ucs		  . #x2FF0)	; ⿰
      )
     ((=ucs		  . #x99AC)	; 馬
      )
     ((=ucs@unicode	  . #x754F)	; &AJ1-01182;
      (=adobe-japan1-0	  . 01182)	; &AJ1-01182;
      ))
    (=ruimoku-v6	. #xE076)	; &RUI6-E076;
    (->subsumptive
     ((==ruimoku-v6	  . #xE076)	; &g2-RUI6-E076;
      ))
    ))
(define-char
  '((<-denotational
     ((=>ruimoku-v6	  . #xE077)	; &A-RUI6-E077;
      ))
    (ideographic-radical . 94)	; ⽝
    (ideographic-strokes . 13)
    (total-strokes	 . 16)
    (ideographic-structure
     ((name . "IDEOGRAPHIC DESCRIPTION CHARACTER LEFT TO RIGHT")
      (=ucs		  . #x2FF0)	; ⿰
      )
     ((=ucs		  . #x72AD)	; 犭
      )
     ((=ucs@unicode	  . #x4592)	; &C3-4441;
      (=cns11643-3	  . #x4441)	; &C3-4441; [36-33]
      (=daikanwa	  . 32717)	; &I-M-32717;
      ))
    (=ruimoku-v6	. #xE077)	; &RUI6-E077;
    (->subsumptive
     ((==ruimoku-v6	  . #xE077)	; &g2-RUI6-E077;
      ))
    ))
(define-char
  '((<-denotational
     ((=>ruimoku-v6	  . #xE078)	; &A-RUI6-E078;
      ))
    (ideographic-radical . 173)	; ⾬
    (ideographic-strokes . 10)
    (total-strokes	 . 18)
    (ideographic-structure
     ((name . "IDEOGRAPHIC DESCRIPTION CHARACTER ABOVE TO BELOW")
      (=ucs		  . #x2FF1)	; ⿱
      )
     ((=ucs		  . #x96E8)	; 雨
      )
     ((=ucs@iso		  . #x247B1)	; &U-000247B1;
      ))
    (=ruimoku-v6	. #xE078)	; &RUI6-E078;
    (->subsumptive
     ((==ruimoku-v6	  . #xE078)	; &g2-RUI6-E078;
      ))
    ))
(define-char
  '((<-denotational
     ((=ucs		  . #x30DA4)	; 𰶤
      ))
    (ideographic-radical . 152)	; ⾗
    (ideographic-strokes . 2)
    (total-strokes	 . 9)
    (ideographic-structure
     ((name . "IDEOGRAPHIC DESCRIPTION CHARACTER LEFT TO RIGHT")
      (=ucs		  . #x2FF0)	; ⿰
      )
     ((=ucs@unicode	  . #x8C55)	; &MJ024906;
      (=mj		  . 024906)	; &MJ024906;
      )
     ((=ucs		  . #x5315)	; 匕
      ))
    (=ucs@JP/hanazono	. #x30DA4)	; &RUI6-E079;
    (=ruimoku-v6	. #xE079)	; &RUI6-E079;
    (->subsumptive
     ((==ruimoku-v6	  . #xE079)	; &g2-RUI6-E079;
      )
     ((==ucs@JP/hanazono  . #x30DA4)	; &g2-hanaJU+30DA4;
      ))
    ))
(define-char
  '((ideographic-radical . 60)	; ⼻
    (ideographic-strokes . 16)
    (total-strokes	 . 19)
    (ideographic-structure
     ((name . "IDEOGRAPHIC DESCRIPTION CHARACTER LEFT TO MIDDLE AND RIGHT")
      (=ucs		  . #x2FF2)	; ⿲
      )
     ((=ucs		  . #x5F73)	; 彳
      )
     ((=ucs		  . #x2BD91)	; 𫶑
      )
     ((=ucs		  . #x53C8)	; 又
      ))
    (=ruimoku-v6	. #xE07A)	; &RUI6-E07A;
    (->subsumptive
     ((==ruimoku-v6	  . #xE07A)	; &g2-RUI6-E07A;
      ))
    ))
(define-char
  '((=ruimoku-v6	. #xE07B)	; &RUI6-E07B;
    ))
(define-char
  '((=ruimoku-v6	. #xE07C)	; &RUI6-E07C;
    ))
(define-char
  '((=ruimoku-v6	. #xE07D)	; &RUI6-E07D;
    ))
(define-char
  '((ideographic-structure
     ((name . "IDEOGRAPHIC DESCRIPTION CHARACTER ABOVE TO BELOW")
      (=ucs		  . #x2FF1)	; ⿱
      )
     ((=ucs		  . #x3CBB)	; 㲻
      )
     ((=ucs		  . #x793A)	; 示
      ))
    (=ruimoku-v6	. #xE07E)	; &RUI6-E07E;
    (->subsumptive
     ((==ruimoku-v6	  . #xE07E)	; &g2-RUI6-E07E;
      ))
    ))
(define-char
  '((ideographic-radical . 38)	; ⼥
    (ideographic-strokes . 9)
    (total-strokes	 . 12)
    (ideographic-structure
     ((name . "IDEOGRAPHIC DESCRIPTION CHARACTER SURROUND FROM UPPER LEFT")
      (=ucs		  . #x2FF8)	; ⿸
      )
     ((=ucs		  . #x36B2)	; 㚲
      )
     ((=ucs		  . #x21B55)	; 𡭕
      ))
    (=ruimoku-v6	. #xE07F)	; &RUI6-E07F;
    (->subsumptive
     ((==ruimoku-v6	  . #xE07F)	; &g2-RUI6-E07F;
      ))
    ))
(define-char
  '((ideographic-radical . 37)	; ⼤
    (ideographic-strokes . 20)
    (total-strokes	 . 23)
    (ideographic-structure
     ((name . "IDEOGRAPHIC DESCRIPTION CHARACTER ABOVE TO BELOW")
      (=ucs		  . #x2FF1)	; ⿱
      )
     ((=ucs		  . #x8051)	; 聑
      )
     ((=ucs		  . #x595C)	; 奜
      ))
    (=ruimoku-v6	. #xE080)	; &RUI6-E080;
    (->subsumptive
     ((==ruimoku-v6	  . #xE080)	; &g2-RUI6-E080;
      ))
    ))
(define-char
  '((<-denotational
     ((=>ruimoku-v6	  . #xE081)	; &A-RUI6-E081;
      ))
    (ideographic-radical . 173)	; ⾬
    (ideographic-strokes . 14)
    (total-strokes	 . 22)
    (ideographic-structure
     ((name . "IDEOGRAPHIC DESCRIPTION CHARACTER ABOVE TO BELOW")
      (=ucs		  . #x2FF1)	; ⿱
      )
     ((=ucs		  . #x96E8)	; 雨
      )
     ((=ucs@unicode	  . #x9CF3)	; &AJ1-03679;
      (=adobe-japan1-0	  . 03679)	; &AJ1-03679;
      ))
    (=ruimoku-v6	. #xE081)	; &RUI6-E081;
    (->subsumptive
     ((==ruimoku-v6	  . #xE081)	; &g2-RUI6-E081;
      ))
    ))
(define-char
  '((<-denotational
     ((=>ruimoku-v6	  . #xE082)	; &A-RUI6-E082;
      ))
    (ideographic-radical . 72)	; ⽇
    (ideographic-strokes . 6)
    (total-strokes	 . 10)
    (ideographic-structure
     ((name . "IDEOGRAPHIC DESCRIPTION CHARACTER FULL SURROUND")
      (=ucs		  . #x2FF4)	; ⿴
      )
     ((=ucs		  . #x620C)	; 戌
      )
     ((=ucs		  . #x65E5)	; 日
      ))
    (=ruimoku-v6	. #xE082)	; &RUI6-E082;
    (->subsumptive
     ((==ruimoku-v6	  . #xE082)	; &g2-RUI6-E082;
      ))
    ))
(define-char
  '((<-denotational
     ((=ucs		  . #x2E6BE)	; 𮚾
      ))
    (ideographic-radical . 156)	; ⾛
    (ideographic-strokes . 10)
    (total-strokes	 . 17)
    (ideographic-structure
     ((name . "IDEOGRAPHIC DESCRIPTION CHARACTER SURROUND FROM LOWER LEFT")
      (=ucs		  . #x2FFA)	; ⿺
      )
     ((=ucs		  . #x8D70)	; 走
      )
     ((=ucs@iso		  . #x20CCB)	; &MJ032285;
      (=mj		  . 032285)	; &MJ032285;
      ))
    (=ucs@JP/hanazono	. #x2E6BE)	; &RUI6-E083;
    (=ruimoku-v6	. #xE083)	; &RUI6-E083;
    (->subsumptive
     ((==ucs@JP/hanazono  . #x2E6BE)	; &g2-hanaJU+2E6BE;
      (==ruimoku-v6	  . #xE083)	; &g2-hanaJU+2E6BE;
      (->subsumptive
       ((===ruimoku-v6	    . #xE083)	; &R-RUI6-E083;
	))
      ))
    ))
(define-char
  '((<-denotational
     ((=>ruimoku-v6	  . #xE084)	; &A-RUI6-E084;
      ))
    (ideographic-radical . 140)	; ⾋
    (ideographic-strokes . 11)
    (total-strokes	 . 14)
    (ideographic-structure
     ((name . "IDEOGRAPHIC DESCRIPTION CHARACTER ABOVE TO BELOW")
      (=ucs		  . #x2FF1)	; ⿱
      )
     ((name		  . "CJK RADICAL GRASS ONE")
      (=ucs		  . #x2EBE)	; ⺾
      )
     ((ideographic-structure
       ((name . "IDEOGRAPHIC DESCRIPTION CHARACTER LEFT TO RIGHT")
	(=ucs		    . #x2FF0)	; ⿰
	)
       ((=ucs		    . #x9091)	; 邑
	)
       ((=ucs		    . #x6208)	; 戈
	))
      ))
    (=ruimoku-v6	. #xE084)	; &RUI6-E084;
    (<-unknown
     ((=ucs		  . #x2B24F)	; 𫉏
      ))
    (->subsumptive
     ((==ruimoku-v6	  . #xE084)	; &g2-RUI6-E084;
      ))
    ))
(define-char
  '((<-denotational
     ((=ucs		  . #x795E)	; 神
      ))
    (name		. "CJK COMPATIBILITY IDEOGRAPH-FA19")
    (=>ucs*		. #x795E)	; 神
    (ideographic-radical . 113)	; ⽰
    (ideographic-strokes . 5)
    (total-strokes	 . 10)
    (ideographic-structure
     ((name . "IDEOGRAPHIC DESCRIPTION CHARACTER LEFT TO RIGHT")
      (=ucs		  . #x2FF0)	; ⿰
      )
     ((=ucs		  . #x793A)	; 示
      )
     ((=ucs		  . #x7533)	; 申
      ))
    (=ucs		. #xFA19)	; 神
    (=ucs@ks		. #x795E)	; 神
    (=adobe-japan1-2	. 08580)	; &I-AJ1-08580;
    (=ks-x1001		. #x636A)	; &I-K0-636A; [67-74]
    (=jis-x0213-1	. #x793C)	; &I-JX1-793C; [89-28]
    (=daikanwa		. 24673)	; &I-M-24673;
    (=jef-china3	. #x5CDA)	; &I-JC3-5CDA;
    (=ruimoku-v6	. #xE085)	; &I-RUI6-E085;
    (->subsumptive
     ((=decomposition@cid
       ((=ucs		    . #x795E)	; 神
	)
       ((=ucs		    . #xE0100)	; 󠄀
	))
      (==ucs@unicode	  . #xFA19)	; &g2-AJ1-08580;
      (==ucs@ks		  . #x795E)	; &g2-AJ1-08580;
      (==adobe-japan1-2	  . 08580)	; &g2-AJ1-08580;
      (==ks-x1001	  . #x636A)	; &g2-AJ1-08580; [67-74]
      (==jis-x0213-1	  . #x793C)	; &g2-AJ1-08580; [89-28]
      (==daikanwa	  . 24673)	; &g2-AJ1-08580;
      (==jef-china3	  . #x5CDA)	; &g2-AJ1-08580;
      (==ruimoku-v6	  . #xE085)	; &g2-AJ1-08580;
      (->subsumptive
       ((===daikanwa	    . 24673)	; &R-M-24673;
	)
       ((===jis-x0213-1	    . #x793C)	; &R-JX1-793C; [89-28]
	)
       ((===adobe-japan1-2  . 08580)	; &R-AJ1-08580;
	)
       ((===jef-china3	    . #x5CDA)	; &R-JC3-5CDA;
	)
       ((===ucs@ks	    . #x795E)	; &R-K0-636A;
	(===ks-x1001	    . #x636A)	; &R-K0-636A; [67-74]
	)
       ((===ucs@unicode	    . #xFA19)	; &R-UU+FA19;
	)
       ((===ruimoku-v6	    . #xE085)	; &R-RUI6-E085;
	))
      )
     ((===hng-khm	  . 04670)	; &HNG011-04670;
      )
     ((===hng-kar	  . 11230)	; &HNG018-11230;
      ))
    ))
(define-char
  '((<-denotational
     ((=ucs		  . #x698A)	; 榊
      ))
    (ideographic-radical . 75)	; ⽊
    (ideographic-strokes . 10)
    (total-strokes	 . 14)
    (=ucs@jis/2004	. #x698A)	; &MJ014256;
    (=mj		. 014256)	; &MJ014256;
    (=adobe-japan1-0	. 07686)	; &I-AJ1-07686;
    (=jis-x0208@1978	. #x3A67)	; &I-J78-3A67; [26-71]
    (=jis-x0213-1@2004	. #x3A67)	; &I-JX3-3A67; [26-71]
    (=hanyo-denshi/ft	. #x3228)	; &I-HD-FT-3228; [18-08]
    (=daikanwa		. 15352)	; &I-M-15352;
    (=ruimoku-v6	. #xE086)	; &I-RUI6-E086;
    (->subsumptive
     ((=decomposition@hanyo-denshi
       ((=ucs		    . #x698A)	; 榊
	)
       ((=ucs		    . #xE0103)	; 󠄃
	))
      (==ucs@jis/2004	  . #x698A)	; &g2-MJ014256;
      (==mj		  . 014256)	; &g2-MJ014256;
      (==adobe-japan1-0	  . 07686)	; &g2-MJ014256;
      (==jis-x0208@1978	  . #x3A67)	; &g2-MJ014256; [26-71]
      (==jis-x0213-1@2004 . #x3A67)	; &g2-MJ014256; [26-71]
      (==hanyo-denshi/ft  . #x3228)	; &g2-MJ014256; [18-08]
      (==daikanwa	  . 15352)	; &g2-MJ014256;
      (==ruimoku-v6	  . #xE086)	; &g2-MJ014256;
      (->subsumptive
       ((===daikanwa	    . 15352)	; &R-M-15352;
	)
       ((===jis-x0208@1978  . #x3A67)	; &R-J78-3A67; [26-71]
	)
       ((===jis-x0213-1@2004 . #x3A67)	; &R-HD-FT-3228; [26-71]
	(===hanyo-denshi/ft . #x3228)	; &R-HD-FT-3228; [18-08]
	)
       ((===adobe-japan1-0  . 07686)	; &R-AJ1-07686;
	)
       ((===mj		    . 014256)	; &R-MJ014256;
	)
       ((===ruimoku-v6	    . #xE086)	; &R-RUI6-E086;
	))
      ))
    ))
(define-char
  '((<-denotational
     ((=>ruimoku-v6	  . #xE087)	; &A-RUI6-E087;
      ))
    (ideographic-radical . 34)	; ⼡
    (ideographic-strokes . 11)
    (total-strokes	 . 14)
    (ideographic-structure
     ((name . "IDEOGRAPHIC DESCRIPTION CHARACTER ABOVE TO BELOW")
      (=ucs		  . #x2FF1)	; ⿱
      )
     ((=ucs@unicode	  . #x5906)	; &AJ1-21371;
      (=adobe-japan1-6	  . 21371)	; &AJ1-21371;
      )
     ((=ucs@unicode	  . #x56E7)	; &AJ1-21316;
      (=adobe-japan1-6	  . 21316)	; &AJ1-21316;
      ))
    (=ruimoku-v6	. #xE087)	; &RUI6-E087;
    ))
(define-char
  '((<-denotational
     ((=ucs		  . #x2B1C6)	; 𫇆
      ))
    (ideographic-radical . 131)	; ⾂
    (ideographic-strokes . 6)
    (total-strokes	 . 12)
    (ideographic-structure
     ((name . "IDEOGRAPHIC DESCRIPTION CHARACTER ABOVE TO BELOW")
      (=ucs		  . #x2FF1)	; ⿱
      )
     ((=ucs		  . #x81E3)	; 臣
      )
     ((=ucs		  . #x81E3)	; 臣
      ))
    (=ucs@JP/hanazono	. #x2B1C6)	; &RUI6-E088;
    (=ruimoku-v6	. #xE088)	; &RUI6-E088;
    (->subsumptive
     ((==ucs@JP/hanazono  . #x2B1C6)	; &g2-hanaJU+2B1C6;
      )
     ((==ruimoku-v6	  . #xE088)	; &g2-RUI6-E088;
      ))
    ))
(define-char
  '((<-denotational
     ((=>ruimoku-v6	  . #xE089)	; &A-RUI6-E089;
      ))
    (ideographic-radical . 167)	; ⾦
    (ideographic-strokes . 10)
    (total-strokes	 . 18)
    (ideographic-structure
     ((name . "IDEOGRAPHIC DESCRIPTION CHARACTER ABOVE TO BELOW")
      (=ucs		  . #x2FF1)	; ⿱
      )
     ((ideographic-structure
       ((name . "IDEOGRAPHIC DESCRIPTION CHARACTER LEFT TO RIGHT")
	(=ucs		    . #x2FF0)	; ⿰
	)
       ((=ucs		    . #x821F)	; 舟
	)
       ((=ucs		    . #x706B)	; 火
	))
      )
     ((=ucs@unicode	  . #x91D1)	; &AJ1-01754;
      (=adobe-japan1-0	  . 01754)	; &AJ1-01754;
      ))
    (=ruimoku-v6	. #xE089)	; &RUI6-E089;
    ))
(define-char
  '((=ruimoku-v6	. #xE08A)	; &RUI6-E08A;
    ))
(define-char
  '((=ruimoku-v6	. #xE08B)	; &RUI6-E08B;
    ))
(define-char
  '((ideographic-radical . 38)	; ⼥
    (ideographic-strokes . 2)
    (total-strokes	 . 5)
    (ideographic-structure
     ((name . "IDEOGRAPHIC DESCRIPTION CHARACTER LEFT TO RIGHT")
      (=ucs		  . #x2FF0)	; ⿰
      )
     ((=ucs		  . #x5973)	; 女
      )
     ((=ucs		  . #x529B)	; 力
      ))
    (=ucs		. #x302F6)	; 𰋶
    (=cbeta		. 11757)	; &I-CB11757;
    (=ruimoku-v6	. #xE08C)	; &I-RUI6-E08C;
    (->subsumptive
     ((==ucs@JP/hanazono  . #x302F6)	; &g2-CB11757;
      (==cbeta		  . 11757)	; &g2-CB11757;
      (==ruimoku-v6	  . #xE08C)	; &g2-CB11757;
      (->subsumptive
       ((===ruimoku-v6	    . #xE08C)	; &R-RUI6-E08C;
	)
       ((===cbeta	    . 11757)	; &R-CB11757;
	))
      ))
    ))
(define-char
  '((<-denotational
     ((=ucs		  . #x209AA)	; 𠦪
      ))
    (ideographic-radical . 24)	; ⼗
    (ideographic-strokes . 9)
    (total-strokes	 . 11)
    (ideographic-structure
     ((name . "IDEOGRAPHIC DESCRIPTION CHARACTER ABOVE TO BELOW")
      (=ucs		  . #x2FF1)	; ⿱
      )
     ((=ucs@unicode	  . #x5349)	; &AJ1-04312;
      (=adobe-japan1-0	  . 04312)	; &AJ1-04312;
      )
     ((=ucs@unicode	  . #x5932)	; &AJ1-04546;
      (=adobe-japan1-0	  . 04546)	; &AJ1-04546;
      ))
    (=ruimoku-v6	. #xE08D)	; &RUI6-E08D;
    (=ucs-var-001	. #x209AA)	; &RUI6-E08D;
    (->subsumptive
     ((==ucs-var-001	  . #x209AA)	; &g2-U-v001+209AA;
      )
     ((==ruimoku-v6	  . #xE08D)	; &g2-RUI6-E08D;
      ))
    ))
(define-char
  '((=ruimoku-v6	. #xE08E)	; &RUI6-E08E;
    ))
(define-char
  '((<-denotational
     ((=>ruimoku-v6	  . #xE08F)	; &A-RUI6-E08F;
      ))
    (ideographic-radical . 85)	; ⽔
    (ideographic-strokes . 9)
    (total-strokes	 . 12)
    (ideographic-structure
     ((name . "IDEOGRAPHIC DESCRIPTION CHARACTER LEFT TO RIGHT")
      (=ucs		  . #x2FF0)	; ⿰
      )
     ((=ucs		  . #x6C35)	; 氵
      )
     ((ideographic-structure
       ((name . "IDEOGRAPHIC DESCRIPTION CHARACTER SURROUND FROM UPPER LEFT")
	(=ucs		    . #x2FF8)	; ⿸
	)
       ((=ucs@unicode	    . #x864D)	; &AJ1-06479;
	(=adobe-japan1-0    . 06479)	; &AJ1-06479;
	)
       ((=ucs		    . #x4E5F)	; 也
	))
      ))
    (=ruimoku-v6	. #xE08F)	; &RUI6-E08F;
    (->subsumptive
     ((==ruimoku-v6	  . #xE08F)	; &g2-RUI6-E08F;
      ))
    ))
(define-char
  '((=ruimoku-v6	. #xE090)	; &RUI6-E090;
    ))
(define-char
  '((=ruimoku-v6	. #xE091)	; &RUI6-E091;
    ))
(define-char
  '((=ruimoku-v6	. #xE092)	; &RUI6-E092;
    ))
(define-char
  '((<-denotational
     ((=>ruimoku-v6	  . #xE093)	; &A-RUI6-E093;
      ))
    (ideographic-radical . 32)	; ⼟
    (ideographic-strokes . 8)
    (total-strokes	 . 11)
    (ideographic-structure
     ((name . "IDEOGRAPHIC DESCRIPTION CHARACTER LEFT TO RIGHT")
      (=ucs		  . #x2FF0)	; ⿰
      )
     ((=gt-k		  . 00199)	; &GT-K00199;
      )
     ((=ucs		  . #x7AFA)	; 竺
      ))
    (=ruimoku-v6	. #xE093)	; &RUI6-E093;
    (->subsumptive
     ((==ruimoku-v6	  . #xE093)	; &g2-RUI6-E093;
      ))
    ))
(define-char
  '((=ruimoku-v6	. #xE094)	; &RUI6-E094;
    ))
(define-char
  '((=ruimoku-v6	. #xE095)	; &RUI6-E095;
    ))
(define-char
  '((=ruimoku-v6	. #xE096)	; &RUI6-E096;
    ))
(define-char
  '((=ruimoku-v6	. #xE097)	; &RUI6-E097;
    ))
(define-char
  '((<-denotational
     ((=>ruimoku-v6	  . #xE098)	; &A-RUI6-E098;
      ))
    (ideographic-radical . 39)	; ⼦
    (ideographic-strokes . 4)
    (total-strokes	 . 7)
    (ideographic-structure
     ((name . "IDEOGRAPHIC DESCRIPTION CHARACTER LEFT TO RIGHT")
      (=ucs		  . #x2FF0)	; ⿰
      )
     ((=ucs@cns		  . #x4E11)	; &MJ006315;
      (=ucs@jis		  . #x4E11)	; &MJ006315;
      (=ucs@ks		  . #x4E11)	; &MJ006315;
      (=mj		  . 006315)	; &MJ006315;
      )
     ((=ucs		  . #x5B50)	; 子
      ))
    (=ruimoku-v6	. #xE098)	; &RUI6-E098;
    (->subsumptive
     ((==ruimoku-v6	  . #xE098)	; &g2-RUI6-E098;
      ))
    ))
(define-char
  '((ideographic-radical . 57)	; ⼸
    (ideographic-strokes . 5)
    (total-strokes	 . 8)
    (ideographic-structure
     ((name . "IDEOGRAPHIC DESCRIPTION CHARACTER SURROUND FROM UPPER LEFT")
      (=ucs		  . #x2FF8)	; ⿸
      )
     ((=ucs		  . #x2AABA)	; 𪪺
      )
     ((=ucs		  . #x4E8C)	; 二
      ))
    (=ruimoku-v6	. #xE099)	; &RUI6-E099;
    ))
(define-char
  '((<-denotational
     ((=ucs		  . #x30AE8)	; 𰫨
      ))
    (ideographic-radical . 120)	; ⽷
    (ideographic-strokes . 7)
    (total-strokes	 . 13)
    (ideographic-structure
     ((name . "IDEOGRAPHIC DESCRIPTION CHARACTER LEFT TO RIGHT")
      (=ucs		  . #x2FF0)	; ⿰
      )
     ((=ucs@unicode	  . #x7CF8)	; &AJ1-02227;
      (=adobe-japan1-0	  . 02227)	; &AJ1-02227;
      )
     ((=ucs		  . #x2BA5E)	; 𫩞
      ))
    (=ucs@JP/hanazono	. #x30AE8)	; &RUI6-E09A;
    (=ruimoku-v6	. #xE09A)	; &RUI6-E09A;
    (->subsumptive
     ((==ucs@JP/hanazono  . #x30AE8)	; &g2-hanaJU+30AE8;
      )
     ((ideographic-structure
       ((name . "IDEOGRAPHIC DESCRIPTION CHARACTER LEFT TO RIGHT")
	(=ucs		    . #x2FF0)	; ⿰
	)
       ((name		    . "CJK RADICAL SILK")
	(=ucs		    . #x2EAF)	; ⺯
	)
       ((=ucs		    . #x2BA5E)	; 𫩞
	))
      (=>>ruimoku-v6	  . #xE09A)	; &G-RUI6-E09A;
      (->subsumptive
       ((==ruimoku-v6	    . #xE09A)	; &g2-RUI6-E09A;
	))
      ))
    ))
(define-char
  '((<-denotational
     ((=>ucs@cognate	  . #x2A7CA)	; &A-cgnU+2A7CA;
      ))
    (ideographic-radical . 29)	; ⼜
    (ideographic-strokes . 9)
    (total-strokes	 . 11)
    (ideographic-structure
     ((name . "IDEOGRAPHIC DESCRIPTION CHARACTER ABOVE TO BELOW")
      (=ucs		  . #x2FF1)	; ⿱
      )
     ((=big5-cdp	  . #x8DE0)	; &CDP-8DE0;
      )
     ((=ucs		  . #x5206)	; 分
      ))
    (=ucs		. #x2A7CA)	; 𪟊
    (=gt-k		. 08316)	; &I-GT-K08316;
    (=gt-pj-k1		. #x3650)	; 𪟊 [22-48]
    (=ruimoku-v6	. #xE09B)	; &I-RUI6-E09B;
    (->subsumptive
     ((==ucs@JP/hanazono  . #x2A7CA)	; &g2-hanaJU+2A7CA;
      )
     ((==gt-k		  . 08316)	; &g2-GT-K08316;
      (==gt-pj-k1	  . #x3650)	; &g2-GT-K08316; [22-48]
      )
     ((==ruimoku-v6	  . #xE09B)	; &g2-RUI6-E09B;
      ))
    ))
(define-char
  '((<-denotational
     ((=>ruimoku-v6	  . #xE09C)	; &A-RUI6-E09C;
      ))
    (ideographic-radical . 104)	; ⽧
    (ideographic-strokes . 15)
    (total-strokes	 . 20)
    (ideographic-structure
     ((name . "IDEOGRAPHIC DESCRIPTION CHARACTER SURROUND FROM UPPER LEFT")
      (=ucs		  . #x2FF8)	; ⿸
      )
     ((=ucs		  . #x7592)	; 疒
      )
     ((=ucs@unicode	  . #x66B4)	; &AJ1-03691;
      (=adobe-japan1-0	  . 03691)	; &AJ1-03691;
      ))
    (=ruimoku-v6	. #xE09C)	; &RUI6-E09C;
    ))
(define-char
  '((<-denotational
     ((=ucs		  . #x28615)	; 𨘕
      ))
    (ideographic-radical . 162)	; ⾡
    (ideographic-strokes . 14)
    (total-strokes	 . 17)
    (ideographic-structure
     ((name . "IDEOGRAPHIC DESCRIPTION CHARACTER SURROUND FROM LOWER LEFT")
      (=ucs		  . #x2FFA)	; ⿺
      )
     ((name		  . "CJK RADICAL SIMPLIFIED WALK")
      (=ucs		  . #x2ECC)	; ⻌
      )
     ((=ucs@unicode	  . #x8207)	; &AJ1-06299;
      (=adobe-japan1-0	  . 06299)	; &AJ1-06299;
      ))
    (=ucs@iso		. #x28615)	; &RUI6-E09D;
    (=ruimoku-v6	. #xE09D)	; &RUI6-E09D;
    (->subsumptive
     ((==ucs@iso	  . #x28615)	; &g2-IU-00028615;
      )
     ((==ruimoku-v6	  . #xE09D)	; &g2-RUI6-E09D;
      ))
    ))
(define-char
  '((ideographic-radical . 163)	; ⾢
    (ideographic-strokes . 5)
    (total-strokes	 . 8)
    (ideographic-structure
     ((name . "IDEOGRAPHIC DESCRIPTION CHARACTER LEFT TO RIGHT")
      (=ucs		  . #x2FF0)	; ⿰
      )
     ((=ucs		  . #x6209)	; 戉
      )
     ((name		  . "CJK RADICAL CITY")
      (=ucs		  . #x2ECF)	; ⻏
      ))
    (=ucs		. #x2B45B)	; 𫑛
    (=ruimoku-v6	. #xE09E)	; &I-RUI6-E09E;
    (->subsumptive
     ((==ucs@JP/hanazono  . #x2B45B)	; &g2-hanaJU+2B45B;
      (==ruimoku-v6	  . #xE09E)	; &g2-hanaJU+2B45B;
      (->subsumptive
       ((===ruimoku-v6	    . #xE09E)	; &R-RUI6-E09E;
	))
      ))
    ))
(define-char
  '((<-denotational
     ((=>ruimoku-v6	  . #xE09F)	; &A-RUI6-E09F;
      ))
    (ideographic-radical . 170)	; ⾩
    (ideographic-strokes . 4)
    (total-strokes	 . 7)
    (ideographic-structure
     ((name . "IDEOGRAPHIC DESCRIPTION CHARACTER LEFT TO RIGHT")
      (=ucs		  . #x2FF0)	; ⿰
      )
     ((name		  . "CJK RADICAL MOUND TWO")
      (=ucs		  . #x2ED6)	; ⻖
      )
     ((=ucs@unicode	  . #x5316)	; &AJ1-13665;
      (=adobe-japan1-4	  . 13665)	; &AJ1-13665;
      ))
    (=ruimoku-v6	. #xE09F)	; &RUI6-E09F;
    ))
(define-char
  '((<-denotational
     ((=>ruimoku-v6	  . #xE0A0)	; &A-RUI6-E0A0;
      ))
    (ideographic-radical . 38)	; ⼥
    (ideographic-strokes . 21)
    (total-strokes	 . 24)
    (ideographic-structure
     ((name . "IDEOGRAPHIC DESCRIPTION CHARACTER LEFT TO RIGHT")
      (=ucs		  . #x2FF0)	; ⿰
      )
     ((=ucs		  . #x5973)	; 女
      )
     ((=ucs@unicode	  . #x56C2)	; &AJ1-04450;
      (=adobe-japan1-0	  . 04450)	; &AJ1-04450;
      ))
    (=ruimoku-v6	. #xE0A0)	; &RUI6-E0A0;
    ))
(define-char
  '((ideographic-radical . 86)	; ⽕
    (ideographic-strokes . 3)
    (total-strokes	 . 7)
    (ideographic-structure
     ((name . "IDEOGRAPHIC DESCRIPTION CHARACTER ABOVE TO BELOW")
      (=ucs		  . #x2FF1)	; ⿱
      )
     ((=ucs		  . #x706B)	; 火
      )
     ((=ucs		  . #x53E3)	; 口
      ))
    (=ucs		. #x2C278)	; 𬉸
    (=ruimoku-v6	. #xE0A1)	; &I-RUI6-E0A1;
    (->subsumptive
     ((==ucs@JP/hanazono  . #x2C278)	; &g2-hanaJU+2C278;
      (==ruimoku-v6	  . #xE0A1)	; &g2-hanaJU+2C278;
      (->subsumptive
       ((===ruimoku-v6	    . #xE0A1)	; &R-RUI6-E0A1;
	))
      ))
    ))
(define-char
  '((=ruimoku-v6	. #xE0A2)	; &RUI6-E0A2;
    ))
(define-char
  '((=ruimoku-v6	. #xE0A3)	; &RUI6-E0A3;
    ))
(define-char
  '((=ruimoku-v6	. #xE0A4)	; &RUI6-E0A4;
    ))
(define-char
  '((=ruimoku-v6	. #xE0A5)	; &RUI6-E0A5;
    ))
(define-char
  '((=ruimoku-v6	. #xE0A6)	; &RUI6-E0A6;
    ))
(define-char
  '((=ruimoku-v6	. #xE0A7)	; &RUI6-E0A7;
    ))
(define-char
  '((<-denotational
     ((=ucs		  . #x2931E)	; 𩌞
      ))
    (ideographic-radical . 177)	; ⾰
    (ideographic-strokes . 10)
    (total-strokes	 . 19)
    (ideographic-structure
     ((name . "IDEOGRAPHIC DESCRIPTION CHARACTER LEFT TO RIGHT")
      (=ucs		  . #x2FF0)	; ⿰
      )
     ((=ucs@unicode	  . #x9769)	; &AJ1-01461;
      (=adobe-japan1-0	  . 01461)	; &AJ1-01461;
      )
     ((=ucs@iso		  . #x2332D)	; &C6-3842;
      (=cns11643-6	  . #x3842)	; &C6-3842; [24-34]
      (=daikanwa	  . 14295)	; &I-M-14295;
      ))
    (sources		ruimoku=A2009-10984-00015)
    (=ucs@iso		. #x2931E)	; &RUI6-E0A8;
    (=ruimoku-v6	. #xE0A8)	; &RUI6-E0A8;
    (->subsumptive
     ((==ucs@iso	  . #x2931E)	; &g2-IU-0002931E;
      (==ruimoku-v6	  . #xE0A8)	; &g2-IU-0002931E;
      (->subsumptive
       ((===ucs@iso	    . #x2931E)	; &R-U-0002931E;
	)
       ((===ruimoku-v6	    . #xE0A8)	; &R-RUI6-E0A8;
	))
      ))
    ))
(define-char
  '((=ruimoku-v6	. #xE0A9)	; &RUI6-E0A9;
    ))
(define-char
  '((ideographic-radical . 149)	; ⾔
    (ideographic-strokes . 12)
    (total-strokes	 . 19)
    (ideographic-structure
     ((name . "IDEOGRAPHIC DESCRIPTION CHARACTER LEFT TO RIGHT")
      (=ucs		  . #x2FF0)	; ⿰
      )
     ((=ucs		  . #x8A00)	; 言
      )
     ((=ucs-itaiji-002	  . #x45B5)	; &U-i002+45B5;
      ))
    (=ucs		. #x32077)	; 𲁷
    (=ruimoku-v6	. #xE0AA)	; &I-RUI6-E0AA;
    (->subsumptive
     ((==ucs@JP/hanazono  . #x32077)	; &g2-hanaJU+32077;
      (==ruimoku-v6	  . #xE0AA)	; &g2-hanaJU+32077;
      (->subsumptive
       ((===ruimoku-v6	    . #xE0AA)	; &R-RUI6-E0AA;
	))
      ))
    ))
(define-char
  '((=ruimoku-v6	. #xE0AB)	; &RUI6-E0AB;
    ))
(define-char
  '((<-denotational
     ((=ucs		  . #x30ED4)	; 𰻔
      ))
    (ideographic-radical . 162)	; ⾡
    (ideographic-strokes . 12)
    (total-strokes	 . 15)
    (ideographic-structure
     ((name . "IDEOGRAPHIC DESCRIPTION CHARACTER SURROUND FROM LOWER LEFT")
      (=ucs		  . #x2FFA)	; ⿺
      )
     ((name		  . "CJK RADICAL SIMPLIFIED WALK")
      (=ucs		  . #x2ECC)	; ⻌
      )
     ((ideographic-structure
       ((name . "IDEOGRAPHIC DESCRIPTION CHARACTER SURROUND FROM UPPER LEFT")
	(=ucs		    . #x2FF8)	; ⿸
	)
       ((=ucs		    . #x2D928)	; 𭤨
	)
       ((=ucs		    . #x7F8A)	; 羊
	))
      ))
    (=ruimoku-v6	. #xE0AC)	; &RUI6-E0AC;
    (->subsumptive
     ((==ruimoku-v6	  . #xE0AC)	; &g2-RUI6-E0AC;
      ))
    ))
(define-char
  '((<-denotational
     ((=>ruimoku-v6	  . #xE0AD)	; &A-RUI6-E0AD;
      ))
    (ideographic-radical . 145)	; ⾐
    (ideographic-strokes . 15)
    (total-strokes	 . 21)
    (ideographic-structure
     ((name . "IDEOGRAPHIC DESCRIPTION CHARACTER FULL SURROUND")
      (=ucs		  . #x2FF4)	; ⿴
      )
     ((=ucs@unicode	  . #x8863)	; &AJ1-01189;
      (=adobe-japan1-0	  . 01189)	; &AJ1-01189;
      )
     ((=ucs@unicode	  . #x885B)	; &AJ1-01268;
      (=adobe-japan1-0	  . 01268)	; &AJ1-01268;
      ))
    (=ruimoku-v6	. #xE0AD)	; &RUI6-E0AD;
    (->subsumptive
     ((==ruimoku-v6	  . #xE0AD)	; &g2-RUI6-E0AD;
      ))
    ))
(define-char
  '((<-denotational
     ((=ucs		  . #x30EE5)	; 𰻥
      ))
    (ideographic-radical . 163)	; ⾢
    (ideographic-strokes . 4)
    (total-strokes	 . 7)
    (ideographic-structure
     ((name . "IDEOGRAPHIC DESCRIPTION CHARACTER LEFT TO RIGHT")
      (=ucs		  . #x2FF0)	; ⿰
      )
     ((=ucs@unicode	  . #x5316)	; &AJ1-13665;
      (=adobe-japan1-4	  . 13665)	; &AJ1-13665;
      )
     ((name		  . "CJK RADICAL CITY")
      (=ucs		  . #x2ECF)	; ⻏
      ))
    (=ucs@JP/hanazono	. #x30EE5)	; &RUI6-E0AE;
    (=ruimoku-v6	. #xE0AE)	; &RUI6-E0AE;
    (->subsumptive
     ((==ucs@JP/hanazono  . #x30EE5)	; &g2-hanaJU+30EE5;
      (==ruimoku-v6	  . #xE0AE)	; &g2-hanaJU+30EE5;
      (->subsumptive
       ((===ruimoku-v6	    . #xE0AE)	; &R-RUI6-E0AE;
	))
      ))
    ))
(define-char
  '((ideographic-radical . 40)	; ⼧
    (ideographic-strokes . 7)
    (total-strokes	 . 10)
    (ideographic-structure
     ((name . "IDEOGRAPHIC DESCRIPTION CHARACTER ABOVE TO BELOW")
      (=ucs		  . #x2FF1)	; ⿱
      )
     ((=ucs		  . #x5B80)	; 宀
      )
     ((=ucs		  . #x9091)	; 邑
      ))
    (=ruimoku-v6	. #xE0AF)	; &RUI6-E0AF;
    (->subsumptive
     ((==ruimoku-v6	  . #xE0AF)	; &g2-RUI6-E0AF;
      ))
    ))
(define-char
  '((<-denotational
     ((=>ruimoku-v6	  . #xE0B0)	; &A-RUI6-E0B0;
      ))
    (ideographic-radical . 61)	; ⼼
    (ideographic-strokes . 7)
    (total-strokes	 . 11)
    (ideographic-structure
     ((name . "IDEOGRAPHIC DESCRIPTION CHARACTER ABOVE TO BELOW")
      (=ucs		  . #x2FF1)	; ⿱
      )
     ((=ucs-itaiji-001	  . #x2BB44)	; &U-i001+2BB44;
      )
     ((=ucs		  . #x5FC3)	; 心
      ))
    (=ruimoku-v6	. #xE0B0)	; &RUI6-E0B0;
    ))
(define-char
  '((<-denotational
     ((=>ruimoku-v6	  . #xE0B1)	; &A-RUI6-E0B1;
      ))
    (ideographic-radical . 162)	; ⾡
    (ideographic-strokes . 7)
    (total-strokes	 . 10)
    (ideographic-structure
     ((name . "IDEOGRAPHIC DESCRIPTION CHARACTER SURROUND FROM LOWER LEFT")
      (=ucs		  . #x2FFA)	; ⿺
      )
     ((name		  . "CJK RADICAL WALK ONE")
      (=ucs		  . #x2ECD)	; ⻍
      )
     ((=big5-cdp-var-001  . #x8DB6)	; &CDP-v001-8DB6;
      ))
    (=ruimoku-v6	. #xE0B1)	; &RUI6-E0B1;
    ))
(define-char
  '((<-denotational
     ((=>ruimoku-v6	  . #xE0B2)	; &A-RUI6-E0B2;
      ))
    (ideographic-radical . 185)	; ⾸
    (ideographic-strokes . 4)
    (total-strokes	 . 13)
    (ideographic-structure
     ((name . "IDEOGRAPHIC DESCRIPTION CHARACTER ABOVE TO BELOW")
      (=ucs		  . #x2FF1)	; ⿱
      )
     ((=ucs@unicode	  . #x37A2)	; &C3-2162;
      (=cns11643-3	  . #x2162)	; &C3-2162; [01-66]
      (=daikanwa	  . 07829)	; &I-M-07829;
      )
     ((=ucs@unicode	  . #x9996)	; &AJ1-02335;
      (=adobe-japan1-0	  . 02335)	; &AJ1-02335;
      ))
    (=ruimoku-v6	. #xE0B2)	; &RUI6-E0B2;
    ))
(define-char
  '((<-denotational
     ((=ucs		  . #x2D4FD)	; 𭓽
      ))
    (ideographic-radical . 40)	; ⼧
    (ideographic-strokes . 8)
    (total-strokes	 . 11)
    (ideographic-structure
     ((name . "IDEOGRAPHIC DESCRIPTION CHARACTER ABOVE TO BELOW")
      (=ucs		  . #x2FF1)	; ⿱
      )
     ((=ucs-itaiji-003	  . #x5B93)	; &U-i003+5B93;
      )
     ((=ucs		  . #x5C71)	; 山
      ))
    (sound@ja/kun	"sasa")
    (=ucs@JP/hanazono	. #x2D4FD)	; &RUI6-E0B3;
    (=koseki		. 552700)	; &RUI6-E0B3;
    (=gt		. 09052)	; &RUI6-E0B3;
    (=gt-pj-9		. #x3943)	; &RUI6-E0B3; [25-35]
    (=ruimoku-v6	. #xE0B3)	; &I-RUI6-E0B3;
    (<-formed
     ((=ucs		  . #x5BC6)	; 密
      ))
    (<-formed*sources
     koseki mj)
    (->subsumptive
     ((==ucs@JP/hanazono  . #x2D4FD)	; &g2-KOSEKI-552700;
      (==koseki		  . 552700)	; &g2-KOSEKI-552700;
      (==ruimoku-v6	  . #xE0B3)	; &g2-KOSEKI-552700;
      (->subsumptive
       ((===ruimoku-v6	    . #xE0B3)	; &R-RUI6-E0B3;
	))
      )
     ((=ruimoku-v6	  . #xE0BA)	; &RUI6-E0BA;
      (->subsumptive
       ((==ruimoku-v6	    . #xE0BA)	; &g2-RUI6-E0BA;
	))
      ))
    ))
(define-char
  '((<-denotational
     ((=>ruimoku-v6	  . #xE0B4)	; &A-RUI6-E0B4;
      ))
    (ideographic-radical . 73)	; ⽈
    (ideographic-strokes . 8)
    (total-strokes	 . 12)
    (ideographic-structure
     ((name . "IDEOGRAPHIC DESCRIPTION CHARACTER LEFT TO RIGHT")
      (=ucs		  . #x2FF0)	; ⿰
      )
     ((=mj		  . 013489)	; &MJ013489;
      )
     ((=ucs		  . #x6535)	; 攵
      ))
    (=ruimoku-v6	. #xE0B4)	; &RUI6-E0B4;
    (->subsumptive
     ((==ruimoku-v6	  . #xE0B4)	; &g2-RUI6-E0B4;
      ))
    ))
(define-char
  '((<-denotational
     ((=ucs		  . #x3054E)	; 𰕎
      ))
    (ideographic-radical . 66)	; ⽁
    (ideographic-strokes . 8)
    (total-strokes	 . 12)
    (ideographic-structure
     ((name . "IDEOGRAPHIC DESCRIPTION CHARACTER LEFT TO RIGHT")
      (=ucs		  . #x2FF0)	; ⿰
      )
     ((=ucs		  . #x2F982)	; 育
      )
     ((=ucs		  . #x6535)	; 攵
      ))
    (=ucs@JP/hanazono	. #x3054E)	; &RUI6-E0B5;
    (=cbeta		. 12172)	; &RUI6-E0B5;
    (=ruimoku-v6	. #xE0B5)	; &I-RUI6-E0B5;
    (->subsumptive
     ((==ucs@JP/hanazono  . #x3054E)	; &g2-hanaJU+3054E;
      (==ruimoku-v6	  . #xE0B5)	; &g2-hanaJU+3054E;
      (->subsumptive
       ((===ruimoku-v6	    . #xE0B5)	; &R-RUI6-E0B5;
	))
      )
     ((==cbeta		  . 12172)	; &g2-CB12172;
      ))
    ))
(define-char
  '((=ruimoku-v6	. #xE0B6)	; &RUI6-E0B6;
    ))
(define-char
  '((=ruimoku-v6	. #xE0B7)	; &RUI6-E0B7;
    ))
(define-char
  '((=ruimoku-v6	. #xE0B8)	; &RUI6-E0B8;
    ))
(define-char
  '((<-denotational
     ((=>ruimoku-v6	  . #xE0B9)	; &A-RUI6-E0B9;
      ))
    (ideographic-radical . 163)	; ⾢
    (ideographic-strokes . 6)
    (total-strokes	 . 9)
    (ideographic-structure
     ((name . "IDEOGRAPHIC DESCRIPTION CHARACTER LEFT TO RIGHT")
      (=ucs		  . #x2FF0)	; ⿰
      )
     ((=ucs@unicode	  . #x65E8)	; &AJ1-02219;
      (=adobe-japan1-0	  . 02219)	; &AJ1-02219;
      )
     ((name		  . "CJK RADICAL CITY")
      (=ucs		  . #x2ECF)	; ⻏
      ))
    (=ruimoku-v6	. #xE0B9)	; &RUI6-E0B9;
    (->subsumptive
     ((==ruimoku-v6	  . #xE0B9)	; &g2-RUI6-E0B9;
      ))
    ))
(define-char
  '((<-subsumptive
     ((=ucs@JP/hanazono	  . #x2D4FD)	; &RUI6-E0B3;
      (=gt		  . 09052)	; &RUI6-E0B3;
      ))
    (=ruimoku-v6	. #xE0BA)	; &RUI6-E0BA;
    (->subsumptive
     ((==ruimoku-v6	  . #xE0BA)	; &g2-RUI6-E0BA;
      ))
    ))
(define-char
  '((ideographic-radical . 70)	; ⽅
    (ideographic-strokes . 7)
    (total-strokes	 . 11)
    (ideographic-structure
     ((name . "IDEOGRAPHIC DESCRIPTION CHARACTER SURROUND FROM UPPER LEFT")
      (=ucs		  . #x2FF8)	; ⿸
      )
     ((=ucs		  . #x2D928)	; 𭤨
      )
     ((=ucs		  . #x53F2)	; 史
      ))
    (=ucs		. #x2C008)	; 𬀈
    (=ruimoku-v6	. #xE0BB)	; &I-RUI6-E0BB;
    (->subsumptive
     ((==ucs@JP/hanazono  . #x2C008)	; &g2-hanaJU+2C008;
      (==ruimoku-v6	  . #xE0BB)	; &g2-hanaJU+2C008;
      (->subsumptive
       ((===ruimoku-v6	    . #xE0BB)	; &R-RUI6-E0BB;
	))
      ))
    ))
(define-char
  '((<-denotational
     ((=ucs		  . #x2CB1A)	; 𬬚
      ))
    (ideographic-radical . 167)	; ⾦
    (ideographic-strokes . 15)
    (total-strokes	 . 23)
    (ideographic-structure
     ((name . "IDEOGRAPHIC DESCRIPTION CHARACTER LEFT TO RIGHT")
      (=ucs		  . #x2FF0)	; ⿰
      )
     ((=ucs@unicode	  . #x91D1)	; &AJ1-01754;
      (=adobe-japan1-0	  . 01754)	; &AJ1-01754;
      )
     ((=ucs@unicode	  . #x819A)	; &AJ1-03544;
      (=adobe-japan1-0	  . 03544)	; &AJ1-03544;
      ))
    (=ucs@JP/hanazono	. #x2CB1A)	; &RUI6-E0BC;
    (=ruimoku-v6	. #xE0BC)	; &RUI6-E0BC;
    (->subsumptive
     ((==ucs@JP/hanazono  . #x2CB1A)	; &g2-hanaJU+2CB1A;
      (==ruimoku-v6	  . #xE0BC)	; &g2-hanaJU+2CB1A;
      (->subsumptive
       ((===ruimoku-v6	    . #xE0BC)	; &R-RUI6-E0BC;
	))
      ))
    ))
(define-char
  '((ideographic-radical . 31)	; ⼞
    (ideographic-strokes . 5)
    (total-strokes	 . 8)
    (ideographic-structure
     ((name . "IDEOGRAPHIC DESCRIPTION CHARACTER FULL SURROUND")
      (=ucs		  . #x2FF4)	; ⿴
      )
     ((=ucs		  . #x56D7)	; 囗
      )
     ((=ucs		  . #x5360)	; 占
      ))
    (=ucs		. #x3024F)	; 𰉏
    (=ruimoku-v6	. #xE0BD)	; &I-RUI6-E0BD;
    (->subsumptive
     ((==ucs@JP/hanazono  . #x3024F)	; &g2-hanaJU+3024F;
      (==ruimoku-v6	  . #xE0BD)	; &g2-hanaJU+3024F;
      (->subsumptive
       ((===ruimoku-v6	    . #xE0BD)	; &R-RUI6-E0BD;
	))
      ))
    ))
(define-char
  '((<-denotational
     ((=>ucs@iso	  . #x21249)	; &A-IU+21249;
      ))
    (ideographic-radical . 32)	; ⼟
    (ideographic-strokes . 3)
    (total-strokes	 . 6)
    (ideographic-structure
     ((name . "IDEOGRAPHIC DESCRIPTION CHARACTER ABOVE TO BELOW")
      (=ucs		  . #x2FF1)	; ⿱
      )
     ((=ucs		  . #x58EB)	; 士
      )
     ((=ucs		  . #x2053C)	; 𠔼
      ))
    (=ruimoku-v6	. #xE0BE)	; &RUI6-E0BE;
    (=ucs-var-001	. #x21249)	; &RUI6-E0BE;
    (->subsumptive
     ((==ruimoku-v6	  . #xE0BE)	; &g2-U-v001+21249;
      (==ucs-var-001	  . #x21249)	; &g2-U-v001+21249;
      (->subsumptive
       ((===ruimoku-v6	    . #xE0BE)	; &R-RUI6-E0BE;
	))
      ))
    ))
(define-char
  '((<-denotational
     ((=>ruimoku-v6	  . #xE0BF)	; &A-RUI6-E0BF;
      ))
    (ideographic-radical . 213)	; ⿔
    (ideographic-strokes . 5)
    (total-strokes	 . 12)
    (ideographic-structure
     ((name . "IDEOGRAPHIC DESCRIPTION CHARACTER ABOVE TO BELOW")
      (=ucs		  . #x2FF1)	; ⿱
      )
     ((=ucs@unicode	  . #x9F9F)	; &G0-396A;
      (=gb2312		  . #x396A)	; &G0-396A; [25-74]
      )
     ((=ucs		  . #x7518)	; 甘
      ))
    (=ruimoku-v6	. #xE0BF)	; &RUI6-E0BF;
    (->subsumptive
     ((==ruimoku-v6	  . #xE0BF)	; &g2-RUI6-E0BF;
      ))
    ))
(define-char
  '((=ruimoku-v6	. #xE0C0)	; &RUI6-E0C0;
    ))
(define-char
  '((=ruimoku-v6	. #xE0C1)	; &RUI6-E0C1;
    ))
(define-char
  '((<-denotational
     ((=ucs		  . #x2C6E7)	; 𬛧
      ))
    (ideographic-radical . 131)	; ⾂
    (ideographic-strokes . 9)
    (total-strokes	 . 15)
    (ideographic-structure
     ((name . "IDEOGRAPHIC DESCRIPTION CHARACTER LEFT TO RIGHT")
      (=ucs		  . #x2FF0)	; ⿰
      )
     ((=ucs@JP/hanazono	  . #x2B1C6)	; &RUI6-E088;
      (=ruimoku-v6	  . #xE088)	; &RUI6-E088;
      )
     ((=ucs		  . #x5B50)	; 子
      ))
    (=ucs@JP/hanazono	. #x2C6E7)	; &RUI6-E0C2;
    (=ruimoku-v6	. #xE0C2)	; &RUI6-E0C2;
    (->subsumptive
     ((==ucs@JP/hanazono  . #x2C6E7)	; &g2-hanaJU+2C6E7;
      (==ruimoku-v6	  . #xE0C2)	; &g2-hanaJU+2C6E7;
      (->subsumptive
       ((===ruimoku-v6	    . #xE0C2)	; &R-RUI6-E0C2;
	))
      ))
    ))
(define-char
  '((<-denotational
     ((=>ruimoku-v6	  . #xE0C3)	; &A-RUI6-E0C3;
      ))
    (ideographic-radical . 141)	; ⾌
    (ideographic-strokes . 4)
    (total-strokes	 . 12)
    (ideographic-structure
     ((name . "IDEOGRAPHIC DESCRIPTION CHARACTER LEFT TO RIGHT")
      (=ucs		  . #x2FF0)	; ⿰
      )
     ((=ucs@unicode	  . #x864E)	; &G0-3B22;
      (=gb2312		  . #x3B22)	; &G0-3B22; [27-02]
      )
     ((=ucs		  . #x65A4)	; 斤
      ))
    (=ruimoku-v6	. #xE0C3)	; &RUI6-E0C3;
    ))
(define-char
  '((<-denotational
     ((=>ruimoku-v6	  . #xE0C4)	; &A-RUI6-E0C4;
      ))
    (ideographic-radical . 38)	; ⼥
    (ideographic-strokes . 7)
    (total-strokes	 . 10)
    (ideographic-structure
     ((name . "IDEOGRAPHIC DESCRIPTION CHARACTER LEFT TO RIGHT")
      (=ucs		  . #x2FF0)	; ⿰
      )
     ((=ucs		  . #x5973)	; 女
      )
     ((=ucs@unicode	  . #x342C)	; &AJ1-17246;
      (=adobe-japan1-5	  . 17246)	; &AJ1-17246;
      ))
    (=ruimoku-v6	. #xE0C4)	; &RUI6-E0C4;
    (->subsumptive
     ((==ruimoku-v6	  . #xE0C4)	; &g2-RUI6-E0C4;
      ))
    ))
(define-char
  '((ideographic-radical . 59)	; ⼺
    (ideographic-strokes . 7)
    (total-strokes	 . 10)
    (ideographic-structure
     ((name . "IDEOGRAPHIC DESCRIPTION CHARACTER LEFT TO RIGHT")
      (=ucs		  . #x2FF0)	; ⿰
      )
     ((=ucs		  . #x9149)	; 酉
      )
     ((=ucs		  . #x5F61)	; 彡
      ))
    (=ucs		. #x2BE49)	; 𫹉
    (=ruimoku-v6	. #xE0C5)	; &I-RUI6-E0C5;
    (->subsumptive
     ((==ucs@JP/hanazono  . #x2BE49)	; &g2-hanaJU+2BE49;
      (==ruimoku-v6	  . #xE0C5)	; &g2-hanaJU+2BE49;
      (->subsumptive
       ((===ruimoku-v6	    . #xE0C5)	; &R-RUI6-E0C5;
	))
      ))
    ))
(define-char
  '((=ruimoku-v6	. #xE0C6)	; &RUI6-E0C6;
    ))
(define-char
  '((=ruimoku-v6	. #xE0C7)	; &RUI6-E0C7;
    ))
(define-char
  '((=ruimoku-v6	. #xE0C8)	; &RUI6-E0C8;
    ))
(define-char
  '((=ruimoku-v6	. #xE0C9)	; &RUI6-E0C9;
    ))
(define-char
  '((=ruimoku-v6	. #xE0CA)	; &RUI6-E0CA;
    ))
(define-char
  '((=ruimoku-v6	. #xE0CB)	; &RUI6-E0CB;
    ))
(define-char
  '((ideographic-radical . 9)	; ⼈
    (ideographic-strokes . 7)
    (total-strokes	 . 9)
    (ideographic-structure
     ((name . "IDEOGRAPHIC DESCRIPTION CHARACTER LEFT TO RIGHT")
      (=ucs		  . #x2FF0)	; ⿰
      )
     ((=ucs		  . #x4EBB)	; 亻
      )
     ((ideographic-structure
       ((name . "IDEOGRAPHIC DESCRIPTION CHARACTER ABOVE TO BELOW")
	(=ucs		    . #x2FF1)	; ⿱
	)
       ((=ucs		    . #x53B6)	; 厶
	)
       ((=ucs		    . #x53F8)	; 司
	))
      ))
    (=ruimoku-v6	. #xE0CC)	; &RUI6-E0CC;
    ))
(define-char
  '((<-denotational
     ((=ucs		  . #x2C6AA)	; 𬚪
      ))
    (ideographic-radical . 129)	; ⾀
    (ideographic-strokes . 2)
    (total-strokes	 . 6)
    (ideographic-structure
     ((name . "IDEOGRAPHIC DESCRIPTION CHARACTER ABOVE TO BELOW")
      (=ucs		  . #x2FF1)	; ⿱
      )
     ((=ucs@unicode	  . #x807F)	; &AJ1-06227;
      (=adobe-japan1-0	  . 06227)	; &AJ1-06227;
      )
     ((=ucs		  . #x4E42)	; 乂
      ))
    (=ruimoku-v6	. #xE0CD)	; &RUI6-E0CD;
    ))
(define-char
  '((=ruimoku-v6	. #xE0CE)	; &RUI6-E0CE;
    ))
(define-char
  '((=ruimoku-v6	. #xE0CF)	; &RUI6-E0CF;
    ))
(define-char
  '((=ruimoku-v6	. #xE0D0)	; &RUI6-E0D0;
    ))
(define-char
  '((ideographic-radical . 31)	; ⼞
    (ideographic-strokes . 5)
    (total-strokes	 . 8)
    (ideographic-structure
     ((name . "IDEOGRAPHIC DESCRIPTION CHARACTER FULL SURROUND")
      (=ucs		  . #x2FF4)	; ⿴
      )
     ((=ucs		  . #x56D7)	; 囗
      )
     ((=ucs		  . #x767D)	; 白
      ))
    (=ruimoku-v6	. #xE0D1)	; &RUI6-E0D1;
    (->subsumptive
     ((==ruimoku-v6	  . #xE0D1)	; &g2-RUI6-E0D1;
      ))
    ))
(define-char
  '((=ruimoku-v6	. #xE0D2)	; &RUI6-E0D2;
    ))
(define-char
  '((<-denotational
     ((=ucs		  . #x31035)	; 𱀵
      ))
    (ideographic-radical . 170)	; ⾩
    (ideographic-strokes . 12)
    (total-strokes	 . 15)
    (ideographic-structure
     ((name . "IDEOGRAPHIC DESCRIPTION CHARACTER LEFT TO RIGHT")
      (=ucs		  . #x2FF0)	; ⿰
      )
     ((name		  . "CJK RADICAL MOUND TWO")
      (=ucs		  . #x2ED6)	; ⻖
      )
     ((=ucs@ks		  . #x5C0A)	; &MJ010275;
      (=mj		  . 010275)	; &MJ010275;
      ))
    (=ucs@JP/hanazono	. #x31035)	; &RUI6-E0D3;
    (=ruimoku-v6	. #xE0D3)	; &RUI6-E0D3;
    (->subsumptive
     ((==ucs@JP/hanazono  . #x31035)	; &g2-hanaJU+31035;
      )
     ((==ruimoku-v6	  . #xE0D3)	; &g2-RUI6-E0D3;
      ))
    ))
(define-char
  '((<-denotational
     ((=>ruimoku-v6	  . #xE0D4)	; &A-RUI6-E0D4;
      ))
    (ideographic-radical . 168)	; ⾧
    (ideographic-strokes . 8)
    (total-strokes	 . 16)
    (ideographic-structure
     ((name . "IDEOGRAPHIC DESCRIPTION CHARACTER ABOVE TO BELOW")
      (=ucs		  . #x2FF1)	; ⿱
      )
     ((=gt-k		  . 05014)	; &GT-K05014;
      )
     ((=ucs		  . #x9577)	; 長
      ))
    (=ruimoku-v6	. #xE0D4)	; &RUI6-E0D4;
    ))
(define-char
  '((<-denotational
     ((=>ruimoku-v6	  . #xE0D5)	; &A-RUI6-E0D5;
      ))
    (ideographic-radical . 122)	; ⽹
    (ideographic-strokes . 17)
    (total-strokes	 . 22)
    (ideographic-structure
     ((name . "IDEOGRAPHIC DESCRIPTION CHARACTER SURROUND FROM UPPER LEFT")
      (=ucs		  . #x2FF8)	; ⿸
      )
     ((=ucs@unicode	  . #x7F75)	; &AJ1-03331;
      (=adobe-japan1-0	  . 03331)	; &AJ1-03331;
      )
     ((=ucs		  . #x5442)	; 呂
      ))
    (=ruimoku-v6	. #xE0D5)	; &RUI6-E0D5;
    ))
(define-char
  '((<-denotational
     ((=>ruimoku-v6	  . #xE0D6)	; &A-RUI6-E0D6;
      ))
    (ideographic-radical . 193)	; ⿀
    (ideographic-strokes . 9)
    (total-strokes	 . 19)
    (ideographic-structure
     ((name . "IDEOGRAPHIC DESCRIPTION CHARACTER ABOVE TO MIDDLE AND BELOW")
      (=ucs		  . #x2FF3)	; ⿳
      )
     ((=ucs		  . #x5927)	; 大
      )
     ((=ucs		  . #x821B)	; 舛
      )
     ((=ucs@jis		  . #x9B32)	; &AJ1-07300;
      (=adobe-japan1-0	  . 07300)	; &AJ1-07300;
      ))
    (=ruimoku-v6	. #xE0D6)	; &RUI6-E0D6;
    ))
(define-char
  '((ideographic-radical . 1)	; ⼀
    (ideographic-strokes . 10)
    (total-strokes	 . 11)
    (ideographic-structure
     ((name . "IDEOGRAPHIC DESCRIPTION CHARACTER LEFT TO RIGHT")
      (=ucs		  . #x2FF0)	; ⿰
      )
     ((=ucs		  . #x3401)	; 㐁
      )
     ((=ucs		  . #x4E16)	; 世
      ))
    (=ruimoku-v6	. #xE0D7)	; &RUI6-E0D7;
    ))
(define-char
  '((<-denotational
     ((=>ruimoku-v6	  . #xE0D8)	; &A-RUI6-E0D8;
      ))
    (ideographic-radical . 77)	; ⽌
    (ideographic-strokes . 6)
    (total-strokes	 . 10)
    (ideographic-structure
     ((name . "IDEOGRAPHIC DESCRIPTION CHARACTER LEFT TO RIGHT")
      (=ucs		  . #x2FF0)	; ⿰
      )
     ((=ucs		  . #x3401)	; 㐁
      )
     ((=ucs@unicode	  . #x6B62)	; &MJ014823;
      (=mj		  . 014823)	; &MJ014823;
      ))
    (=ruimoku-v6	. #xE0D8)	; &RUI6-E0D8;
    ))
(define-char
  '((<-denotational
     ((=>ruimoku-v6	  . #xE0D9)	; &A-RUI6-E0D9;
      ))
    (ideographic-radical . 29)	; ⼜
    (ideographic-strokes . 8)
    (total-strokes	 . 10)
    (ideographic-structure
     ((name . "IDEOGRAPHIC DESCRIPTION CHARACTER LEFT TO RIGHT")
      (=ucs		  . #x2FF0)	; ⿰
      )
     ((=ucs		  . #x3401)	; 㐁
      )
     ((=ucs@unicode	  . #x53CD)	; &AJ1-03411;
      (=adobe-japan1-0	  . 03411)	; &AJ1-03411;
      ))
    (=ruimoku-v6	. #xE0D9)	; &RUI6-E0D9;
    ))
(define-char
  '((=ruimoku-v6	. #xE0DA)	; &RUI6-E0DA;
    ))
(define-char
  '((=ruimoku-v6	. #xE0DB)	; &RUI6-E0DB;
    ))
(define-char
  '((=ruimoku-v6	. #xE0DC)	; &RUI6-E0DC;
    ))
(define-char
  '((=ruimoku-v6	. #xE0DD)	; &RUI6-E0DD;
    ))
(define-char
  '((=ruimoku-v6	. #xE0DE)	; &RUI6-E0DE;
    ))
(define-char
  '((=ruimoku-v6	. #xE0DF)	; &RUI6-E0DF;
    ))
(define-char
  '((ideographic-radical . 125)	; ⽼
    (ideographic-strokes . 5)
    (total-strokes	 . 9)
    (ideographic-structure
     ((name . "IDEOGRAPHIC DESCRIPTION CHARACTER ABOVE TO BELOW")
      (=ucs		  . #x2FF1)	; ⿱
      )
     ((=ucs		  . #x8002)	; 耂
      )
     ((=ucs		  . #x53E4)	; 古
      ))
    (=ucs		. #x2C68B)	; 𬚋
    (=ruimoku-v6	. #xE0E0)	; &I-RUI6-E0E0;
    (->subsumptive
     ((==ucs@JP/hanazono  . #x2C68B)	; &g2-hanaJU+2C68B;
      )
     ((==ruimoku-v6	  . #xE0E0)	; &g2-RUI6-E0E0;
      ))
    ))
(define-char
  '((=ruimoku-v6	. #xE0E1)	; &RUI6-E0E1;
    ))
(define-char
  '((<-denotational
     ((=>ruimoku-v6	  . #xE0E2)	; &A-RUI6-E0E2;
      ))
    (ideographic-radical . 141)	; ⾌
    (ideographic-strokes . 4)
    (total-strokes	 . 10)
    (ideographic-structure
     ((name . "IDEOGRAPHIC DESCRIPTION CHARACTER SURROUND FROM LOWER LEFT")
      (=ucs		  . #x2FFA)	; ⿺
      )
     ((=ucs@cns		  . #x864E)	; &MJ023272;
      (=ucs@jis		  . #x864E)	; &MJ023272;
      (=ucs@ks		  . #x864E)	; &MJ023272;
      (=mj		  . 023272)	; &MJ023272;
      )
     ((=ucs		  . #x529B)	; 力
      ))
    (=ruimoku-v6	. #xE0E2)	; &RUI6-E0E2;
    ))
(define-char
  '((<-denotational
     ((=ucs		  . #x2AFD6)	; 𪿖
      ))
    (ideographic-radical . 112)	; ⽯
    (ideographic-strokes . 5)
    (total-strokes	 . 10)
    (ideographic-structure
     ((name . "IDEOGRAPHIC DESCRIPTION CHARACTER LEFT TO RIGHT")
      (=ucs		  . #x2FF0)	; ⿰
      )
     ((=ucs@unicode	  . #x77F3)	; &AJ1-02676;
      (=adobe-japan1-0	  . 02676)	; &AJ1-02676;
      )
     ((=ucs@unicode	  . #x7384)	; &AJ1-01904;
      (=adobe-japan1-0	  . 01904)	; &AJ1-01904;
      ))
    (=ucs@JP/hanazono	. #x2AFD6)	; &RUI6-E0E3;
    (=ruimoku-v6	. #xE0E3)	; &RUI6-E0E3;
    (->subsumptive
     ((==ucs@JP/hanazono  . #x2AFD6)	; &g2-hanaJU+2AFD6;
      (==ruimoku-v6	  . #xE0E3)	; &g2-hanaJU+2AFD6;
      (->subsumptive
       ((===ruimoku-v6	    . #xE0E3)	; &R-RUI6-E0E3;
	))
      ))
    ))
(define-char
  '((<-denotational
     ((=ucs		  . #x2BAA6)	; 𫪦
      ))
    (ideographic-radical . 30)	; ⼝
    (ideographic-strokes . 8)
    (total-strokes	 . 11)
    (ideographic-structure
     ((name . "IDEOGRAPHIC DESCRIPTION CHARACTER LEFT TO RIGHT")
      (=ucs		  . #x2FF0)	; ⿰
      )
     ((=ucs		  . #x53E3)	; 口
      )
     ((=ucs@unicode	  . #x77E5)	; &AJ1-02956;
      (=adobe-japan1-0	  . 02956)	; &AJ1-02956;
      ))
    (=ucs@JP/hanazono	. #x2BAA6)	; &RUI6-E0E4;
    (=cbeta		. 02913)	; &RUI6-E0E4;
    (=ruimoku-v6	. #xE0E4)	; &I-RUI6-E0E4;
    (->subsumptive
     ((==ucs@JP/hanazono  . #x2BAA6)	; &g2-CB02913;
      (==cbeta		  . 02913)	; &g2-CB02913;
      (==ruimoku-v6	  . #xE0E4)	; &g2-CB02913;
      (->subsumptive
       ((===ruimoku-v6	    . #xE0E4)	; &R-RUI6-E0E4;
	)
       ((===cbeta	    . 02913)	; &R-CB02913;
	))
      ))
    ))
(define-char
  '((<-denotational
     ((=>ruimoku-v6	  . #xE0E5)	; &A-RUI6-E0E5;
      ))
    (ideographic-radical . 134)	; ⾅
    (ideographic-strokes . 4)
    (total-strokes	 . 10)
    (ideographic-structure
     ((name . "IDEOGRAPHIC DESCRIPTION CHARACTER ABOVE TO BELOW")
      (=ucs		  . #x2FF1)	; ⿱
      )
     ((=ucs@unicode	  . #x81FC)	; &MJ021316;
      (=mj		  . 021316)	; &MJ021316;
      )
     ((=ucs		  . #x6728)	; 木
      ))
    (=ruimoku-v6	. #xE0E5)	; &RUI6-E0E5;
    (->subsumptive
     ((==ruimoku-v6	  . #xE0E5)	; &g2-RUI6-E0E5;
      ))
    ))
(define-char
  '((<-denotational
     ((=>ruimoku-v6	  . #xE0E6)	; &A-RUI6-E0E6;
      ))
    (ideographic-radical . 181)	; ⾴
    (ideographic-strokes . 1)
    (total-strokes	 . 10)
    (ideographic-structure
     ((name . "IDEOGRAPHIC DESCRIPTION CHARACTER LEFT TO RIGHT")
      (=ucs		  . #x2FF0)	; ⿰
      )
     ((=ucs		  . #x4E28)	; 丨
      )
     ((=ucs		  . #x9801)	; 頁
      ))
    (=ruimoku-v6	. #xE0E6)	; &RUI6-E0E6;
    ))
(define-char
  '((<-denotational
     ((=ucs		  . #x30804)	; 𰠄
      ))
    (ideographic-radical . 87)	; ⽖
    (ideographic-strokes . 8)
    (total-strokes	 . 12)
    (ideographic-structure
     ((name . "IDEOGRAPHIC DESCRIPTION CHARACTER ABOVE TO BELOW")
      (=ucs		  . #x2FF1)	; ⿱
      )
     ((name		  . "CJK RADICAL PAW ONE")
      (=ucs		  . #x2EA4)	; ⺤
      )
     ((=ucs@unicode	  . #x5352)	; &AJ1-02836;
      (=adobe-japan1-0	  . 02836)	; &AJ1-02836;
      ))
    (=ruimoku-v6	. #xE0E7)	; &RUI6-E0E7;
    ))
(define-char
  '((=ruimoku-v6	. #xE0E8)	; &RUI6-E0E8;
    ))
(define-char
  '((=ruimoku-v6	. #xE0E9)	; &RUI6-E0E9;
    ))
(define-char
  '((=ruimoku-v6	. #xE0EA)	; &RUI6-E0EA;
    ))
(define-char
  '((ideographic-radical . 44)	; ⼫
    (ideographic-strokes . 5)
    (total-strokes	 . 8)
    (ideographic-structure
     ((name . "IDEOGRAPHIC DESCRIPTION CHARACTER SURROUND FROM UPPER LEFT")
      (=ucs		  . #x2FF8)	; ⿸
      )
     ((=ucs		  . #x5C38)	; 尸
      )
     ((=ucs		  . #x793A)	; 示
      ))
    (=ucs		. #x2BD56)	; 𫵖
    (=big5-cdp		. #x8D70)	; &I-CDP-8D70;
    (=gt-k		. 04469)	; &I-GT-K04469;
    (=gt-pj-k1		. #x4261)	; 𫵖 [34-65]
    (=ruimoku-v6	. #xE0EB)	; &I-RUI6-E0EB;
    (->subsumptive
     ((==ucs@JP/hanazono  . #x2BD56)	; &g2-CDP-8D70;
      (==big5-cdp	  . #x8D70)	; &g2-CDP-8D70;
      (==gt-k		  . 04469)	; &g2-CDP-8D70;
      (==gt-pj-k1	  . #x4261)	; &g2-CDP-8D70; [34-65]
      (==ruimoku-v6	  . #xE0EB)	; &g2-CDP-8D70;
      (->subsumptive
       ((===big5-cdp	    . #x8D70)	; &R-CDP-8D70;
	)
       ((===gt-k	    . 04469)	; &R-GT-K04469;
	)
       ((===ruimoku-v6	    . #xE0EB)	; &R-RUI6-E0EB;
	))
      ))
    ))
(define-char
  '((<-denotational
     ((=ucs		  . #x313BC)	; 𱎼
      ))
    (ideographic-radical . 9)	; ⼈
    (ideographic-strokes . 9)
    (total-strokes	 . 11)
    (ideographic-structure
     ((name . "IDEOGRAPHIC DESCRIPTION CHARACTER LEFT TO RIGHT")
      (=ucs		  . #x2FF0)	; ⿰
      )
     ((=ucs		  . #x4EBB)	; 亻
      )
     ((=ucs@unicode	  . #x9769)	; &AJ1-01461;
      (=adobe-japan1-0	  . 01461)	; &AJ1-01461;
      ))
    (=ucs@JP/hanazono	. #x313BC)	; &RUI6-E0EC;
    (=ruimoku-v6	. #xE0EC)	; &RUI6-E0EC;
    (->subsumptive
     ((==ucs@JP/hanazono  . #x313BC)	; &g2-hanaJU+313BC;
      (==ruimoku-v6	  . #xE0EC)	; &g2-hanaJU+313BC;
      (->subsumptive
       ((===ruimoku-v6	    . #xE0EC)	; &R-RUI6-E0EC;
	))
      ))
    ))
(define-char
  '((=ruimoku-v6	. #xE0ED)	; &RUI6-E0ED;
    ))
(define-char
  '((<-subsumptive
     ((==ucs@JP/hanazono  . #x2C41E)	; &g2-hanaJU+2C41E;
      (==ruimoku-v6	  . #xE04F)	; &g2-hanaJU+2C41E;
      ))
    (=ruimoku-v6	. #xE0EE)	; &RUI6-E0EE;
    (->subsumptive
     ((==ruimoku-v6	  . #xE0EE)	; &g2-RUI6-E0EE;
      ))
    ))
(define-char
  '((<-denotational
     ((=ucs		  . #x2AE95)	; 𪺕
      ))
    (ideographic-radical . 87)	; ⽖
    (ideographic-strokes . 9)
    (total-strokes	 . 13)
    (ideographic-structure
     ((name . "IDEOGRAPHIC DESCRIPTION CHARACTER ABOVE TO BELOW")
      (=ucs		  . #x2FF1)	; ⿱
      )
     ((name		  . "CJK RADICAL PAW ONE")
      (=ucs		  . #x2EA4)	; ⺤
      )
     ((=ucs-itaiji-001	  . #x221C7)	; &U-i001+221C7;
      ))
    (=ruimoku-v6	. #xE0EF)	; &RUI6-E0EF;
    (->subsumptive
     ((==ruimoku-v6	  . #xE0EF)	; &g2-RUI6-E0EF;
      ))
    ))
(define-char
  '((=ruimoku-v6	. #xE0F0)	; &RUI6-E0F0;
    ))
(define-char
  '((<-denotational
     ((=>ruimoku-v6	  . #xE0F1)	; &A-RUI6-E0F1;
      ))
    (ideographic-radical . 154)	; ⾙
    (ideographic-strokes . 15)
    (total-strokes	 . 22)
    (ideographic-structure
     ((name . "IDEOGRAPHIC DESCRIPTION CHARACTER ABOVE TO BELOW")
      (=ucs		  . #x2FF1)	; ⿱
      )
     ((=ucs		  . #x8CCF)	; 賏
      )
     ((=ucs@unicode	  . #x342D)	; &C4-2534;
      (=cns11643-4	  . #x2534)	; &C4-2534; [05-20]
      (=daikanwa	  . 00300)	; &I-M-00300;
      ))
    (=ruimoku-v6	. #xE0F1)	; &RUI6-E0F1;
    ))
(define-char
  '((ideographic-radical . 137)	; ⾈
    (ideographic-strokes . 5)
    (total-strokes	 . 11)
    (ideographic-structure
     ((name . "IDEOGRAPHIC DESCRIPTION CHARACTER LEFT TO RIGHT")
      (=ucs		  . #x2FF0)	; ⿰
      )
     ((=ucs		  . #x821F)	; 舟
      )
     ((=ucs		  . #x4E3B)	; 主
      ))
    (=ruimoku-v6	. #xE0F2)	; &RUI6-E0F2;
    ))
(define-char
  '((ideographic-radical . 29)	; ⼜
    (ideographic-strokes . 5)
    (total-strokes	 . 7)
    (ideographic-structure
     ((name . "IDEOGRAPHIC DESCRIPTION CHARACTER ABOVE TO BELOW")
      (=ucs		  . #x2FF1)	; ⿱
      )
     ((=ucs		  . #x620A)	; 戊
      )
     ((=ucs		  . #x53C8)	; 又
      ))
    (=ucs		. #x2BA35)	; 𫨵
    (=ruimoku-v6	. #xE0F3)	; &I-RUI6-E0F3;
    (->subsumptive
     ((==ucs@JP/hanazono  . #x2BA35)	; &g2-hanaJU+2BA35;
      (==ruimoku-v6	  . #xE0F3)	; &g2-hanaJU+2BA35;
      (->subsumptive
       ((===ruimoku-v6	    . #xE0F3)	; &R-RUI6-E0F3;
	))
      ))
    ))
(define-char
  '((=ruimoku-v6	. #xE0F4)	; &RUI6-E0F4;
    ))
(define-char
  '((=ruimoku-v6	. #xE0F5)	; &RUI6-E0F5;
    ))
(define-char
  '((<-denotational
     ((=>ucs@iwds-1	  . #x2BE78)	; &A-IWDSU+2BE78;
      ))
    (ideographic-radical . 61)	; ⼼
    (ideographic-strokes . 4)
    (total-strokes	 . 8)
    (ideographic-structure
     ((name . "IDEOGRAPHIC DESCRIPTION CHARACTER SURROUND FROM UPPER RIGHT")
      (=ucs		  . #x2FF9)	; ⿹
      )
     ((=ucs-itaiji-001	  . #x53F8)	; &U-i001+53F8;
      )
     ((=ucs		  . #x5FC3)	; 心
      ))
    (=ucs		. #x2BE78)	; 𫹸
    (=ruimoku-v6	. #xE0F6)	; &I-RUI6-E0F6;
    (->subsumptive
     ((==ruimoku-v6	  . #xE0F6)	; &g2-RUI6-E0F6;
      )
     ((==ucs@JP/hanazono  . #x2BE78)	; &g2-hanaJU+2BE78;
      ))
    ))
(define-char
  '((=ruimoku-v6	. #xE0F7)	; &RUI6-E0F7;
    ))
(define-char
  '((=ruimoku-v6	. #xE0F8)	; &RUI6-E0F8;
    ))
(define-char
  '((=ruimoku-v6	. #xE0F9)	; &RUI6-E0F9;
    ))
(define-char
  '((ideographic-radical . 193)	; ⿀
    (ideographic-strokes . 10)
    (total-strokes	 . 20)
    (ideographic-structure
     ((name . "IDEOGRAPHIC DESCRIPTION CHARACTER ABOVE TO MIDDLE AND BELOW")
      (=ucs		  . #x2FF3)	; ⿳
      )
     ((=ucs		  . #x592D)	; 夭
      )
     ((=ucs		  . #x821B)	; 舛
      )
     ((=ucs		  . #x9B32)	; 鬲
      ))
    (=ruimoku-v6	. #xE0FA)	; &RUI6-E0FA;
    ))
(define-char
  '((=ruimoku-v6	. #xE0FB)	; &RUI6-E0FB;
    ))
(define-char
  '((=ruimoku-v6	. #xE0FC)	; &RUI6-E0FC;
    ))
(define-char
  '((=ruimoku-v6	. #xE0FD)	; &RUI6-E0FD;
    ))
(define-char
  '((<-denotational
     ((=>ruimoku-v6	  . #xE0FE)	; &A-RUI6-E0FE;
      ))
    (ideographic-radical . 108)	; ⽫
    (ideographic-strokes . 13)
    (total-strokes	 . 18)
    (ideographic-structure
     ((name . "IDEOGRAPHIC DESCRIPTION CHARACTER ABOVE TO BELOW")
      (=ucs		  . #x2FF1)	; ⿱
      )
     ((=ucs@unicode	  . #x9259)	; &AJ1-08653;
      (=adobe-japan1-2	  . 08653)	; &AJ1-08653;
      )
     ((=ucs		  . #x76BF)	; 皿
      ))
    (=ruimoku-v6	. #xE0FE)	; &RUI6-E0FE;
    ))
(define-char
  '((<-denotational
     ((=ucs		  . #x30012)	; 𰀒
      ))
    (ideographic-radical . 1)	; ⼀
    (ideographic-strokes . 9)
    (total-strokes	 . 10)
    (ideographic-structure
     ((name . "IDEOGRAPHIC DESCRIPTION CHARACTER ABOVE TO BELOW")
      (=ucs		  . #x2FF1)	; ⿱
      )
     ((=mj		  . 006328)	; &MJ006328;
      )
     ((=mj		  . 006328)	; &MJ006328;
      ))
    (=ruimoku-v6	. #xE0FF)	; &RUI6-E0FF;
    (->subsumptive
     ((==ruimoku-v6	  . #xE0FF)	; &g2-RUI6-E0FF;
      ))
    ))
(define-char
  '((ideographic-radical . 149)	; ⾔
    (ideographic-strokes . 7)
    (total-strokes	 . 14)
    (ideographic-structure
     ((name . "IDEOGRAPHIC DESCRIPTION CHARACTER LEFT TO RIGHT")
      (=ucs		  . #x2FF0)	; ⿰
      )
     ((=ucs		  . #x8A00)	; 言
      )
     ((ideographic-structure
       ((name . "IDEOGRAPHIC DESCRIPTION CHARACTER ABOVE TO BELOW")
	(=ucs		    . #x2FF1)	; ⿱
	)
       ((=ucs		    . #x5348)	; 午
	)
       ((=ucs		    . #x53E3)	; 口
	))
      ))
    (=ruimoku-v6	. #xE100)	; &RUI6-E100;
    ))
(define-char
  '((ideographic-radical . 10)	; ⼉
    (ideographic-strokes . 6)
    (total-strokes	 . 8)
    (ideographic-structure
     ((name . "IDEOGRAPHIC DESCRIPTION CHARACTER LEFT TO RIGHT")
      (=ucs		  . #x2FF0)	; ⿰
      )
     ((=ucs		  . #x5143)	; 元
      )
     ((=ucs		  . #x5143)	; 元
      ))
    (=ruimoku-v6	. #xE101)	; &RUI6-E101;
    (->subsumptive
     ((==ruimoku-v6	  . #xE101)	; &g2-RUI6-E101;
      ))
    ))
(define-char
  '((=ruimoku-v6	. #xE102)	; &RUI6-E102;
    ))
(define-char
  '((=ruimoku-v6	. #xE103)	; &RUI6-E103;
    ))
(define-char
  '((=ruimoku-v6	. #xE104)	; &RUI6-E104;
    ))
(define-char
  '((=ruimoku-v6	. #xE105)	; &RUI6-E105;
    ))
(define-char
  '((ideographic-radical . 31)	; ⼞
    (ideographic-strokes . 13)
    (total-strokes	 . 16)
    (ideographic-structure
     ((name . "IDEOGRAPHIC DESCRIPTION CHARACTER FULL SURROUND")
      (=ucs		  . #x2FF4)	; ⿴
      )
     ((=ucs		  . #x56D7)	; 囗
      )
     ((=ucs		  . #x8C88)	; 貈
      ))
    (=ucs		. #x2BB53)	; 𫭓
    (=ruimoku-v6	. #xE106)	; &I-RUI6-E106;
    (->subsumptive
     ((==ucs@JP/hanazono  . #x2BB53)	; &g2-hanaJU+2BB53;
      )
     ((==ruimoku-v6	  . #xE106)	; &g2-RUI6-E106;
      ))
    ))
(define-char
  '((<-denotational
     ((=>ruimoku-v6	  . #xE107)	; &A-RUI6-E107;
      ))
    (ideographic-radical . 85)	; ⽔
    (ideographic-strokes . 8)
    (total-strokes	 . 11)
    (ideographic-structure
     ((name . "IDEOGRAPHIC DESCRIPTION CHARACTER LEFT TO RIGHT")
      (=ucs		  . #x2FF0)	; ⿰
      )
     ((=ucs		  . #x6C35)	; 氵
      )
     ((=ucs@unicode	  . #x39B0)	; &GT-14580;
      (=cns11643-4	  . #x2654)	; &GT-14580; [06-52]
      (=daikanwa	  . 11556)	; &I-M-11556;
      ))
    (=ruimoku-v6	. #xE107)	; &RUI6-E107;
    ))
(define-char
  '((<-denotational
     ((=ucs		  . #x2DBA8)	; 𭮨
      ))
    (ideographic-radical . 79)	; ⽎
    (ideographic-strokes . 4)
    (total-strokes	 . 8)
    (ideographic-structure
     ((name . "IDEOGRAPHIC DESCRIPTION CHARACTER LEFT TO RIGHT")
      (=ucs		  . #x2FF0)	; ⿰
      )
     ((=ucs@unicode	  . #x5384)	; &AJ1-03837;
      (=adobe-japan1-0	  . 03837)	; &AJ1-03837;
      )
     ((=ucs@unicode	  . #x6BB3)	; &MJ014908;
      (=mj		  . 014908)	; &MJ014908;
      ))
    (=ucs@JP/hanazono	. #x2DBA8)	; &RUI6-E108;
    (=cbeta		. 00142)	; &RUI6-E108;
    (=ruimoku-v6	. #xE108)	; &I-RUI6-E108;
    (->subsumptive
     ((==ucs@JP/hanazono  . #x2DBA8)	; &g2-CB00142;
      (==cbeta		  . 00142)	; &g2-CB00142;
      (==ruimoku-v6	  . #xE108)	; &g2-CB00142;
      (->subsumptive
       ((===ruimoku-v6	    . #xE108)	; &R-RUI6-E108;
	)
       ((===cbeta	    . 00142)	; &R-CB00142;
	))
      ))
    ))
(define-char
  '((<-denotational
     ((=>ruimoku-v6	  . #xE109)	; &A-RUI6-E109;
      ))
    (ideographic-radical . 57)	; ⼸
    (ideographic-strokes . 12)
    (total-strokes	 . 15)
    (ideographic-structure
     ((name . "IDEOGRAPHIC DESCRIPTION CHARACTER LEFT TO RIGHT")
      (=ucs		  . #x2FF0)	; ⿰
      )
     ((=ucs		  . #x5F13)	; 弓
      )
     ((=ucs@unicode	  . #x7121)	; &AJ1-03777;
      (=adobe-japan1-0	  . 03777)	; &AJ1-03777;
      ))
    (=ruimoku-v6	. #xE109)	; &RUI6-E109;
    ))
(define-char
  '((<-denotational
     ((=>ruimoku-v6	  . #xE10A)	; &A-RUI6-E10A;
      ))
    (ideographic-radical . 196)	; ⿃
    (ideographic-strokes . 8)
    (total-strokes	 . 19)
    (ideographic-structure
     ((name . "IDEOGRAPHIC DESCRIPTION CHARACTER LEFT TO RIGHT")
      (=ucs		  . #x2FF0)	; ⿰
      )
     ((name		  . "CJK RADICAL C-SIMPLIFIED BIRD")
      (=ucs		  . #x2EE6)	; ⻦
      )
     ((=ucs@unicode	  . #x548E)	; &MJ057147;
      (=mj		  . 057147)	; &MJ057147;
      ))
    (=ruimoku-v6	. #xE10A)	; &RUI6-E10A;
    (<-simplified@CN
     ((=ucs-var-001	  . #x31230)	; &U-v001+31230;
      ))
    ))
(define-char
  '((<-denotational
     ((=ucs		  . #x2C510)	; 𬔐
      ))
    (ideographic-radical . 116)	; ⽳
    (ideographic-strokes . 11)
    (total-strokes	 . 16)
    (ideographic-structure
     ((name . "IDEOGRAPHIC DESCRIPTION CHARACTER ABOVE TO BELOW")
      (=ucs		  . #x2FF1)	; ⿱
      )
     ((=>ucs@iwds-1	  . #x7A74)	; &A-IWDSU+7A74;
      (=>iwds-1		  .  0223)	; &A-IWDSU+7A74;
      )
     ((=ucs@unicode	  . #x5807)	; &C2-343E;
      (=gb2312		  . #x5D40)	; &C2-343E; [61-32]
      (=cns11643-2	  . #x343E)	; &I-C2-343E; [20-30]
      (=daikanwa	  . 05212)	; &I-M-05212;
      ))
    (=ucs@JP/hanazono	. #x2C510)	; &RUI6-E10B;
    (=ruimoku-v6	. #xE10B)	; &RUI6-E10B;
    (->subsumptive
     ((==ucs@JP/hanazono  . #x2C510)	; &g2-hanaJU+2C510;
      (==ruimoku-v6	  . #xE10B)	; &g2-hanaJU+2C510;
      (->subsumptive
       ((===ruimoku-v6	    . #xE10B)	; &R-RUI6-E10B;
	))
      ))
    ))
(define-char
  '((=ruimoku-v6	. #xE10C)	; &RUI6-E10C;
    ))
(define-char
  '((<-denotational
     ((=>ruimoku-v6	  . #xE10D)	; &A-RUI6-E10D;
      ))
    (ideographic-radical . 66)	; ⽁
    (ideographic-strokes . 13)
    (total-strokes	 . 17)
    (ideographic-structure
     ((name . "IDEOGRAPHIC DESCRIPTION CHARACTER LEFT TO RIGHT")
      (=ucs		  . #x2FF0)	; ⿰
      )
     ((=ucs@unicode	  . #x6703)	; &AJ1-04171;
      (=adobe-japan1-0	  . 04171)	; &AJ1-04171;
      )
     ((=ucs		  . #x6535)	; 攵
      ))
    (=ruimoku-v6	. #xE10D)	; &RUI6-E10D;
    ))
(define-char
  '((<-denotational
     ((=ucs		  . #x30A29)	; 𰨩
      ))
    (ideographic-radical . 115)	; ⽲
    (ideographic-strokes . 7)
    (total-strokes	 . 12)
    (ideographic-structure
     ((name . "IDEOGRAPHIC DESCRIPTION CHARACTER LEFT TO RIGHT")
      (=ucs		  . #x2FF0)	; ⿰
      )
     ((=ucs		  . #x268FB)	; 𦣻
      )
     ((=ucs@unicode	  . #x79BE)	; &AJ1-01363;
      (=adobe-japan1-0	  . 01363)	; &AJ1-01363;
      ))
    (=ucs@JP/hanazono	. #x30A29)	; &RUI6-E10E;
    (=ruimoku-v6	. #xE10E)	; &RUI6-E10E;
    (->subsumptive
     ((==ucs@JP/hanazono  . #x30A29)	; &g2-hanaJU+30A29;
      )
     ((==ruimoku-v6	  . #xE10E)	; &g2-RUI6-E10E;
      ))
    ))
(define-char
  '((<-denotational
     ((=ucs		  . #x2B74A)	; 𫝊
      ))
    (ideographic-radical . 9)	; ⼈
    (ideographic-strokes . 9)
    (total-strokes	 . 11)
    (ideographic-structure
     ((name . "IDEOGRAPHIC DESCRIPTION CHARACTER LEFT TO RIGHT")
      (=ucs		  . #x2FF0)	; ⿰
      )
     ((=ucs		  . #x4EBB)	; 亻
      )
     ((=ucs@unicode	  . #x5C02)	; &AJ1-02704;
      (=adobe-japan1-0	  . 02704)	; &AJ1-02704;
      ))
    (=ucs@JP/hanazono	. #x2B74A)	; &MJ007022;
    (=mj		. 007022)	; &MJ007022;
    (=koseki		. 010800)	; &MJ007022;
    (=ruimoku-v6	. #xE10F)	; &I-RUI6-E10F;
    (<-formed
     ((=ucs@unicode	  . #x50B3)	; &MJ007023;
      (=mj		  . 007023)	; &MJ007023;
      ))
    (<-formed*sources
     koseki mj)
    (->subsumptive
     ((==ucs@JP/hanazono  . #x2B74A)	; &g2-MJ007022;
      (==mj		  . 007022)	; &g2-MJ007022;
      (==koseki		  . 010800)	; &g2-MJ007022;
      (==ruimoku-v6	  . #xE10F)	; &g2-MJ007022;
      (->subsumptive
       ((===mj		    . 007022)	; &R-MJ007022;
	)
       ((===ruimoku-v6	    . #xE10F)	; &R-RUI6-E10F;
	))
      ))
    ))
(define-char
  '((<-denotational
     ((=>ruimoku-v6	  . #xE0D6)	; &A-RUI6-E0D6;
      ))
    (ideographic-radical . 193)	; ⿀
    (ideographic-strokes . 9)
    (total-strokes	 . 19)
    (ideographic-structure
     ((name . "IDEOGRAPHIC DESCRIPTION CHARACTER ABOVE TO MIDDLE AND BELOW")
      (=ucs		  . #x2FF3)	; ⿳
      )
     ((=ucs		  . #x5927)	; 大
      )
     ((=ucs		  . #x821B)	; 舛
      )
     ((=ucs@unicode	  . #x9B32)	; &C1-586F;
      (=gb2312		  . #x582A)	; &C1-586F; [56-10]
      (=cns11643-1	  . #x586F)	; &I-C1-586F; [56-79]
      ))
    (=ruimoku-v6	. #xE110)	; &RUI6-E110;
    ))
(define-char
  '((=ruimoku-v6	. #xE111)	; &RUI6-E111;
    ))
(define-char
  '((<-denotational
     ((=ucs		  . #x256DC)	; 𥛜
      ))
    (ideographic-radical . 113)	; ⽰
    (ideographic-strokes . 11)
    (total-strokes	 . 16)
    (hanyu-dazidian	4 2407 6)
    (ideographic-structure
     ((name . "IDEOGRAPHIC DESCRIPTION CHARACTER LEFT TO RIGHT")
      (=ucs		  . #x2FF0)	; ⿰
      )
     ((=ucs		  . #x793A)	; 示
      )
     ((=ucs		  . #x8658)	; 虘
      ))
    (=gt		. 31952)	; &RUI6-E112;
    (=gt-pj-5		. #x5175)	; &RUI6-E112; [49-85]
    (=daikanwa		. 24816)	; &I-M-24816;
    (=cbeta		. 07572)	; &I-CB07572;
    (=ruimoku-v6	. #xE112)	; &I-RUI6-E112;
    (<-formed
     ((=ucs		  . #x8A5B)	; 詛
      ))
    (<-formed*sources
     jiyun)
    (<-same
     ((=ucs		  . #x8A5B)	; 詛
      (=daikanwa	  . 35390)	; &I-M-35390;
      ))
    (<-same*sources
     jiyun daikanwa)
    (->subsumptive
     ((==gt		  . 31952)	; &g2-GT-31952;
      (==daikanwa	  . 24816)	; &g2-GT-31952;
      (==cbeta		  . 07572)	; &g2-GT-31952;
      (==ruimoku-v6	  . #xE112)	; &g2-GT-31952;
      (->subsumptive
       ((===daikanwa	    . 24816)	; &R-M-24816;
	)
       ((===gt		    . 31952)	; &R-GT-31952;
	)
       ((=hanziku-6	    . #xF1B5)	; &HZK06-F1B5;
	)
       ((===ruimoku-v6	    . #xE112)	; &R-RUI6-E112;
	)
       ((===cbeta	    . 07572)	; &R-CB07572;
	))
      ))
    ))
(define-char
  '((ideographic-radical . 113)	; ⽰
    (ideographic-strokes . 8)
    (total-strokes	 . 13)
    (ideographic-structure
     ((name . "IDEOGRAPHIC DESCRIPTION CHARACTER ABOVE TO BELOW")
      (=ucs		  . #x2FF1)	; ⿱
      )
     ((=ucs		  . #x660E)	; 明
      )
     ((=ucs		  . #x793A)	; 示
      ))
    (=ucs		. #x31D57)	; 𱵗
    (=ruimoku-v6	. #xE113)	; &I-RUI6-E113;
    (->subsumptive
     ((==ucs@JP/hanazono  . #x31D57)	; &g2-hanaJU+31D57;
      (==ruimoku-v6	  . #xE113)	; &g2-hanaJU+31D57;
      (->subsumptive
       ((===ruimoku-v6	    . #xE113)	; &R-RUI6-E113;
	))
      ))
    ))
(define-char
  '((=ruimoku-v6	. #xE114)	; &RUI6-E114;
    ))
(define-char
  '((<-denotational
     ((=ucs		  . #x31230)	; 𱈰
      ))
    (ideographic-radical . 196)	; ⿃
    (ideographic-strokes . 8)
    (total-strokes	 . 19)
    (ideographic-structure
     ((name . "IDEOGRAPHIC DESCRIPTION CHARACTER LEFT TO RIGHT")
      (=ucs		  . #x2FF0)	; ⿰
      )
     ((=ucs@unicode	  . #x9CE5)	; &AJ1-03031;
      (=adobe-japan1-0	  . 03031)	; &I-AJ1-03031;
      )
     ((=ucs@jis		  . #x548E)	; &AJ1-04352;
      (=adobe-japan1-0	  . 04352)	; &AJ1-04352;
      ))
    (=ucs@JP/hanazono	. #x31230)	; &RUI6-E115;
    (=ruimoku-v6	. #xE115)	; &RUI6-E115;
    (->subsumptive
     ((==ucs@JP/hanazono  . #x31230)	; &g2-hanaJU+31230;
      (==ruimoku-v6	  . #xE115)	; &g2-hanaJU+31230;
      (->subsumptive
       ((===ruimoku-v6	    . #xE115)	; &R-RUI6-E115;
	))
      ))
    ))
(define-char
  '((=ruimoku-v6	. #xE116)	; &RUI6-E116;
    ))
(define-char
  '((=ruimoku-v6	. #xE117)	; &RUI6-E117;
    ))
(define-char
  '((=ruimoku-v6	. #xE118)	; &RUI6-E118;
    ))
(define-char
  '((<-denotational
     ((=ucs		  . #x2B2A3)	; 𫊣
      ))
    (ideographic-radical . 141)	; ⾌
    (ideographic-strokes . 11)
    (total-strokes	 . 17)
    (ideographic-structure
     ((name . "IDEOGRAPHIC DESCRIPTION CHARACTER SURROUND FROM UPPER LEFT")
      (=ucs		  . #x2FF8)	; ⿸
      )
     ((=ucs@unicode	  . #x864D)	; &AJ1-06479;
      (=adobe-japan1-0	  . 06479)	; &AJ1-06479;
      )
     ((=ucs@unicode	  . #x9B5A)	; &AJ1-01685;
      (=adobe-japan1-0	  . 01685)	; &AJ1-01685;
      ))
    (=ucs@JP/hanazono	. #x2B2A3)	; &RUI6-E119;
    (=ruimoku-v6	. #xE119)	; &RUI6-E119;
    (->subsumptive
     ((==ucs@JP/hanazono  . #x2B2A3)	; &g2-hanaJU+2B2A3;
      (==ruimoku-v6	  . #xE119)	; &g2-hanaJU+2B2A3;
      (->subsumptive
       ((===ruimoku-v6	    . #xE119)	; &R-RUI6-E119;
	))
      ))
    ))
(define-char
  '((<-denotational
     ((=ucs		  . #x2D910)	; 𭤐
      ))
    (ideographic-radical . 66)	; ⽁
    (ideographic-strokes . 11)
    (total-strokes	 . 15)
    (ideographic-structure
     ((name . "IDEOGRAPHIC DESCRIPTION CHARACTER LEFT TO RIGHT")
      (=ucs		  . #x2FF0)	; ⿰
      )
     ((=ucs@JP/hanazono	  . #x2B2A3)	; &RUI6-E119;
      (=ruimoku-v6	  . #xE119)	; &RUI6-E119;
      )
     ((=ucs		  . #x6534)	; 攴
      ))
    (=ucs@JP/hanazono	. #x2D910)	; &RUI6-E11A;
    (=cbeta		. 01078)	; &RUI6-E11A;
    (=ruimoku-v6	. #xE11A)	; &I-RUI6-E11A;
    (->subsumptive
     ((==ucs@JP/hanazono  . #x2D910)	; &g2-CB01078;
      (==cbeta		  . 01078)	; &g2-CB01078;
      (->subsumptive
       ((===cbeta	    . 01078)	; &R-CB01078;
	))
      )
     ((==ruimoku-v6	  . #xE11A)	; &g2-RUI6-E11A;
      ))
    ))
(define-char
  '((<-denotational
     ((=ucs		  . #x30329)	; 𰌩
      ))
    (ideographic-radical . 39)	; ⼦
    (ideographic-strokes . 8)
    (total-strokes	 . 11)
    (ideographic-structure
     ((name . "IDEOGRAPHIC DESCRIPTION CHARACTER ABOVE TO BELOW")
      (=ucs		  . #x2FF1)	; ⿱
      )
     ((=ucs@unicode	  . #x4F86)	; &AJ1-04141;
      (=adobe-japan1-0	  . 04141)	; &AJ1-04141;
      )
     ((=ucs		  . #x5B50)	; 子
      ))
    (=ucs@JP/hanazono	. #x30329)	; &RUI6-E11B;
    (=ruimoku-v6	. #xE11B)	; &RUI6-E11B;
    (->subsumptive
     ((==ucs@JP/hanazono  . #x30329)	; &g2-hanaJU+30329;
      (==ruimoku-v6	  . #xE11B)	; &g2-hanaJU+30329;
      (->subsumptive
       ((===ruimoku-v6	    . #xE11B)	; &R-RUI6-E11B;
	))
      ))
    ))
(define-char
  '((<-denotational
     ((=>ruimoku-v6	  . #xE11C)	; &A-RUI6-E11C;
      ))
    (ideographic-radical . 109)	; ⽬
    (ideographic-strokes . 11)
    (total-strokes	 . 16)
    (ideographic-structure
     ((name . "IDEOGRAPHIC DESCRIPTION CHARACTER ABOVE TO BELOW")
      (=ucs		  . #x2FF1)	; ⿱
      )
     ((name		  . "CJK RADICAL EYE")
      (=ucs		  . #x2EAB)	; ⺫
      )
     ((=ucs		  . #x86CA)	; 蛊
      ))
    (=ruimoku-v6	. #xE11C)	; &RUI6-E11C;
    (->subsumptive
     ((==ruimoku-v6	  . #xE11C)	; &g2-RUI6-E11C;
      ))
    ))
(define-char
  '((ideographic-radical . 7)	; ⼆
    (ideographic-strokes . 6)
    (total-strokes	 . 8)
    (ideographic-structure
     ((name . "IDEOGRAPHIC DESCRIPTION CHARACTER ABOVE TO BELOW")
      (=ucs		  . #x2FF1)	; ⿱
      )
     ((=ucs		  . #x4E94)	; 五
      )
     ((=ucs		  . #x4E94)	; 五
      ))
    (=ruimoku-v6	. #xE11D)	; &RUI6-E11D;
    (->subsumptive
     ((==ruimoku-v6	  . #xE11D)	; &g2-RUI6-E11D;
      ))
    ))
(define-char
  '((=ruimoku-v6	. #xE11E)	; &RUI6-E11E;
    ))
(define-char
  '((=ruimoku-v6	. #xE11F)	; &RUI6-E11F;
    ))
(define-char
  '((=ruimoku-v6	. #xE120)	; &RUI6-E120;
    ))
(define-char
  '((=ruimoku-v6	. #xE121)	; &RUI6-E121;
    ))
(define-char
  '((=ruimoku-v6	. #xE122)	; &RUI6-E122;
    ))
(define-char
  '((ideographic-radical . 120)	; ⽷
    (ideographic-strokes . 4)
    (total-strokes	 . 7)
    (ideographic-structure
     ((name . "IDEOGRAPHIC DESCRIPTION CHARACTER LEFT TO RIGHT")
      (=ucs		  . #x2FF0)	; ⿰
      )
     ((=ucs		  . #x7E9F)	; 纟
      )
     ((=ucs		  . #x3749)	; 㝉
      ))
    (=ucs		. #x7EBB)	; 纻
    (=iso-ir165		. #x2C42)	; &I-EGB-2C42; [12-34]
    (=ruimoku-v6	. #xE123)	; &I-RUI6-E123;
    (<-simplified@CN/misc
     ((=ucs		  . #x7D35)	; 紵
      ))
    (->subsumptive
     ((==ucs@unicode	  . #x7EBB)	; &g2-UU+7EBB;
      (==iso-ir165	  . #x2C42)	; &g2-UU+7EBB; [12-34]
      (==ruimoku-v6	  . #xE123)	; &g2-UU+7EBB;
      (->subsumptive
       ((===ruimoku-v6	    . #xE123)	; &R-RUI6-E123;
	)
       ((===ucs@unicode	    . #x7EBB)	; &R-UU+7EBB;
	))
      ))
    ))
(define-char
  '((=ruimoku-v6	. #xE124)	; &RUI6-E124;
    ))
(define-char
  '((=ruimoku-v6	. #xE125)	; &RUI6-E125;
    ))
(define-char
  '((<-denotational
     ((=ucs		  . #x2C4B5)	; 𬒵
      ))
    (ideographic-radical . 113)	; ⽰
    (ideographic-strokes . 6)
    (total-strokes	 . 11)
    (ideographic-structure
     ((name . "IDEOGRAPHIC DESCRIPTION CHARACTER ABOVE TO BELOW")
      (=ucs		  . #x2FF1)	; ⿱
      )
     ((=ucs@unicode	  . #x4EA6)	; &AJ1-03744;
      (=adobe-japan1-0	  . 03744)	; &AJ1-03744;
      )
     ((=ucs		  . #x793A)	; 示
      ))
    (=ucs@JP/hanazono	. #x2C4B5)	; &RUI6-E126;
    (=ruimoku-v6	. #xE126)	; &RUI6-E126;
    (->subsumptive
     ((==ucs@JP/hanazono  . #x2C4B5)	; &g2-hanaJU+2C4B5;
      (==ruimoku-v6	  . #xE126)	; &g2-hanaJU+2C4B5;
      (->subsumptive
       ((===ruimoku-v6	    . #xE126)	; &R-RUI6-E126;
	))
      ))
    ))
(define-char
  '((<-denotational
     ((=>ruimoku-v6	  . #xE127)	; &A-RUI6-E127;
      ))
    (ideographic-radical . 108)	; ⽫
    (ideographic-strokes . 13)
    (total-strokes	 . 18)
    (ideographic-structure
     ((name . "IDEOGRAPHIC DESCRIPTION CHARACTER ABOVE TO BELOW")
      (=ucs		  . #x2FF1)	; ⿱
      )
     ((=ucs@unicode	  . #x916A)	; &AJ1-03929;
      (=adobe-japan1-0	  . 03929)	; &AJ1-03929;
      )
     ((=ucs		  . #x76BF)	; 皿
      ))
    (=ruimoku-v6	. #xE127)	; &RUI6-E127;
    ))
(define-char
  '((ideographic-radical . 108)	; ⽫
    (ideographic-strokes . 7)
    (total-strokes	 . 12)
    (ideographic-structure
     ((name . "IDEOGRAPHIC DESCRIPTION CHARACTER ABOVE TO BELOW")
      (=ucs		  . #x2FF1)	; ⿱
      )
     ((=ucs		  . #x91C6)	; 釆
      )
     ((=ucs		  . #x76BF)	; 皿
      ))
    (=ruimoku-v6	. #xE128)	; &RUI6-E128;
    (->subsumptive
     ((==ruimoku-v6	  . #xE128)	; &g2-RUI6-E128;
      ))
    ))
(define-char
  '((<-denotational
     ((=>ruimoku-v6	  . #xE129)	; &A-RUI6-E129;
      ))
    (ideographic-radical . 86)	; ⽕
    (ideographic-strokes . 8)
    (total-strokes	 . 12)
    (ideographic-structure
     ((name . "IDEOGRAPHIC DESCRIPTION CHARACTER ABOVE TO BELOW")
      (=ucs		  . #x2FF1)	; ⿱
      )
     ((=ucs@unicode	  . #x5141)	; &MJ007209;
      (=mj		  . 007209)	; &MJ007209;
      )
     ((=ucs		  . #x7085)	; 炅
      ))
    (=ruimoku-v6	. #xE129)	; &RUI6-E129;
    ))
(define-char
  '((<-denotational
     ((=>ruimoku-v6	  . #xE12A)	; &A-RUI6-E12A;
      ))
    (ideographic-radical . 1)	; ⼀
    (ideographic-strokes . 6)
    (total-strokes	 . 7)
    (ideographic-structure
     ((name . "IDEOGRAPHIC DESCRIPTION CHARACTER LEFT TO RIGHT")
      (=ucs		  . #x2FF0)	; ⿰
      )
     ((=ucs		  . #x4E0D)	; 不
      )
     ((=ucs@unicode	  . #x4E47)	; &MJ006381;
      (=mj		  . 006381)	; &MJ006381;
      ))
    (=ruimoku-v6	. #xE12A)	; &RUI6-E12A;
    ))
(define-char
  '((<-denotational
     ((=>ruimoku-v6	  . #xE12B)	; &A-RUI6-E12B;
      ))
    (ideographic-radical . 177)	; ⾰
    (ideographic-strokes . 3)
    (total-strokes	 . 12)
    (ideographic-structure
     ((name . "IDEOGRAPHIC DESCRIPTION CHARACTER LEFT TO RIGHT")
      (=ucs		  . #x2FF0)	; ⿰
      )
     ((=ucs@unicode	  . #x9769)	; &AJ1-01461;
      (=adobe-japan1-0	  . 01461)	; &AJ1-01461;
      )
     ((=ucs		  . #x5B50)	; 子
      ))
    (=ruimoku-v6	. #xE12B)	; &RUI6-E12B;
    (->subsumptive
     ((==ruimoku-v6	  . #xE12B)	; &g2-RUI6-E12B;
      ))
    ))
(define-char
  '((<-denotational
     ((=>ruimoku-v6	  . #xE12C)	; &A-RUI6-E12C;
      ))
    (ideographic-radical . 75)	; ⽊
    (ideographic-strokes . 10)
    (total-strokes	 . 14)
    (ideographic-structure
     ((name . "IDEOGRAPHIC DESCRIPTION CHARACTER ABOVE TO BELOW")
      (=ucs		  . #x2FF1)	; ⿱
      )
     ((=ucs		  . #x6797)	; 林
      )
     ((=ucs		  . #x767D)	; 白
      ))
    (=ruimoku-v6	. #xE12C)	; &RUI6-E12C;
    ))
(define-char
  '((=ruimoku-v6	. #xE12D)	; &RUI6-E12D;
    ))
(define-char
  '((=ruimoku-v6	. #xE12E)	; &RUI6-E12E;
    ))
(define-char
  '((=ruimoku-v6	. #xE12F)	; &RUI6-E12F;
    ))
(define-char
  '((<-denotational
     ((=>ruimoku-v6	  . #xE130)	; &A-RUI6-E130;
      ))
    (ideographic-radical . 128)	; ⽿
    (ideographic-strokes . 5)
    (total-strokes	 . 11)
    (ideographic-structure
     ((name . "IDEOGRAPHIC DESCRIPTION CHARACTER LEFT TO RIGHT")
      (=ucs		  . #x2FF0)	; ⿰
      )
     ((=ucs		  . #x8033)	; 耳
      )
     ((=ucs@iso		  . #x20B22)	; &MJ032014;
      (=mj		  . 032014)	; &MJ032014;
      ))
    (=ruimoku-v6	. #xE130)	; &RUI6-E130;
    ))
(define-char
  '((<-denotational
     ((=ucs		  . #x23130)	; 𣄰
      ))
    (ideographic-radical . 71)	; ⽆
    (ideographic-strokes . 5)
    (total-strokes	 . 9)
    (ideographic-structure
     ((name . "IDEOGRAPHIC DESCRIPTION CHARACTER LEFT TO RIGHT")
      (=ucs		  . #x2FF0)	; ⿰
      )
     ((=ucs		  . #x65E1)	; 旡
      )
     ((=ucs		  . #x53EF)	; 可
      ))
    (=ruimoku-v6	. #xE131)	; &RUI6-E131;
    (->subsumptive
     ((==ruimoku-v6	  . #xE131)	; &g2-RUI6-E131;
      ))
    ))
(define-char
  '((<-denotational
     ((=>ruimoku-v6	  . #xE132)	; &A-RUI6-E132;
      ))
    (ideographic-radical . 162)	; ⾡
    (ideographic-strokes . 10)
    (total-strokes	 . 13)
    (ideographic-structure
     ((name . "IDEOGRAPHIC DESCRIPTION CHARACTER SURROUND FROM LOWER LEFT")
      (=ucs		  . #x2FFA)	; ⿺
      )
     ((name		  . "CJK RADICAL SIMPLIFIED WALK")
      (=ucs		  . #x2ECC)	; ⻌
      )
     ((=ucs		  . #x200B9)	; 𠂹
      ))
    (=ruimoku-v6	. #xE132)	; &RUI6-E132;
    (->subsumptive
     ((==ruimoku-v6	  . #xE132)	; &g2-RUI6-E132;
      ))
    ))
(define-char
  '((=ruimoku-v6	. #xE133)	; &RUI6-E133;
    ))
(define-char
  '((<-denotational
     ((=ucs		  . #x31687)	; 𱚇
      ))
    (ideographic-radical . 40)	; ⼧
    (ideographic-strokes . 7)
    (total-strokes	 . 10)
    (ideographic-structure
     ((name . "IDEOGRAPHIC DESCRIPTION CHARACTER ABOVE TO BELOW")
      (=ucs		  . #x2FF1)	; ⿱
      )
     ((=ucs		  . #x5B80)	; 宀
      )
     ((=ucs		  . #x8C9D)	; 貝
      ))
    (=ucs@JP/hanazono	. #x31687)	; &RUI6-E134;
    (=ruimoku-v6	. #xE134)	; &RUI6-E134;
    (->subsumptive
     ((==ucs@JP/hanazono  . #x31687)	; &g2-hanaJU+31687;
      (==ruimoku-v6	  . #xE134)	; &g2-hanaJU+31687;
      (->subsumptive
       ((===ruimoku-v6	    . #xE134)	; &R-RUI6-E134;
	))
      ))
    ))
(define-char
  '((=ruimoku-v6	. #xE135)	; &RUI6-E135;
    ))
(define-char
  '((=ruimoku-v6	. #xE136)	; &RUI6-E136;
    ))
(define-char
  '((<-denotational
     ((=ucs		  . #x303EF)	; 𰏯
      ))
    (ideographic-radical . 52)	; ⼳
    (ideographic-strokes . 2)
    (total-strokes	 . 5)
    (ideographic-structure
     ((name . "IDEOGRAPHIC DESCRIPTION CHARACTER ABOVE TO BELOW")
      (=ucs		  . #x2FF1)	; ⿱
      )
     ((=ucs		  . #x5E7A)	; 幺
      )
     ((=ucs		  . #x513F)	; 儿
      ))
    (=ucs@JP/hanazono	. #x303EF)	; &RUI6-E137;
    (=ruimoku-v6	. #xE137)	; &RUI6-E137;
    (->subsumptive
     ((==ucs@JP/hanazono  . #x303EF)	; &g2-hanaJU+303EF;
      (==ruimoku-v6	  . #xE137)	; &g2-hanaJU+303EF;
      (->subsumptive
       ((===ruimoku-v6	    . #xE137)	; &R-RUI6-E137;
	))
      ))
    ))
(define-char
  '((=ruimoku-v6	. #xE138)	; &RUI6-E138;
    ))
(define-char
  '((<-denotational
     ((=ucs		  . #x2CB90)	; 𬮐
      ))
    (ideographic-radical . 169)	; ⾨
    (ideographic-strokes . 10)
    (total-strokes	 . 18)
    (ideographic-structure
     ((name . "IDEOGRAPHIC DESCRIPTION CHARACTER SURROUND FROM ABOVE")
      (=ucs		  . #x2FF5)	; ⿵
      )
     ((=ucs		  . #x9580)	; 門
      )
     ((=ucs@unicode	  . #x65C5)	; &HD-JT-B272;
      (=gb2312		  . #x4243)	; &HD-JT-B272; [34-35]
      (=ks-x1001	  . #x5569)	; &I-K0-5569; [53-73]
      (=cns11643-1	  . #x556A)	; &I-C1-556A; [53-74]
      (=daikanwa	  . 13644)	; &I-M-13644;
      ))
    (=ucs@JP/hanazono	. #x2CB90)	; &RUI6-E139;
    (=ruimoku-v6	. #xE139)	; &RUI6-E139;
    (->subsumptive
     ((==ucs@JP/hanazono  . #x2CB90)	; &g2-hanaJU+2CB90;
      (==ruimoku-v6	  . #xE139)	; &g2-hanaJU+2CB90;
      (->subsumptive
       ((===ruimoku-v6	    . #xE139)	; &R-RUI6-E139;
	))
      ))
    ))
(define-char
  '((<-denotational
     ((=>ruimoku-v6	  . #xE13A)	; &A-RUI6-E13A;
      ))
    (ideographic-radical . 206)	; ⿍
    (ideographic-strokes . 11)
    (total-strokes	 . 24)
    (ideographic-structure
     ((name . "IDEOGRAPHIC DESCRIPTION CHARACTER ABOVE TO BELOW")
      (=ucs		  . #x2FF1)	; ⿱
      )
     ((=ucs		  . #x5211)	; 刑
      )
     ((=ucs		  . #x9F0E)	; 鼎
      ))
    (=ruimoku-v6	. #xE13A)	; &RUI6-E13A;
    ))
(define-char
  '((<-denotational
     ((=>ruimoku-v6	  . #xE13B)	; &A-RUI6-E13B;
      ))
    (ideographic-radical . 61)	; ⼼
    (ideographic-strokes . 9)
    (total-strokes	 . 12)
    (ideographic-structure
     ((name . "IDEOGRAPHIC DESCRIPTION CHARACTER LEFT TO RIGHT")
      (=ucs		  . #x2FF0)	; ⿰
      )
     ((=ucs		  . #x5FC4)	; 忄
      )
     ((=ucs@unicode	  . #x9996)	; &AJ1-02335;
      (=adobe-japan1-0	  . 02335)	; &AJ1-02335;
      ))
    (=ruimoku-v6	. #xE13B)	; &RUI6-E13B;
    ))
(define-char
  '((=ruimoku-v6	. #xE13C)	; &RUI6-E13C;
    ))
(define-char
  '((<-denotational
     ((=ucs		  . #x3207A)	; 𲁺
      ))
    (ideographic-radical . 149)	; ⾔
    (ideographic-strokes . 12)
    (total-strokes	 . 19)
    (ideographic-structure
     ((name . "IDEOGRAPHIC DESCRIPTION CHARACTER LEFT TO RIGHT")
      (=ucs		  . #x2FF0)	; ⿰
      )
     ((=ucs		  . #x8A00)	; 言
      )
     ((=ucs-var-003	  . #x904E)	; &U-v003+904E;
      ))
    (=ruimoku-v6	. #xE13D)	; &RUI6-E13D;
    ))
(define-char
  '((<-denotational
     ((=>ruimoku-v6	  . #xE13E)	; &A-RUI6-E13E;
      ))
    (ideographic-radical . 104)	; ⽧
    (ideographic-strokes . 9)
    (total-strokes	 . 14)
    (ideographic-structure
     ((name . "IDEOGRAPHIC DESCRIPTION CHARACTER SURROUND FROM UPPER LEFT")
      (=ucs		  . #x2FF8)	; ⿸
      )
     ((=ucs		  . #x7592)	; 疒
      )
     ((ideographic-structure
       ((name . "IDEOGRAPHIC DESCRIPTION CHARACTER ABOVE TO BELOW")
	(=ucs		    . #x2FF1)	; ⿱
	)
       ((=ucs@unicode	    . #x536F)	; &AJ1-01230;
	(=adobe-japan1-0    . 01230)	; &AJ1-01230;
	)
       ((name		    . "CJK RADICAL MEAT")
	(=ucs		    . #x2EBC)	; ⺼
	))
      ))
    (=ruimoku-v6	. #xE13E)	; &RUI6-E13E;
    ))
(define-char
  '((<-denotational
     ((=>ruimoku-v6	  . #xE13F)	; &A-RUI6-E13F;
      ))
    (ideographic-radical . 62)	; ⼽
    (ideographic-strokes . 6)
    (total-strokes	 . 10)
    (ideographic-structure
     ((name . "IDEOGRAPHIC DESCRIPTION CHARACTER LEFT TO RIGHT")
      (=ucs		  . #x2FF0)	; ⿰
      )
     ((=gt-k		  . 01821)	; &GT-K01821;
      )
     ((=ucs		  . #x6208)	; 戈
      ))
    (=ruimoku-v6	. #xE13F)	; &RUI6-E13F;
    ))
(define-char
  '((=ruimoku-v6	. #xE140)	; &RUI6-E140;
    ))
(define-char
  '((=ruimoku-v6	. #xE141)	; &RUI6-E141;
    ))
(define-char
  '((ideographic-radical . 109)	; ⽬
    (ideographic-strokes . 14)
    (total-strokes	 . 19)
    (ideographic-structure
     ((name . "IDEOGRAPHIC DESCRIPTION CHARACTER ABOVE TO BELOW")
      (=ucs		  . #x2FF1)	; ⿱
      )
     ((=ucs		  . #x4020)	; 䀠
      )
     ((=ucs		  . #x7678)	; 癸
      ))
    (=ucs		. #x2C46A)	; 𬑪
    (=ruimoku-v6	. #xE142)	; &I-RUI6-E142;
    (->subsumptive
     ((==ucs@JP/hanazono  . #x2C46A)	; &g2-hanaJU+2C46A;
      )
     ((==ruimoku-v6	  . #xE142)	; &g2-RUI6-E142;
      ))
    ))
(define-char
  '((<-denotational
     ((=>ruimoku-v6	  . #xE143)	; &A-RUI6-E143;
      ))
    (ideographic-radical . 163)	; ⾢
    (ideographic-strokes . 12)
    (total-strokes	 . 15)
    (ideographic-structure
     ((name . "IDEOGRAPHIC DESCRIPTION CHARACTER LEFT TO RIGHT")
      (=ucs		  . #x2FF0)	; ⿰
      )
     ((=ucs@cns		  . #x6562)	; &AJ1-01526;
      (=ucs@jis		  . #x6562)	; &AJ1-01526;
      (=ucs@ks		  . #x6562)	; &AJ1-01526;
      (=adobe-japan1-0	  . 01526)	; &AJ1-01526;
      )
     ((name		  . "CJK RADICAL CITY")
      (=ucs		  . #x2ECF)	; ⻏
      ))
    (=ruimoku-v6	. #xE143)	; &RUI6-E143;
    ))
(define-char
  '((=ruimoku-v6	. #xE144)	; &RUI6-E144;
    ))
(define-char
  '((<-denotational
     ((=>ruimoku-v6	  . #xE145)	; &A-RUI6-E145;
      ))
    (ideographic-radical . 70)	; ⽅
    (ideographic-strokes . 12)
    (total-strokes	 . 16)
    (ideographic-structure
     ((name . "IDEOGRAPHIC DESCRIPTION CHARACTER ABOVE TO BELOW")
      (=ucs		  . #x2FF1)	; ⿱
      )
     ((ideographic-structure
       ((name . "IDEOGRAPHIC DESCRIPTION CHARACTER LEFT TO RIGHT")
	(=ucs		    . #x2FF0)	; ⿰
	)
       ((=ucs@unicode	    . #x58F0)	; &AJ1-02656;
	(=adobe-japan1-0    . 02656)	; &AJ1-02656;
	)
       ((=ucs		    . #x6209)	; 戉
	))
      )
     ((=ucs		  . #x65B9)	; 方
      ))
    (=ruimoku-v6	. #xE145)	; &RUI6-E145;
    ))
(define-char
  '((=ruimoku-v6	. #xE146)	; &RUI6-E146;
    ))
(define-char
  '((<-denotational
     ((=>ruimoku-v6	  . #xE147)	; &A-RUI6-E147;
      ))
    (ideographic-radical . 44)	; ⼫
    (ideographic-strokes . 3)
    (total-strokes	 . 6)
    (ideographic-structure
     ((name . "IDEOGRAPHIC DESCRIPTION CHARACTER SURROUND FROM UPPER LEFT")
      (=ucs		  . #x2FF8)	; ⿸
      )
     ((=ucs		  . #x5C38)	; 尸
      )
     ((=ucs@unicode	  . #x4E47)	; &MJ006381;
      (=mj		  . 006381)	; &MJ006381;
      ))
    (=ruimoku-v6	. #xE147)	; &RUI6-E147;
    (->subsumptive
     ((==ruimoku-v6	  . #xE147)	; &g2-RUI6-E147;
      ))
    ))
(define-char
  '((=ruimoku-v6	. #xE148)	; &RUI6-E148;
    ))
(define-char
  '((<-denotational
     ((=>ruimoku-v6	  . #xE149)	; &A-RUI6-E149;
      ))
    (ideographic-radical . 141)	; ⾌
    (ideographic-strokes . 4)
    (total-strokes	 . 10)
    (ideographic-structure
     ((name . "IDEOGRAPHIC DESCRIPTION CHARACTER ABOVE TO BELOW")
      (=ucs		  . #x2FF1)	; ⿱
      )
     ((=ucs		  . #x516B)	; 八
      )
     ((=ucs@cns		  . #x864E)	; &MJ023272;
      (=ucs@jis		  . #x864E)	; &MJ023272;
      (=ucs@ks		  . #x864E)	; &MJ023272;
      (=mj		  . 023272)	; &MJ023272;
      ))
    (=ruimoku-v6	. #xE149)	; &RUI6-E149;
    ))
(define-char
  '((total-strokes	 . 10)
    (ideographic-structure
     ((name . "IDEOGRAPHIC DESCRIPTION CHARACTER ABOVE TO BELOW")
      (=ucs		  . #x2FF1)	; ⿱
      )
     ((=ucs		  . #x5401)	; 吁
      )
     ((=ucs		  . #x5FC3)	; 心
      ))
    (=ruimoku-v6	. #xE14A)	; &RUI6-E14A;
    ))
(define-char
  '((<-denotational
     ((=>ruimoku-v6	  . #xE14B)	; &A-RUI6-E14B;
      ))
    (ideographic-radical . 37)	; ⼤
    (ideographic-strokes . 2)
    (total-strokes	 . 5)
    (ideographic-structure
     ((name . "IDEOGRAPHIC DESCRIPTION CHARACTER ABOVE TO BELOW")
      (=ucs		  . #x2FF1)	; ⿱
      )
     ((=ucs		  . #x5927)	; 大
      )
     ((=ucs@unicode	  . #x535C)	; &AJ1-03708;
      (=adobe-japan1-0	  . 03708)	; &AJ1-03708;
      ))
    (=ruimoku-v6	. #xE14B)	; &RUI6-E14B;
    ))
(define-char
  '((<-denotational
     ((=ucs		  . #x2B340)	; 𫍀
      ))
    (ideographic-radical . 167)	; ⾦
    (ideographic-strokes . 15)
    (total-strokes	 . 23)
    (ideographic-structure
     ((name . "IDEOGRAPHIC DESCRIPTION CHARACTER LEFT TO RIGHT")
      (=ucs		  . #x2FF0)	; ⿰
      )
     ((=ucs		  . #x8A00)	; 言
      )
     ((=ucs@unicode	  . #x91D1)	; &AJ1-01754;
      (=adobe-japan1-0	  . 01754)	; &AJ1-01754;
      ))
    (=ucs@JP/hanazono	. #x2B340)	; &RUI6-E14C;
    (=ruimoku-v6	. #xE14C)	; &RUI6-E14C;
    (->subsumptive
     ((==ucs@JP/hanazono  . #x2B340)	; &g2-hanaJU+2B340;
      )
     ((==ruimoku-v6	  . #xE14C)	; &g2-RUI6-E14C;
      ))
    ))
(define-char
  '((=ruimoku-v6	. #xE14D)	; &RUI6-E14D;
    ))
(define-char
  '((<-denotational
     ((=ucs		  . #x2BC8C)	; 𫲌
      ))
    (ideographic-radical . 38)	; ⼥
    (ideographic-strokes . 14)
    (total-strokes	 . 17)
    (=ruimoku-v6	. #xE14E)	; &RUI6-E14E;
    (->subsumptive
     ((==ruimoku-v6	  . #xE14E)	; &g2-RUI6-E14E;
      ))
    ))
(define-char
  '((=ruimoku-v6	. #xE14F)	; &RUI6-E14F;
    ))
(define-char
  '((<-denotational
     ((=>ruimoku-v6	  . #xE150)	; &A-RUI6-E150;
      ))
    (ideographic-radical . 50)	; ⼱
    (ideographic-strokes . 8)
    (total-strokes	 . 11)
    (ideographic-structure
     ((name . "IDEOGRAPHIC DESCRIPTION CHARACTER LEFT TO RIGHT")
      (=ucs		  . #x2FF0)	; ⿰
      )
     ((=ucs		  . #x5DFE)	; 巾
      )
     ((=mj		  . 007646)	; &MJ007646;
      ))
    (=ruimoku-v6	. #xE150)	; &RUI6-E150;
    ))
(define-char
  '((<-denotational
     ((=ucs		  . #x2BF06)	; 𫼆
      ))
    (ideographic-radical . 62)	; ⼽
    (ideographic-strokes . 12)
    (total-strokes	 . 16)
    (ideographic-structure
     ((name . "IDEOGRAPHIC DESCRIPTION CHARACTER LEFT TO RIGHT")
      (=ucs		  . #x2FF0)	; ⿰
      )
     ((=ucs@unicode	  . #x83D0)	; &AJ1-18500;
      (=adobe-japan1-5	  . 18500)	; &AJ1-18500;
      )
     ((=ucs		  . #x6208)	; 戈
      ))
    (=ucs@JP/hanazono	. #x2BF06)	; &RUI6-E151;
    (=ruimoku-v6	. #xE151)	; &RUI6-E151;
    (->subsumptive
     ((==ucs@JP/hanazono  . #x2BF06)	; &g2-hanaJU+2BF06;
      )
     ((==ruimoku-v6	  . #xE151)	; &g2-RUI6-E151;
      ))
    ))
(define-char
  '((<-denotational
     ((=>ruimoku-v6	  . #xE152)	; &A-RUI6-E152;
      ))
    (ideographic-radical . 149)	; ⾔
    (ideographic-strokes . 12)
    (total-strokes	 . 19)
    (ideographic-structure
     ((name . "IDEOGRAPHIC DESCRIPTION CHARACTER LEFT TO RIGHT")
      (=ucs		  . #x2FF0)	; ⿰
      )
     ((=ucs		  . #x8A00)	; 言
      )
     ((=ucs-itaiji-001	  . #x9028)	; &U-i001+9028;
      ))
    (=ruimoku-v6	. #xE152)	; &RUI6-E152;
    (->subsumptive
     ((==ruimoku-v6	  . #xE152)	; &g2-RUI6-E152;
      ))
    ))
(define-char
  '((<-denotational
     ((=>ruimoku-v6	  . #xE153)	; &A-RUI6-E153;
      ))
    (ideographic-radical . 141)	; ⾌
    (ideographic-strokes . 8)
    (total-strokes	 . 14)
    (ideographic-structure
     ((name . "IDEOGRAPHIC DESCRIPTION CHARACTER SURROUND FROM UPPER LEFT")
      (=ucs		  . #x2FF8)	; ⿸
      )
     ((ideographic-structure
       ((name . "IDEOGRAPHIC DESCRIPTION CHARACTER SURROUND FROM UPPER LEFT")
	(=ucs		    . #x2FF8)	; ⿸
	)
       ((=ucs@unicode	    . #x864D)	; &AJ1-06479;
	(=adobe-japan1-0    . 06479)	; &AJ1-06479;
	)
       ((=ucs		    . #x5360)	; 占
	))
      )
     ((=ucs		  . #x5C71)	; 山
      ))
    (=ruimoku-v6	. #xE153)	; &RUI6-E153;
    ))
(define-char
  '((ideographic-radical . 108)	; ⽫
    (ideographic-strokes . 4)
    (total-strokes	 . 9)
    (ideographic-structure
     ((name . "IDEOGRAPHIC DESCRIPTION CHARACTER ABOVE TO BELOW")
      (=ucs		  . #x2FF1)	; ⿱
      )
     ((=ucs		  . #x5B54)	; 孔
      )
     ((=ucs		  . #x76BF)	; 皿
      ))
    (=ucs		. #x31C8D)	; 𱲍
    (=ruimoku-v6	. #xE154)	; &I-RUI6-E154;
    (->subsumptive
     ((==ucs@JP/hanazono  . #x31C8D)	; &g2-hanaJU+31C8D;
      (==ruimoku-v6	  . #xE154)	; &g2-hanaJU+31C8D;
      (->subsumptive
       ((===ruimoku-v6	    . #xE154)	; &R-RUI6-E154;
	))
      ))
    ))
(define-char
  '((ideographic-radical . 117)	; ⽴
    (ideographic-strokes . 6)
    (total-strokes	 . 11)
    (ideographic-structure
     ((name . "IDEOGRAPHIC DESCRIPTION CHARACTER ABOVE TO BELOW")
      (=ucs		  . #x2FF1)	; ⿱
      )
     ((=ucs		  . #x7ACB)	; 立
      )
     ((=ucs		  . #x200A4)	; 𠂤
      ))
    (=ruimoku-v6	. #xE155)	; &RUI6-E155;
    ))
(define-char
  '((=ruimoku-v6	. #xE156)	; &RUI6-E156;
    ))
(define-char
  '((<-denotational
     ((=>ruimoku-v6	  . #xE157)	; &A-RUI6-E157;
      ))
    (ideographic-radical . 205)	; ⿌
    (ideographic-strokes . 4)
    (total-strokes	 . 17)
    (ideographic-structure
     ((name . "IDEOGRAPHIC DESCRIPTION CHARACTER ABOVE TO BELOW")
      (=ucs		  . #x2FF1)	; ⿱
      )
     ((=ucs		  . #x5929)	; 天
      )
     ((=mj		  . 029952)	; &MJ029952;
      ))
    (=ruimoku-v6	. #xE157)	; &RUI6-E157;
    ))
(define-char
  '((<-denotational
     ((=ucs		  . #x2C2F4)	; 𬋴
      ))
    (ideographic-radical . 87)	; ⽖
    (ideographic-strokes . 12)
    (total-strokes	 . 16)
    (=ruimoku-v6	. #xE158)	; &RUI6-E158;
    (->subsumptive
     ((==ruimoku-v6	  . #xE158)	; &g2-RUI6-E158;
      ))
    ))
(define-char
  '((<-denotational
     ((=ucs		  . #x2A7E3)	; 𪟣
      ))
    (ideographic-radical . 19)	; ⼒
    (ideographic-strokes . 11)
    (total-strokes	 . 13)
    (ideographic-structure
     ((name . "IDEOGRAPHIC DESCRIPTION CHARACTER LEFT TO RIGHT")
      (=ucs		  . #x2FF0)	; ⿰
      )
     ((=ucs@JP/hanazono	  . #x2BAA1)	; &hanaJU+2BAA1;
      )
     ((=ucs		  . #x529B)	; 力
      ))
    (=ucs@JP/hanazono	. #x2A7E3)	; &RUI6-E159;
    (=ruimoku-v6	. #xE159)	; &RUI6-E159;
    (->subsumptive
     ((==ucs@JP/hanazono  . #x2A7E3)	; &g2-hanaJU+2A7E3;
      (==ruimoku-v6	  . #xE159)	; &g2-hanaJU+2A7E3;
      (->subsumptive
       ((===ruimoku-v6	    . #xE159)	; &R-RUI6-E159;
	))
      ))
    ))
(define-char
  '((<-denotational
     ((=>ruimoku-v6	  . #xE15A)	; &A-RUI6-E15A;
      ))
    (ideographic-radical . 163)	; ⾢
    (ideographic-strokes . 4)
    (total-strokes	 . 7)
    (ideographic-structure
     ((name . "IDEOGRAPHIC DESCRIPTION CHARACTER LEFT TO RIGHT")
      (=ucs		  . #x2FF0)	; ⿰
      )
     ((=ucs		  . #x592A)	; 太
      )
     ((name		  . "CJK RADICAL CITY")
      (=ucs		  . #x2ECF)	; ⻏
      ))
    (=ruimoku-v6	. #xE15A)	; &RUI6-E15A;
    ))
(define-char
  '((<-denotational
     ((=>ruimoku-v6	  . #xE15B)	; &A-RUI6-E15B;
      ))
    (ideographic-radical . 75)	; ⽊
    (ideographic-strokes . 6)
    (total-strokes	 . 10)
    (ideographic-structure
     ((name . "IDEOGRAPHIC DESCRIPTION CHARACTER ABOVE TO BELOW")
      (=ucs		  . #x2FF1)	; ⿱
      )
     ((=ucs		  . #x6797)	; 林
      )
     ((=ucs		  . #x52F9)	; 勹
      ))
    (=ruimoku-v6	. #xE15B)	; &RUI6-E15B;
    ))
(define-char
  '((ideographic-radical . 115)	; ⽲
    (ideographic-strokes . 0)
    (total-strokes	 . 5)
    (sound@ja/on	"kei" "gai")
    (=ucs		. #x2574C)	; 𥝌
    (=cns11643-4	. #x217E)	; &I-C4-217E; [01-94]
    (=daikanwa		. 24907)	; &I-M-24907;
    (=ruimoku-v6	. #xE15C)	; &I-RUI6-E15C;
    (->mistakable
     ((=ucs@unicode	  . #x79BE)	; &AJ1-01363;
      (=adobe-japan1-0	  . 01363)	; &AJ1-01363;
      (=daikanwa	  . 24906)	; &I-M-24906;
      ))
    (->mistakable*sources
     zihui-biansi-2char daikanwa)
    (->subsumptive
     ((==ucs@iso	  . #x2574C)	; &g2-M-24907;
      (==cns11643-4	  . #x217E)	; &g2-M-24907; [01-94]
      (==daikanwa	  . 24907)	; &g2-M-24907;
      (==ruimoku-v6	  . #xE15C)	; &g2-M-24907;
      (->subsumptive
       ((===daikanwa	    . 24907)	; &R-M-24907;
	)
       ((===cns11643-4	    . #x217E)	; &R-C4-217E; [01-94]
	)
       ((===ruimoku-v6	    . #xE15C)	; &R-RUI6-E15C;
	)
       ((===ucs@iso	    . #x2574C)	; &R-U-0002574C;
	))
      ))
    ))
(define-char
  '((<-denotational
     ((=>ruimoku-v6	  . #xE15D)	; &A-RUI6-E15D;
      ))
    (ideographic-radical . 205)	; ⿌
    (ideographic-strokes . 9)
    (total-strokes	 . 22)
    (ideographic-structure
     ((name . "IDEOGRAPHIC DESCRIPTION CHARACTER ABOVE TO BELOW")
      (=ucs		  . #x2FF1)	; ⿱
      )
     ((=ucs		  . #x67EC)	; 柬
      )
     ((=ucs@unicode	  . #x9EFD)	; &AJ1-07449;
      (=adobe-japan1-0	  . 07449)	; &AJ1-07449;
      ))
    (=ruimoku-v6	. #xE15D)	; &RUI6-E15D;
    ))
(define-char
  '((=ruimoku-v6	. #xE15E)	; &RUI6-E15E;
    ))
(define-char
  '((ideographic-structure
     ((name . "IDEOGRAPHIC DESCRIPTION CHARACTER SURROUND FROM ABOVE")
      (=ucs		  . #x2FF5)	; ⿵
      )
     ((ideographic-structure
       ((name . "IDEOGRAPHIC DESCRIPTION CHARACTER SURROUND FROM BELOW")
	(=ucs		    . #x2FF6)	; ⿶
	)
       ((=big5-cdp	    . #x89CC)	; &CDP-89CC;
	)
       ((=ucs		    . #x4E28)	; 丨
	))
      )
     ((=ucs		  . #x4E28)	; 丨
      ))
    (=ruimoku-v6	. #xE15F)	; &RUI6-E15F;
    ))
(define-char
  '((=ruimoku-v6	. #xE160)	; &RUI6-E160;
    ))
(define-char
  '((ideographic-radical . 29)	; ⼜
    (ideographic-strokes . 5)
    (total-strokes	 . 7)
    (ideographic-structure
     ((name . "IDEOGRAPHIC DESCRIPTION CHARACTER ABOVE TO BELOW")
      (=ucs		  . #x2FF1)	; ⿱
      )
     ((=ucs		  . #x4E4D)	; 乍
      )
     ((=ucs		  . #x53C8)	; 又
      ))
    (=ucs		. #x301BA)	; 𰆺
    (=ruimoku-v6	. #xE161)	; &I-RUI6-E161;
    (->subsumptive
     ((==ucs@JP/hanazono  . #x301BA)	; &g2-hanaJU+301BA;
      (==ruimoku-v6	  . #xE161)	; &g2-hanaJU+301BA;
      (->subsumptive
       ((===ruimoku-v6	    . #xE161)	; &R-RUI6-E161;
	))
      ))
    ))
(define-char
  '((<-denotational
     ((=>ruimoku-v6	  . #xE162)	; &A-RUI6-E162;
      ))
    (ideographic-radical . 53)	; ⼴
    (ideographic-strokes . 8)
    (total-strokes	 . 11)
    (ideographic-structure
     ((name . "IDEOGRAPHIC DESCRIPTION CHARACTER SURROUND FROM UPPER LEFT")
      (=ucs		  . #x2FF8)	; ⿸
      )
     ((=big5-cdp	  . #x8D56)	; &CDP-8D56;
      )
     ((=ucs@iso		  . #x27607)	; &CDP-8B67;
      (=big5-cdp	  . #x8B67)	; &CDP-8B67;
      ))
    (=ruimoku-v6	. #xE162)	; &RUI6-E162;
    (->subsumptive
     ((==ruimoku-v6	  . #xE162)	; &g2-RUI6-E162;
      ))
    ))
(define-char
  '((<-denotational
     ((=>ruimoku-v6	  . #xE163)	; &A-RUI6-E163;
      ))
    (ideographic-radical . 62)	; ⼽
    (ideographic-strokes . 2)
    (total-strokes	 . 6)
    (ideographic-structure
     ((name . "IDEOGRAPHIC DESCRIPTION CHARACTER SURROUND FROM LOWER RIGHT")
      (=ucs		  . #x2FFD)	; ⿽
      )
     ((=ucs		  . #x6208)	; 戈
      )
     ((=ucs		  . #xFA3C)	; 屮
      ))
    (=ruimoku-v6	. #xE163)	; &RUI6-E163;
    ))
(define-char
  '((<-denotational
     ((=>ruimoku-v6	  . #xE164)	; &A-RUI6-E164;
      ))
    (ideographic-radical . 50)	; ⼱
    (ideographic-strokes . 10)
    (total-strokes	 . 13)
    (ideographic-structure
     ((name . "IDEOGRAPHIC DESCRIPTION CHARACTER LEFT TO RIGHT")
      (=ucs		  . #x2FF0)	; ⿰
      )
     ((=ucs-itaiji-011	  . #x38C7)	; &U-i011+38C7;
      )
     ((=ucs-itaiji-002	  . #x5E0C)	; &U-i002+5E0C;
      ))
    (=ruimoku-v6	. #xE164)	; &RUI6-E164;
    ))
(define-char
  '((<-denotational
     ((=ucs		  . #x304A6)	; 𰒦
      ))
    (ideographic-radical . 62)	; ⼽
    (ideographic-strokes . 5)
    (total-strokes	 . 9)
    (ideographic-structure
     ((name . "IDEOGRAPHIC DESCRIPTION CHARACTER SURROUND FROM UPPER RIGHT")
      (=ucs		  . #x2FF9)	; ⿹
      )
     ((=ucs		  . #x6208)	; 戈
      )
     ((=ucs		  . #x2F81A)	; 冬
      ))
    (=ruimoku-v6	. #xE165)	; &RUI6-E165;
    (->subsumptive
     ((==ruimoku-v6	  . #xE165)	; &g2-RUI6-E165;
      ))
    ))
(define-char
  '((<-denotational
     ((=>ruimoku-v6	  . #xE166)	; &A-RUI6-E166;
      ))
    (ideographic-radical . 75)	; ⽊
    (ideographic-strokes . 9)
    (total-strokes	 . 13)
    (ideographic-structure
     ((name . "IDEOGRAPHIC DESCRIPTION CHARACTER ABOVE TO BELOW")
      (=ucs		  . #x2FF1)	; ⿱
      )
     ((=ucs		  . #x6728)	; 木
      )
     ((=ucs@unicode	  . #x58F4)	; &AJ1-17467;
      (=adobe-japan1-5	  . 17467)	; &AJ1-17467;
      ))
    (=ruimoku-v6	. #xE166)	; &RUI6-E166;
    ))
(define-char
  '((ideographic-radical . 85)	; ⽔
    (ideographic-strokes . 5)
    (total-strokes	 . 8)
    (ideographic-structure
     ((name . "IDEOGRAPHIC DESCRIPTION CHARACTER LEFT TO RIGHT")
      (=ucs		  . #x2FF0)	; ⿰
      )
     ((=ucs		  . #x6C35)	; 氵
      )
     ((=ucs		  . #x4E19)	; 丙
      ))
    (=ucs		. #x2C1DE)	; 𬇞
    (=ruimoku-v6	. #xE167)	; &I-RUI6-E167;
    (->subsumptive
     ((==ucs@JP/hanazono  . #x2C1DE)	; &g2-hanaJU+2C1DE;
      (==ruimoku-v6	  . #xE167)	; &g2-hanaJU+2C1DE;
      (->subsumptive
       ((===ruimoku-v6	    . #xE167)	; &R-RUI6-E167;
	))
      ))
    ))
(define-char
  '((<-denotational
     ((=+>ucs@unicode	  . #x4EA6)	; &o-J0-4B72;
      (=+>jis-x0208	  . #x4B72)	; &o-J0-4B72; [43-82]
      ))
    (ideographic-radical . 8)	; ⼇
    (ideographic-strokes . 4)
    (total-strokes	 . 6)
    (=ucs		. #x3005C)	; 𰁜
    (=ruimoku-v6	. #xE168)	; &I-RUI6-E168;
    (<-simplified@CN/component
     ((=ucs@unicode	  . #x470C)	; &AJ1-18690;
      (=adobe-japan1-5	  . 18690)	; &AJ1-18690;
      )
     ((=ucs		  . #x9FBB)	; 龻
      ))
    (->subsumptive
     ((==ucs@JP/hanazono  . #x3005C)	; &g2-hanaJU+3005C;
      (==ruimoku-v6	  . #xE168)	; &g2-hanaJU+3005C;
      (->subsumptive
       ((===ruimoku-v6	    . #xE168)	; &R-RUI6-E168;
	))
      ))
    ))
(define-char
  '((<-denotational
     ((=>ruimoku-v6	  . #xE169)	; &A-RUI6-E169;
      ))
    (ideographic-radical . 141)	; ⾌
    (ideographic-strokes . 9)
    (total-strokes	 . 15)
    (ideographic-structure
     ((name . "IDEOGRAPHIC DESCRIPTION CHARACTER SURROUND FROM UPPER LEFT")
      (=ucs		  . #x2FF8)	; ⿸
      )
     ((=ucs		  . #x864D)	; 虍
      )
     ((=hanyo-denshi/tk	  . 01031610)	; &HD-TK-01031610;
      ))
    (=ruimoku-v6	. #xE169)	; &RUI6-E169;
    ))
(define-char
  '((<-denotational
     ((=>ruimoku-v6	  . #xE16A)	; &A-RUI6-E16A;
      ))
    (ideographic-radical . 149)	; ⾔
    (ideographic-strokes . 6)
    (total-strokes	 . 13)
    (ideographic-structure
     ((name . "IDEOGRAPHIC DESCRIPTION CHARACTER LEFT TO RIGHT")
      (=ucs		  . #x2FF0)	; ⿰
      )
     ((=ucs		  . #x8A00)	; 言
      )
     ((=ucs		  . #x7F36)	; 缶
      ))
    (=ruimoku-v6	. #xE16A)	; &RUI6-E16A;
    ))
(define-char
  '((ideographic-radical . 30)	; ⼝
    (ideographic-strokes . 6)
    (total-strokes	 . 9)
    (ideographic-structure
     ((name . "IDEOGRAPHIC DESCRIPTION CHARACTER LEFT TO RIGHT")
      (=ucs		  . #x2FF0)	; ⿰
      )
     ((=ucs		  . #x53E5)	; 句
      )
     ((=ucs		  . #x592B)	; 夫
      ))
    (=ruimoku-v6	. #xE16B)	; &RUI6-E16B;
    ))
(define-char
  '((ideographic-radical . 13)	; ⼌
    (ideographic-strokes . 11)
    (total-strokes	 . 13)
    (ideographic-structure
     ((name . "IDEOGRAPHIC DESCRIPTION CHARACTER LEFT TO RIGHT")
      (=ucs		  . #x2FF0)	; ⿰
      )
     ((=ucs		  . #x5CA1)	; 岡
      )
     ((=ucs		  . #x53BB)	; 去
      ))
    (=ucs		. #x2B937)	; 𫤷
    (=ruimoku-v6	. #xE16C)	; &I-RUI6-E16C;
    (->subsumptive
     ((==ucs@JP/hanazono  . #x2B937)	; &g2-hanaJU+2B937;
      )
     ((==ruimoku-v6	  . #xE16C)	; &g2-RUI6-E16C;
      ))
    ))
(define-char
  '((<-denotational
     ((=>ruimoku-v6	  . #xE16D)	; &A-RUI6-E16D;
      ))
    (ideographic-radical . 62)	; ⼽
    (ideographic-strokes . 17)
    (total-strokes	 . 21)
    (ideographic-structure
     ((name . "IDEOGRAPHIC DESCRIPTION CHARACTER LEFT TO RIGHT")
      (=ucs		  . #x2FF0)	; ⿰
      )
     ((=ucs@unicode	  . #x76E7)	; &AJ1-05807;
      (=adobe-japan1-0	  . 05807)	; &AJ1-05807;
      )
     ((=ucs		  . #x620A)	; 戊
      ))
    (=ruimoku-v6	. #xE16D)	; &RUI6-E16D;
    ))
(define-char
  '((<-denotational
     ((=>ruimoku-v6	  . #xE16E)	; &A-RUI6-E16E;
      ))
    (ideographic-radical . 212)	; ⿓
    (ideographic-strokes . 21)
    (total-strokes	 . 37)
    (ideographic-structure
     ((name . "IDEOGRAPHIC DESCRIPTION CHARACTER ABOVE TO BELOW")
      (=ucs		  . #x2FF1)	; ⿱
      )
     ((=ucs		  . #x971D)	; 霝
      )
     ((ideographic-structure
       ((name . "IDEOGRAPHIC DESCRIPTION CHARACTER SURROUND FROM LOWER LEFT")
	(=ucs		    . #x2FFA)	; ⿺
	)
       ((=ucs		    . #x9F8D)	; 龍
	)
       ((=ucs		    . #x738B)	; 王
	))
      ))
    (=ruimoku-v6	. #xE16E)	; &RUI6-E16E;
    (<-unknown
     ((=ucs		  . #x9F97)	; 龗
      ))
    (->subsumptive
     ((==ruimoku-v6	  . #xE16E)	; &g2-RUI6-E16E;
      ))
    ))
